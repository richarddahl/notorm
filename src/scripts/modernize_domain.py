...
LEGACY_PATTERNS = [
    # Legacy imports
    (r"from uno\.domain\.core import Entity", "from uno.domain.models import Entity"),
    (
        r"from uno\.domain\.core import (.*Entity.*)",
        r"from uno.domain.models import \1",
    ),
    (r"from uno\.domain\.core import (.+)", r"from uno.domain.models import \1"),
    (r"from uno\.domain import core", "from uno.domain import models"),
    ...
    (r"class\s+(\w+)\s*\(\s*core\.ValueObject\s*\)", r"class \1(models.ValueObject)"),
    ...
    # Legacy method calls
    (r"\.register_domain_event\(", ".register_event("),
    (r"\.get_domain_events\(", ".clear_events("),
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Modernize domain model code in uno framework"
    )
    parser.add_argument(
        "--scan",
        action="store_true",
        help="Only scan for legacy patterns without modifying files",
    )
    parser.add_argument(
