...
class AttributeCLI:
                ...
                json.dumps(
                    {
                        ...
                        "created_at": (
                            created_type.created_at.isoformat()
                            if created_type.created_at
                            else None
                        ),
                    },
                    indent=2,
                    ...
                    json.dumps(
                        {
                            ...
                            "describes": (
                                [
                                    ...
                                    for mt in attribute_type.describes
                                ]
                            ...
                            "value_types": (
                                [
                                    {"id": mt.id, "name": mt.name}
                                    for mt in attribute_type.value_types
                                ]
    ...
    async def list_attribute_types(self, args):
        ...
        if args.meta_type_id:
            ...
            result = await self.attribute_type_service.get_applicable_attribute_types(
                args.meta_type_id
            )
        ...
        for idx, at in enumerate(attribute_types, 1):
            ...
            print(
                f"   Required: {at.required}, Multiple Allowed: {at.multiple_allowed}"
            )
            if at.describes:
                ...
                json.dumps(
                    {
                        ...
                        "values": (
                            [
                                {"id": v.id, "name": getattr(v, "name", None)}
                                for v in created_attribute.values
                            ]
            ...
            for idx, attr in enumerate(attributes, 1):
                ...
                if not args.no_values and attr.values:
                    ...
                    for v_idx, value in enumerate(attr.values, 1):
                        print(
                            f"      {v_idx}. ID: {value.id}, Name: {getattr(value, 'name', None)}"
                        )
                print()
        else:
            error = result.error
...
def setup_attribute_type_subparsers(subparsers):
    ...
    # Create attribute type
    create_at_parser = subparsers.add_parser(
        "create", help="Create a new attribute type"
    )
    create_at_parser.add_argument(
        "--name", required=True, help="Name of the attribute type"
    )
    create_at_parser.add_argument(
        "--text", required=True, help="Text description of the attribute type"
    )
    create_at_parser.add_argument("--parent-id", help="ID of the parent attribute type")
    create_at_parser.add_argument(
        "--required",
        action="store_true",
        help="Whether attributes of this type are required",
    )
    create_at_parser.add_argument(
        "--multiple-allowed",
        action="store_true",
        help="Whether multiple values are allowed",
    )
    create_at_parser.add_argument(
        "--comment-required", action="store_true", help="Whether a comment is required"
    )
    create_at_parser.add_argument(
        "--display-with-objects",
        action="store_true",
        help="Whether to display with objects",
    )
    create_at_parser.add_argument("--initial-comment", help="Initial comment template")
    create_at_parser.add_argument(
        "--meta-type-ids",
        nargs="+",
        help="IDs of meta types this attribute type applies to",
    )
    create_at_parser.add_argument(
        "--value-type-ids", nargs="+", help="IDs of meta types allowed as values"
    )
    create_at_parser.set_defaults(
        func=lambda cli, args: cli.create_attribute_type(args)
    )
    ...
    list_at_parser.add_argument(
        "--meta-type-id", help="List attribute types applicable to this meta type"
    )
    ...
    delete_at_parser.set_defaults(
        func=lambda cli, args: cli.delete_attribute_type(args)
    )
...
def setup_attribute_subparsers(subparsers):
    ...
    create_parser.add_argument(
        "--attribute-type-id", required=True, help="ID of the attribute type"
    )
    ...
    create_parser.add_argument(
        "--value-ids", nargs="+", help="IDs of values to associate with the attribute"
    )
    ...
    # Add values
    add_values_parser = subparsers.add_parser(
        "add-values", help="Add values to an attribute"
    )
    ...
    # Remove values
    remove_values_parser = subparsers.add_parser(
        "remove-values", help="Remove values from an attribute"
    )
    remove_values_parser.add_argument("id", help="ID of the attribute")
    remove_values_parser.add_argument(
        "value_ids", nargs="+", help="IDs of values to remove"
    )
    remove_values_parser.set_defaults(func=lambda cli, args: cli.remove_values(args))

    # Get attributes for record
    get_for_record_parser = subparsers.add_parser(
        "get-for-record", help="Get attributes for a record"
    )
    get_for_record_parser.add_argument("record_id", help="ID of the record")
    get_for_record_parser.add_argument(
        "--no-values", action="store_true", help="Don't include attribute values"
    )
    get_for_record_parser.set_defaults(
        func=lambda cli, args: cli.get_attributes_for_record(args)
    )
...
def setup_parser():
    ...
    attribute_types_parser = subparsers.add_parser(
        "attribute-types", help="Manage attribute types"
    )
    attribute_types_subparsers = attribute_types_parser.add_subparsers(
        dest="subcommand", help="Attribute type command to run"
    )
    ...
    attributes_subparsers = attributes_parser.add_subparsers(
        dest="subcommand", help="Attribute command to run"
    )
