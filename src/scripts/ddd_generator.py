...
class DDDGenerator:
    ...
    def __init__(self, module_path: str, module_name: str):
        ...
        if not re.match(r"^[a-z][a-z0-9_]*$", module_name):
            logger.error(
                f"Module name '{module_name}' is not valid. "
    ...
    def create_entity(
        ...
        if not re.match(r"^[A-Z][a-zA-Z0-9]*$", entity_class_name):
            logger.error(
                f"Entity name '{entity_class_name}' is not valid. "
        ...
        if f"class {entity_class_name}" in content:
            logger.error(
                f"Entity '{entity_class_name}' already exists in {entities_path}"
            )
            sys.exit(1)
    ...
    def create_repository(
        self, entity_name: str, custom_queries: Optional[List[Dict[str, Any]]] = None
    ) -> Path:
        """
        ...
        if f"class {repository_class_name}" in content:
            logger.error(
                f"Repository '{repository_class_name}' already exists in {repo_path}"
            )
            sys.exit(1)
    ...
    def create_service(
        self, entity_name: str, custom_methods: Optional[List[Dict[str, Any]]] = None
    ) -> Path:
        """
        ...
        if f"class {service_class_name}" in content:
            logger.error(
                f"Service '{service_class_name}' already exists in {service_path}"
            )
            sys.exit(1)
    ...
    def create_endpoints(
        ...
        custom_endpoints: Optional[Dict[str, List[Dict[str, Any]]]] = None,
    ) -> Path:
        """
    ...
    def create_module(self, entity_specs: List[Dict[str, Any]]) -> None:
        ...
        for spec in entity_specs:
            ...
            self.create_repository(
                entity_name=entity_name, custom_queries=spec.get("custom_queries")
            )

            # Create service
            self.create_service(
                entity_name=entity_name, custom_methods=spec.get("custom_methods")
            )
        ...
        logger.info(
            f"Successfully created DDD module '{self.module_name}' with {len(entity_names)} entities"
        )
    ...
    def _update_init_file_for_repository(self, repository_class_name: str) -> None:
        ...
        if init_path.exists():
            ...
            if (
                f"from .domain_repositories import {repository_class_name}"
                in init_content
            ):
                return
    ...
    def _update_init_file_for_endpoints(self, entity_class_names: List[str]) -> None:
        """Update __init__.py to expose the endpoint functions."""
        router_funcs = [
            f"create_{self._to_snake_case(name)}_router" for name in entity_class_names
        ]
    ...
    def _generate_repository_code(
        ...
        if custom_queries:
            for query in custom_queries:
                ...
                param_docs = "\n        ".join(
                    [
                        f"{p['name']}: {p.get('description', 'Field value')}"
                        for p in params
                    ]
                )
    ...
    def _generate_service_code(
        ...
        if custom_methods:
            for method in custom_methods:
                ...
                param_docs = "\n        ".join(
                    [
                        f"{p['name']}: {p.get('description', 'Parameter description')}"
                        for p in params
                    ]
                )
    ...
    def _generate_provider_code(
        ...
        for i, service_class in enumerate(service_class_names):
            ...
            registration_code.append(
                f"    register_service({service_class}, depends=[{repo_class}])"
            )
    ...
    def _generate_endpoints_code(
                ...
                for endpoint in custom_endpoints[entity_name]:
                    ...
                    for param in params:
                        ...
                        if param.get("source") == "path":
                            param_list.append(
                                f"{param['name']}: {param_type} = Path(..., description=\"Parameter description\")"
                            )
                        elif param.get("source") == "query":
                            param_list.append(
                                f"{param['name']}: {param_type} = Query(None, description=\"Parameter description\")"
                            )
                    ...
                    param_list.append(
                        f"service: {service_name} = Depends(lambda: get_service({service_name}))"
                    )
                    params_str = ", ".join(param_list)
                    ...
                    custom_endpoint_code += textwrap.dedent(
                        f'''
        ...
        router_calls = [
            f"    app.include_router(create_{name}_router())" for name in snake_names
        ]
        router_calls_str = "\n".join(router_calls)

        main_router_func = textwrap.dedent(
            f'''
        ...
        '''
            ...
            service_imports=", ".join(
                [f"{name}Service" for name in entity_class_names]
            ),
        )
...
def main():
    ...
    entity_parser.add_argument(
        "--fields", help="Comma-separated list of fields in format 'name:type'"
    )
    entity_parser.add_argument(
        "--aggregate",
        action="store_true",
    ...
    entity_parser.add_argument("--description", help="Description for the entity class")

    # Repository command
    repo_parser = subparsers.add_parser(
        "repository", help="Generate a domain repository"
    )
    ...
    provider_parser = subparsers.add_parser(
        "provider", help="Generate a domain provider"
    )
    ...
    provider_parser.add_argument(
        "entity_names", nargs="+", help="Names of the entity classes"
    )

    # Endpoints command
    endpoints_parser = subparsers.add_parser(
        "endpoints", help="Generate domain endpoints"
    )
    ...
    module_parser = subparsers.add_parser(
        "module", help="Generate a complete DDD module"
    )
    ...
    try:
        if args.command == "entity":
            ...
            for spec in args.entity_specs:
                ...
                entity_specs.append(
                    {"name": entity_name, "fields": fields, "is_aggregate": True}
                )
