...
class ImportModernizer:
    ...
    def __init__(self, base_dir: str, remove_compat: bool = False, dry_run: bool = False, auto_fix: bool = False):
        ...
        for pattern, replacement, import_path in self.LEGACY_CLASS_REPLACEMENTS:
            if re.search(pattern, modified_content):
                ...
                for i, line in enumerate(lines):
                    ...
                    # Skip if it's an alias definition like "UnoModel = BaseModel"
                    if re.search(f"{clean_pattern} = {replacement}", line):
                        continue
                    
                    # Skip if it's a backward compatibility line like "BaseModel = UnoModel"
                ...
                for i, line in enumerate(lines):
                    ...
                    if (re.search(f"{clean_pattern} = {replacement}", line) or 
                        re.search(f"{replacement} = {clean_pattern}", line)):
                        continue
                    ...
                    if i <= insert_pos:
                        if re.match(r'^"""', line) or re.match(r"^'''", line):
                            ...
                            for j in range(i + 1, len(lines)):
                                if re.search(r'"""$', lines[j]) or re.search(r"'''$", lines[j]):
                                    insert_pos = j + 1
                                    break
            ...
            while i < len(lines):
                ...
                for pattern in self.BACKWARD_COMPAT_PATTERNS:
                    if re.search(pattern, line):
                        ...
                        if i+1 < len(lines) and re.match(r'^[A-Za-z0-9_]+ = [A-Za-z0-9_]+', lines[i+1].strip()):
                            print(f"    Removing alias with comment: {lines[i+1].strip()}")
                            i += 2
                            continue
...
def main():
    parser = argparse.ArgumentParser(description="Modernize imports in the Uno framework")
    parser.add_argument("--remove-compat", action="store_true", help="Remove backward compatibility layers")
