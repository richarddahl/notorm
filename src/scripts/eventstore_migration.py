...
from uno.core.events import Event, EventStore, InMemoryEventStore, PostgresEventStore, PostgresEventStoreConfig
...
async def migrate_events(config: MigrationConfig) -> int:
    ...
    logger.info(f"Starting event migration from {config.source_type} to {config.target_type}")
    
    # Create source store
    source_store = create_event_store(
        store_type=config.source_type,
        config=config,
        logger=logger
    )
    ...
    async def get_all_source_events() -> List[Event]:
        ...
        if isinstance(source_store, InMemoryEventStore):
            ...
            logger.warning("Using generic approach to get all events - this might be slow")
    ...
    for aggregate_id, aggregate_events in events_by_aggregate.items():
        logger.info(f"Migrating {len(aggregate_events)} events for aggregate {aggregate_id}")
        ...
        for i in range(0, len(aggregate_events), config.batch_size):
            ...
            try:
                ...
                logger.info(f"Migrated batch of {len(batch)} events for aggregate {aggregate_id}")
            except Exception as e:
                logger.error(f"Error migrating events for aggregate {aggregate_id}: {str(e)}")
                raise
...
def create_event_store(store_type: str, config: MigrationConfig, logger: logging.Logger) -> Optional[EventStore]:
    """
    ...
    if store_type == "memory":
        ...
        logger.info(f"Creating PostgresEventStore with schema {config.schema}, table {config.table_name}")
        ...
        return PostgresEventStore(config=postgres_config, event_class=config.event_class)
    else:
...
async def main():
    ...
    parser.add_argument("--source", required=True, help="Source event store type (memory, postgres)")
    parser.add_argument("--target", required=True, help="Target event store type (memory, postgres)")
    ...
    parser.add_argument("--table", default="domain_events", help="PostgreSQL table name")
    parser.add_argument("--batch-size", type=int, default=100, help="Batch size for migrations")
    parser.add_argument("--no-create-schema", action="store_true", help="Don't create schema if missing")
    parser.add_argument("--no-notifications", action="store_true", help="Don't use PostgreSQL notifications")
