...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--fix", action="store_true", help="Fix issues instead of just reporting them"
    )
    ...
    parser.add_argument(
        "--check-images", action="store_true", help="Check for broken image references"
    )
...
def standardize_headings(file_path: Path, fix: bool = False) -> list[str]:
    ...
    if headings:
        ...
        if current_level != 1:
            issues.append(
                f"First heading should be level 1 (# Heading), found level {current_level}"
            )
        ...
        for markers, text in headings[1:]:
            ...
            if level > prev_level + 1:
                issues.append(
                    f"Skipped heading level: {prev_level} to {level} ({text})"
                )
    ...
    if setext_headings:
        issues.append(
            "Inconsistent heading style: found Setext headings (using === or ---)"
        )
...
def standardize_code_blocks(file_path: Path, fix: bool = False) -> list[str]:
    ...
    if indented_blocks:
        issues.append(
            f"Found {len(indented_blocks)} indented code blocks, use fenced code blocks (```)"
        )
        ...
        if fix:
            ...
            for block in indented_blocks:
                ...
                new_content = new_content.replace(block, f"```\n{fixed_block}\n```")
    ...
    if fenced_blocks:
        issues.append(
            f"Found {len(fenced_blocks)} code blocks without language specified"
        )
