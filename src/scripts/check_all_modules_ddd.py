...
def check_file(file_path: Path, module_name: str) -> Tuple[bool, Optional[str]]:
    ...
    try:
        with open(file_path, 'r') as f:
            ...
            if file_path.name != "entities.py" and "domain" in file_path.name and file_path.name.endswith(".py"):
                ...
                if module_name == "core" and file_path.name == "domain.py":
                    pass
                elif not re.search(rf'from uno\.{module_name}\.entities import', content):
                    return False, f"Domain entities not imported in domain file {file_path}"
            
            # Check if __init__.py properly exports domain entities
            if file_path.name == "__init__.py" and os.path.exists(os.path.join(file_path.parent, "entities.py")):
...
def check_module(module_path: Path) -> Tuple[str, bool, List[str]]:
    ...
    if not has_entities and any(os.path.exists(os.path.join(module_path, f)) for f in [
        "domain_services.py", "domain_repositories.py", "domain_endpoints.py"
    ]):
        all_valid = False
        error_messages.append(f"Module {module_name} has domain files but missing entities.py")
...
def main():
    ...
    modules = [d for d in src_dir.iterdir() if d.is_dir() and not d.name.startswith('.')]
