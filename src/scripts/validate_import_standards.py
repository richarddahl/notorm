...
class ImportStandardsValidator:
    ...
    # Only legacy class names that need to be replaced (NOT the correct Base-prefixed versions)
    ...
    DEPRECATED_IMPORT_PATTERNS = [
        ...
        (r"from uno\.core\.async_manager import", "from uno.core.async.task_manager import"),
        ...
        (r"from uno\.domain\.repository import", "from uno.core.base.repository import"),
        (r"from uno\.domain\.repository_results import", "from uno.core.base.repository import"),
        (r"import uno\.domain\.repository_results", "from uno.core.base.repository import RepositoryResult"),
    ]

    # Patterns that might indicate backward compatibility code
    ...
    def __init__(self, base_dir: str):
        ...
        self.imports_by_file: Dict[str, Set[str]] = {}  # Track imports to avoid false positives
    ...
    def validate_file(self, file_path: Path) -> None:
        ...
        self.imports_by_file[rel_path] = set()
        
        # First pass: collect imports
        for i, line in enumerate(lines, 1):
            # Track imports to help avoid false positives later
            if line.strip().startswith('from ') and ' import ' in line:
                self.imports_by_file[rel_path].add(line.strip())
        ...
        for i, line in enumerate(lines, 1):
            ...
            for pattern, replacement, import_path in self.LEGACY_CLASS_PATTERNS:
                if re.search(pattern, line):
                    # Skip if it's in an import itself
                    if re.search(f"from {import_path} import", line):
                        continue
                    
                    # Skip backward compatibility aliases
                    clean_pattern = pattern.replace('\\b', '')
                    if re.search(f"{replacement} = {clean_pattern}", line) or re.search(f"{clean_pattern} = {replacement}", line):
                        continue
                    
                    # If we already import from the correct path, don't flag as violation
                    has_correct_import = False
                    for imp in self.imports_by_file[rel_path]:
                        if f"from {import_path} import" in imp and replacement in imp:
                            has_correct_import = True
                            break
                    
                    if not has_correct_import:
                        suggestion = f"Replace with {replacement} and import from {import_path}"
            ...
            for pattern in self.BACKWARD_COMPAT_PATTERNS:
                if re.search(pattern, line):
                    # Skip detection of backward compatibility in the validation script itself
    ...
    def validate_directory(self, directory: Optional[Path] = None) -> None:
        ...
        for item in directory.iterdir():
            # Skip validation of our validation scripts
            if item.name == 'validate_import_standards.py' or item.name == 'modernize_imports.py':
                continue
    ...
    def generate_report(self) -> str:
        ...
        report.append(f"Found {total_violations} violations across {total_files} files.\n")
    ...
    def generate_fix_guide(self) -> str:
        ...
        total_violations = len(self.violations)
        total_files = len(violations_by_file)
        guide.append(f"This guide provides specific fixes for {total_violations} violations across {total_files} files.\n")
        ...
        for file_path, file_violations in sorted_files:
            ...
            while i < len(sorted_violations):
                ...
                if current.violation_type == "Legacy Class Name":
                    ...
                    for p, replacement, import_path in self.LEGACY_CLASS_PATTERNS:
                        if re.search(p, current.line):
                            ...
                            guide.append(f"1. Add import: `from {import_path} import {replacement}`")
                            guide.append(f"2. Replace all occurrences of `{pattern}` with `{replacement}`")
                            break
                ...
                # If it's a Backward Compatibility layer, provide context
                    ...
                    guide.append("If this is a backward compatibility layer, consider removing it completely if not needed.")
                    guide.append("If this is a deprecation warning, ensure it directs users to the correct standardized imports.")
