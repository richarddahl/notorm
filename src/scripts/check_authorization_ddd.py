#!/usr/bin/env python
"""
Validation script to ensure the authorization module is using domain-driven design.

This script checks that:
1. No UnoObj references are used in the module
2. Domain entities (User, Group, Role, etc.) are used instead
...
def check_file(file_path):
    ...
    with open(file_path, 'r') as f:
        ...
        if file_path.name == "entities.py":
            required_entities = [
                'User', 'Group', 'Role', 'Permission', 'Tenant'
            ]
        ...
        if file_path.name != "entities.py" and file_path.name.endswith(".py") and "domain" in file_path.name:
            if not re.search(r'from uno\.authorization\.entities import', content):
        ...
        if file_path.name == "__init__.py":
            ...
            required_exports = ["User", "Role", "Tenant"]
            missing_exports = [
...
def main():
    """Check all Python files in the authorization module."""
    # Get the project root directory
    script_dir = Path(__file__).resolve().parent
    project_root = script_dir.parent.parent
    auth_dir = project_root / "src" / "uno" / "authorization"
    
    if not auth_dir.exists():
        print(f"Error: Authorization directory not found at {auth_dir}")
        return 1
    
    print(f"Checking authorization module at {auth_dir}")
    
    # Check all Python files in the authorization directory
    all_valid = True
    ...
    if all_valid:
        print("\nVALIDATION PASSED: Authorization module uses domain-driven design!")
        return 0
    else:
        print("\nVALIDATION FAILED: Some issues found in the authorization module.")
        return 1
