...
try:
    ...
    print(
        "Warning: markdown or pygments not installed. Run: pip install markdown pygments"
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Generate documentation for Uno framework"
    )
    ...
    parser.add_argument(
        "--coverage", action="store_true", help="Generate documentation coverage report"
    )
...
def build_mkdocs(mkdocs_dir: Path) -> bool:
    ...
    try:
        ...
        logger.error(
            "MkDocs not found. Please install with: pip install mkdocs mkdocs-material mkdocstrings"
        )
        return False
                    ...
                    if inspect.isfunction(obj) or inspect.ismethod(obj):
                        ...
                        for param_name in sig.parameters:
                            if (
                                ...
                                and f"{param_name}:" not in doc
                                and f"{param_name} " not in doc
                            ):
                                issues["missing_params"].append(
                                    f"{name} - {param_name}"
                                )
                        ...
                        if (
                            ...
                            if (
                                "return" not in doc.lower()
                                and "returns" not in doc.lower()
                            ):
                        ...
                        if (
                            "raise" in source
                            and "raises" not in doc.lower()
                            and "raise" not in doc.lower()
                        ):
...
def generate_coverage_report(modules: list[str], output_dir: str) -> None:
    ...
    logger.info(
        f"Documentation coverage: {coverage:.2%} ({documented_items}/{total_items})"
    )
            ...
            if DocFormat.MARKDOWN in config.formats:
                ...
                if build_mkdocs(mkdocs_dir):
                    ...
                    if args.serve:
                        ...
                        try:
                            subprocess.run(
                                ["mkdocs", "serve"], cwd=mkdocs_dir, check=True
                            )
                        except subprocess.CalledProcessError as e:
                            ...
                            logger.error(
                                "MkDocs not found. Please install with: pip install mkdocs"
                            )
                            return 1
                else:
