#!/usr/bin/env python
"""
Validation script to ensure the reports module is using domain-driven design.

This script checks that:
1. No UnoObj references are used in the module
2. Domain entities (ReportTemplate, etc.) are used instead
...
def check_file(file_path):
    ...
    with open(file_path, 'r') as f:
        ...
        if file_path.name == "entities.py":
            required_entities = [
                'ReportTemplate', 'ReportFieldDefinition', 'ReportTrigger',
                'ReportOutput', 'ReportExecution', 'ReportOutputExecution'
            ]
        ...
        if file_path.name != "entities.py" and file_path.name.endswith(".py") and "domain" in file_path.name:
            if not re.search(r'from uno\.reports\.entities import', content):
        ...
        if file_path.name == "__init__.py":
            ...
            required_exports = ["ReportTemplate", "ReportFieldDefinition"]
            missing_exports = [
...
def main():
    """Check all Python files in the reports module."""
    # Get the project root directory
    script_dir = Path(__file__).resolve().parent
    project_root = script_dir.parent.parent
    reports_dir = project_root / "src" / "uno" / "reports"
    
    if not reports_dir.exists():
        print(f"Error: Reports directory not found at {reports_dir}")
        return 1
    
    print(f"Checking reports module at {reports_dir}")
    
    # Check all Python files in the reports directory
    all_valid = True
    for file_path in reports_dir.glob("**/*.py"):
        print(f"Checking {file_path.relative_to(project_root)}")
        if not check_file(file_path):
            all_valid = False
    
    if all_valid:
        print("\nVALIDATION PASSED: Reports module uses domain-driven design!")
        return 0
    else:
        print("\nVALIDATION FAILED: Some issues found in the reports module.")
        return 1
