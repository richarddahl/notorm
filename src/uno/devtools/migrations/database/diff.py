...
class SchemaDiff:
    ...
    def has_changes(self) -> bool:
        ...
        return any([
            self.added_tables, self.removed_tables, self.modified_tables,
            self.added_columns, self.removed_columns, self.modified_columns,
            self.added_indexes, self.removed_indexes, 
    ...
    def summary(self) -> str:
        ...
        if self.added_columns:
            column_info = [f"{table}: {', '.join(cols)}" for table, cols in self.added_columns.items()]
            parts.append(f"Added columns: {'; '.join(column_info)}")
            
        if self.removed_columns:
            column_info = [f"{table}: {', '.join(cols)}" for table, cols in self.removed_columns.items()]
            parts.append(f"Removed columns: {'; '.join(column_info)}")
            
        if self.modified_columns:
            column_info = [f"{table}: {', '.join(cols.keys())}" for table, cols in self.modified_columns.items()]
            parts.append(f"Modified columns: {'; '.join(column_info)}")
            
        if self.added_indexes:
            index_info = [f"{table}: {', '.join(idxs)}" for table, idxs in self.added_indexes.items()]
            parts.append(f"Added indexes: {'; '.join(index_info)}")
            
        if self.removed_indexes:
            index_info = [f"{table}: {', '.join(idxs)}" for table, idxs in self.removed_indexes.items()]
            parts.append(f"Removed indexes: {'; '.join(index_info)}")
            
        if self.added_constraints:
            constraint_info = [f"{table}: {', '.join(cons)}" for table, cons in self.added_constraints.items()]
            parts.append(f"Added constraints: {'; '.join(constraint_info)}")
            
        if self.removed_constraints:
            constraint_info = [f"{table}: {', '.join(cons)}" for table, cons in self.removed_constraints.items()]
...
def diff_model_to_db(
    ...
    for table_name in common_tables:
        ...
        table_removed_columns = [name for name in db_columns if name not in model_columns]
...
def diff_db_to_db(
    ...
    source_tables = {
        table.name: table for table in source_metadata.tables.values()
        if (include_tables is None or table.name in include_tables)
        and table.name not in exclude_tables
    }
    
    target_tables = {
        table.name: table for table in target_metadata.tables.values()
        if (include_tables is None or table.name in include_tables)
        and table.name not in exclude_tables
    }
    ...
    for table_name in common_tables:
        ...
        table_added_columns = [name for name in source_columns if name not in target_columns]
        table_removed_columns = [name for name in target_columns if name not in source_columns]
