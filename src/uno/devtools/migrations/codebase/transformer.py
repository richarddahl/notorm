...
class CodeTransformer:
            ...
            for transformation in transformations_to_apply:
                if transformation in self.transformers:
                    ...
                    issues = [
                        ...
                        if issue.issue_type in transformation or issue.issue_type == transformation
                    ]
                    ...
                    try:
                        ...
                        logger.error(f"Error applying transformation {transformation} to {file_path}: {e}")
                else:
            ...
            class ImportTransformer(ast.NodeTransformer):
                        ...
                        for name in node.names:
                            ...
                            for issue in issues:
                                if name.name == issue.issue_type:
                                    ...
                                    replacement = issue.suggestion.replace("Replace with ", "")
                ...
                def visit_ImportFrom(self, node):
                    if node.lineno in self.issues_by_line:
                        ...
                        for issue in issues:
                            if node.module == issue.description.replace("The '", "").replace("' module is deprecated", ""):
                                # Replace with suggested import
                                replacement = issue.suggestion.replace("Replace with ", "")
    ...
    def _transform_dependency_injection(
        ...
        if "service_locator" in [i.issue_type for i in issues] and "import inject" not in content:
...
def apply_transformations(
    ...
    if path.is_file():
        ...
        results = transformer.transform_directory(path, transformations, dry_run=dry_run)
    else:
    ...
    if output_format == "text":
        ...
        summary.append(f"  Issues remaining: {sum(len(r.issues_remaining) for r in results)}")
