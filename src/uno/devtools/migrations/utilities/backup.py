...
def backup_directory(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        exclude_patterns = exclude_patterns or ["__pycache__", "*.pyc", "*.pyo", ".git", ".DS_Store"]
        ...
        for root, dirs, files in os.walk(directory):
            ...
            for file in files:
                if not any(
                    file == pat or 
                    (pat.startswith("*") and file.endswith(pat[1:])) or
                    (pat.endswith("*") and file.startswith(pat[:-1]))
        ...
        if compress:
            ...
            shutil.make_archive(str(backup_path).rstrip(".gz"), "gztar", temp_dir, backup_name)
        else:
            backup_path = backup_dir / f"{backup_name}.tar"
            shutil.make_archive(str(backup_path).rstrip(".tar"), "tar", temp_dir, backup_name)
...
def save_migration_metadata(
    ...
    metadata["backup_file"] = {
        ...
        "created": datetime.datetime.fromtimestamp(backup_path.stat().st_ctime).isoformat()
    }
...
def backup_before_migration(
    ...
    metadata_path = output_dir / f"migration_backup_{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}.meta.json"
    with open(metadata_path, "w") as f:
        json.dump(metadata, f, indent=2)
