...
class MarkdownRenderer(BaseRenderer):
    ...
    def render(
            ...
            for module_data in data["modules"]:
                ...
                if "functions" in module_data:
                    for function_data in module_data["functions"]:
                        ...
                        result[f"{module_path}/{function_data['name']}.md"] = function_content
        ...
        if "endpoints" in data:
            ...
            for endpoint_data in data["endpoints"]:
                ...
                endpoint_name = endpoint_data.get("name", "").replace("/", "_") or f"endpoint_{len(result)}"
        ...
        if "examples" in data:
            ...
            for example_data in data["examples"]:
                ...
                example_name = example_data.get("name", "").replace("/", "_") or f"example_{len(result)}"
    ...
    def _render_module_index(self, module_data: Dict[str, Any]) -> str:
        ...
        if "classes" in module_data and module_data["classes"]:
            ...
            for class_data in module_data["classes"]:
                ...
                class_desc = class_data.get("description", "").split(".")[0] + "." if class_data.get("description") else ""
        ...
        if "functions" in module_data and module_data["functions"]:
            ...
            for function_data in module_data["functions"]:
                ...
                function_desc = function_data.get("description", "").split(".")[0] + "." if function_data.get("description") else ""
                
                lines.append(f"- [{function_name}]({function_name}.md): {function_desc}")
    ...
    def _render_api_index(self, endpoints: List[Dict[str, Any]]) -> str:
        ...
        for tag, tag_endpoints in sorted(endpoints_by_tag.items()):
            ...
            for endpoint in tag_endpoints:
                endpoint_name = endpoint.get("name", "").replace("/", "_") or f"endpoint_{len(endpoints_by_tag)}"
                methods = ", ".join(endpoint.get("methods", []))
                path = endpoint.get("path", "")
                desc = endpoint.get("description", "").split(".")[0] + "." if endpoint.get("description") else ""
            ...
            for example in endpoint_data["examples"]:
                ...
                if "request" in example:
                    ...
                    if "body" in example["request"]:
                        ...
                        if isinstance(example["request"]["body"], dict):
                            lines.append(json.dumps(example["request"]["body"], indent=2))
                        else:
                ...
                if "response" in example:
                    ...
                    if "status_code" in example["response"]:
                        lines.append(f"Status Code: {example['response']['status_code']}")
                        lines.append("")
                    ...
                    if "body" in example["response"]:
                        ...
                        if isinstance(example["response"]["body"], dict):
                            lines.append(json.dumps(example["response"]["body"], indent=2))
                        else:
    ...
    def _render_models_index(self, models: List[Dict[str, Any]]) -> str:
        ...
        for module, module_models in sorted(models_by_module.items()):
            ...
            for model in module_models:
                ...
                desc = model.get("description", "").split(".")[0] + "." if model.get("description") else ""
    ...
    def _render_model(self, model_data: Dict[str, Any]) -> str:
        ...
        if "fields" in model_data and model_data["fields"]:
            ...
            for field in model_data["fields"]:
                ...
                required = "Yes" if field.get("required", True) and not field.get("nullable", False) else "No"
    ...
    def _render_examples_index(self, examples: List[Dict[str, Any]]) -> str:
        ...
        for module, module_examples in sorted(examples_by_module.items()):
            ...
            for example in module_examples:
                example_name = example.get("name", "").replace("/", "_") or f"example_{len(examples_by_module)}"
                desc = example.get("description", "").split(".")[0] + "." if example.get("description") else ""
    ...
    def _generate_search_index(self, data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if "endpoints" in data:
            for endpoint_data in data["endpoints"]:
                endpoint_name = endpoint_data.get("name", "").replace("/", "_") or f"endpoint_{len(search_index)}"
                methods = ", ".join(endpoint_data.get("methods", []))
                path = endpoint_data.get("path", "")
        ...
        if "examples" in data:
            for example_data in data["examples"]:
                example_name = example_data.get("name", "").replace("/", "_") or f"example_{len(search_index)}"
...
class OpenAPIRenderer(BaseRenderer):
    ...
    def render(
        ...
        try:
            ...
            self.warnings.append("PyYAML is not installed, OpenAPI YAML output is disabled")
    ...
    def _generate_openapi_schema(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "endpoints" in data:
            for endpoint_data in data["endpoints"]:
                ...
                methods = [method.lower() for method in endpoint_data.get("methods", [])]
                
                if path not in schema["paths"]:
                    schema["paths"][path] = {}
                
                for method in methods:
                    schema["paths"][path][method] = self._convert_endpoint_to_openapi(endpoint_data)
        
        # Add schemas from models
        if "models" in data:
            for model_data in data["models"]:
                model_name = model_data["name"]
                schema["components"]["schemas"][model_name] = self._convert_model_to_openapi(model_data)
        
        return schema
    
    def _convert_endpoint_to_openapi(self, endpoint_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        """
        operation = {
            "summary": endpoint_data.get("description", "").split(".")[0] if endpoint_data.get("description") else "",
            "description": endpoint_data.get("description", ""),
        ...
        if "parameters" in endpoint_data:
            for param in endpoint_data["parameters"]:
                param_name = param["name"]
                param_in = "path" if "{" + param_name + "}" in endpoint_data.get("path", "") else "query"
    ...
    def _convert_model_to_openapi(self, model_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "fields" in model_data:
            for field in model_data["fields"]:
                ...
                field_required = field.get("required", True) and not field.get("nullable", False)
