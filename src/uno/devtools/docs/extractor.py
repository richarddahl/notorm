    ...
    if isinstance(target, str):
        ...
        if os.path.exists(target) and os.path.isfile(target):
            ...
            try:
                ...
                result = _extract_from_object(module, recursive, include_private, include_dunder)
        ...
        result = _extract_from_object(target, recursive, include_private, include_dunder)
    ...
    if doc:
        ...
        if callable(obj):
            try:
                ...
                result[full_name]["return_annotation"] = str(sig.return_annotation) if sig.return_annotation != inspect.Signature.empty else None
    ...
    if recursive:
        for name, member in inspect.getmembers(obj):
            ...
            if inspect.ismodule(member):
                ...
                if member_module != obj_module and (not member_module.startswith(f"{obj_module}.")):
                    continue
            
            # Extract from member
            member_results = _extract_from_object(member, recursive, include_private, include_dunder)
