...
class EnhancedBaseError(BaseError):
    """Enhanced version of BaseError with additional debugging information."""
...
def _uno_error_hook(error: Exception) -> Optional[Exception]:
    ...
    """
    if isinstance(error, BaseError) and not isinstance(error, EnhancedBaseError):
        return EnhancedBaseError(
            message=str(error),
...
def _sql_error_hook(error: Exception) -> Optional[Exception]:
    ...
    if (
        ...
        if isinstance(error, BaseError):
            # Create an enhanced BaseError
            enhanced = EnhancedBaseError(
                message=str(error),
...
def _general_exception_hook(error: Exception) -> Optional[Exception]:
    ...
    Returns:
        Enhanced error if applicable, None otherwise
    """
    if not isinstance(error, BaseError) and not isinstance(error, EnhancedBaseError):
