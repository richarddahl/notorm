...
class RepositoryTracker:
    ...
    def get_operations_by_repository(self, repository_class: str) -> List[RepositoryOperation]:
    ...
    def get_slow_operations(self, threshold_ms: float = 100.0) -> List[RepositoryOperation]:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        for op in self.operations:
            if op.repository_class:
                repo_counts[op.repository_class] = repo_counts.get(op.repository_class, 0) + 1
        ...
        for source, source_ops in operations_by_source.items():
            ...
            for method, method_ops in ops_by_method.items():
                if len(method_ops) > 3 and method in (
                    "get_by_id", "get", "find", "find_one", "find_by_id"
                ):
