...
from uno.devtools.migrations.database.diff import diff_model_to_db, diff_db_to_db, SchemaDiff
from uno.devtools.migrations.database.generate import generate_migration_script, generate_migration_plan
from uno.devtools.migrations.database.apply import apply_migration, apply_migrations, apply_migrations_directory
from uno.devtools.migrations.database.rollback import rollback_migration, rollback_to_revision
...
def diff_schema_command(
    connection_string: Annotated[str, typer.Option("--connection", "-c", help="Database connection string")],
    models_module: Annotated[str, typer.Option("--models", "-m", help="Python module path containing SQLAlchemy models")],
    schema: Annotated[Optional[str], typer.Option("--schema", "-s", help="Database schema to compare")] = None,
    output_format: Annotated[str, typer.Option("--format", "-f", help="Output format (text or markdown)")] = "text",
    ...
    try:
        ...
        try:
            ...
            console.print(f"[bold red]Error:[/bold red] Could not import module {models_module}")
            return
        ...
        if not models:
            console.print(f"[bold yellow]Warning:[/bold yellow] No models found in {models_module}")
            return
            
        # Run the diff
        console.print(f"[bold]Comparing {len(models)} models to database schema...[/bold]")
        ...
        if diff.has_changes:
            ...
            if output_file:
                with open(output_file, "w") as f:
                    f.write(report)
                console.print(f"[bold green]Schema diff saved to {output_file}[/bold green]")
            else:
                if output_format == "markdown":
...
def generate_migration_command(
    connection_string: Annotated[str, typer.Option("--connection", "-c", help="Database connection string")],
    models_module: Annotated[str, typer.Option("--models", "-m", help="Python module path containing SQLAlchemy models")],
    output_dir: Annotated[str, typer.Option("--output-dir", "-o", help="Directory for migration scripts")],
    message: Annotated[str, typer.Option("--message", "-g", help="Migration message (short description)")],
    schema: Annotated[Optional[str], typer.Option("--schema", "-s", help="Database schema")] = None,
    format: Annotated[str, typer.Option("--format", "-f", help="Migration format (python or sql)")] = "python",
    ...
    try:
        ...
        if not diff.has_changes:
            console.print("[bold yellow]No schema differences detected. No migration needed.[/bold yellow]")
            return
        ...
        if not dry_run:
            ...
            console.print(f"[bold green]Migration script generated: {script_path}[/bold green]")
...
def apply_migration_command(
    ...
    connection_string: Annotated[str, typer.Option("--connection", "-c", help="Database connection string")],
    schema: Annotated[Optional[str], typer.Option("--schema", "-s", help="Database schema")] = None,
    dry_run: Annotated[bool, typer.Option("--dry-run", help="Run without applying changes")] = False,
    backup: Annotated[bool, typer.Option("--backup", help="Create database backup before applying")] = True,
    backup_dir: Annotated[Optional[str], typer.Option("--backup-dir", help="Directory for backups")] = None,
) -> None:
    ...
    try:
        ...
        if not script_path.exists():
            console.print(f"[bold red]Error:[/bold red] Migration script not found: {script_path}")
            return
        ...
        # Apply the migration
        console.print(f"[bold]{'Would apply' if dry_run else 'Applying'} migration: {script_path}[/bold]")
        
        result = apply_migration(script_path, engine, schema=schema, dry_run=dry_run)
        
        if result:
            console.print(f"[bold green]Migration {'would be' if dry_run else 'was'} successfully applied[/bold green]")
        else:
            console.print(f"[bold red]Migration {'would fail' if dry_run else 'failed'}![/bold red]")
...
def rollback_migration_command(
    script_path: Annotated[str, typer.Argument(help="Path to the migration script to roll back")],
    connection_string: Annotated[str, typer.Option("--connection", "-c", help="Database connection string")],
    schema: Annotated[Optional[str], typer.Option("--schema", "-s", help="Database schema")] = None,
    dry_run: Annotated[bool, typer.Option("--dry-run", help="Run without applying changes")] = False,
    backup: Annotated[bool, typer.Option("--backup", help="Create database backup before rolling back")] = True,
    backup_dir: Annotated[Optional[str], typer.Option("--backup-dir", help="Directory for backups")] = None,
) -> None:
    ...
    try:
        ...
        console.print(f"[bold]{'Would roll back' if dry_run else 'Rolling back'} migration: {script_path}[/bold]")
        
        result = rollback_migration(script_path, engine, schema=schema, dry_run=dry_run)
        
        if result:
            console.print(f"[bold green]Migration {'would be' if dry_run else 'was'} successfully rolled back[/bold green]")
        else:
            console.print(f"[bold red]Rollback {'would fail' if dry_run else 'failed'}![/bold red]")
...
def analyze_code_command(
    directory: Annotated[str, typer.Argument(help="Directory containing Python code to analyze")],
    output_format: Annotated[str, typer.Option("--format", "-f", help="Output format (text, markdown, or json)")] = "text",
    output_file: Annotated[Optional[str], typer.Option("--output", "-o", help="Output file path")] = None,
    patterns: Annotated[Optional[List[str]], typer.Option("--pattern", "-p", help="Patterns to check for")] = None,
) -> None:
    ...
    try:
        ...
        if output_file:
            with open(output_file, "w") as f:
                f.write(results)
            console.print(f"[bold green]Analysis results saved to {output_file}[/bold green]")
        else:
            if output_format == "markdown":
                syntax = Syntax(results, "markdown", theme="monokai", line_numbers=False)
                console.print(syntax)
...
def transform_code_command(
    directory: Annotated[str, typer.Argument(help="Directory containing Python code to transform")],
    output_format: Annotated[str, typer.Option("--format", "-f", help="Output format (text, diff, or json)")] = "text",
    output_file: Annotated[Optional[str], typer.Option("--output", "-o", help="Output file path")] = None,
    transformations: Annotated[Optional[List[str]], typer.Option("--transform", "-t", help="Transformations to apply")] = None,
    dry_run: Annotated[bool, typer.Option("--dry-run", help="Show changes without modifying files")] = True,
    backup: Annotated[bool, typer.Option("--backup", help="Create backups before transforming")] = True,
    backup_dir: Annotated[Optional[str], typer.Option("--backup-dir", help="Directory for backups")] = None,
) -> None:
    ...
    try:
        ...
        if backup and not dry_run:
            ...
            backups = backup_before_migration([directory_path], backup_dir, compress=True)
            
            for path, backup_path in backups.items():
                console.print(f"[bold green]Backup created: {backup_path}[/bold green]")
                
        # Apply transformations
        console.print(f"[bold]{'Analyzing' if dry_run else 'Transforming'} Python code in {directory}...[/bold]")
        results = apply_transformations(directory, transformations, dry_run, output_format)
        
        # Output the results
        if output_file:
            with open(output_file, "w") as f:
                f.write(results)
            console.print(f"[bold green]Transformation results saved to {output_file}[/bold green]")
        else:
...
def restore_backup_command(
    ...
    force: Annotated[bool, typer.Option("--force", help="Force overwriting existing files")] = False,
    target_dir: Annotated[Optional[str], typer.Option("--target", "-t", help="Target directory for restoration")] = None,
) -> None:
    ...
    try:
        ...
        if not backup_dir_path.exists() or not backup_dir_path.is_dir():
            console.print(f"[bold red]Error:[/bold red] Backup directory not found: {backup_dir}")
            return
        ...
        try:
            ...
            if results['failed'] > 0:
                console.print(f"[bold yellow]Files failed to restore: {results['failed']}[/bold yellow]")
...
def setup_parser(subparsers):
    ...
    diff_parser.add_argument(
        "-s", "--schema",
        dest="schema",
        help="Database schema to compare"
    )
    diff_parser.add_argument(
        "-f", "--format",
        dest="output_format",
    ...
    apply_parser = migrations_subparsers.add_parser(
        "apply-migration",
        help="Apply a database migration script"
    )
    apply_parser.add_argument(
        "script_path",
        help="Path to the migration script"
    )
    apply_parser.add_argument(
        "-c", "--connection",
        dest="connection_string",
    ...
    rollback_parser = migrations_subparsers.add_parser(
        "rollback-migration",
        help="Roll back a database migration"
    )
    rollback_parser.add_argument(
        "script_path",
        help="Path to the migration script to roll back"
    )
    rollback_parser.add_argument(
        "-c", "--connection",
        dest="connection_string",
    ...
    analyze_parser = migrations_subparsers.add_parser(
        "analyze-code",
        help="Analyze Python code for migration needs"
    )
    analyze_parser.add_argument(
        "directory",
        help="Directory containing Python code to analyze"
    )
    analyze_parser.add_argument(
        "-f", "--format",
        dest="output_format",
    ...
    transform_parser = migrations_subparsers.add_parser(
        "transform-code",
        help="Transform Python code to migrate patterns and APIs"
    )
    transform_parser.add_argument(
        "directory",
        help="Directory containing Python code to transform"
    )
    transform_parser.add_argument(
        "-f", "--format",
        dest="output_format",
...
def handle_command(args):
    ...
    if args.subcommand in handlers:
        ...
        kwargs = {k: v for k, v in vars(args).items() if k != "subcommand" and k != "command"}
