...
def _generate_model_imports(
    ...
    for field_info in fields.values():
        ...
        if any(validator in field_info for validator in ["min_length", "max_length", "regex", "ge", "le"]):
...
def _generate_model_class(
    ...
    if timestamps:
        ...
        lines.append('    created_at = sa.Column(sa.DateTime, default=datetime.utcnow, nullable=False)')
        lines.append('    updated_at = sa.Column(sa.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow, nullable=False)')
...
def _generate_schema_class(
    ...
    lines.append('        """Schema configuration."""')
    lines.append('')
...
def _generate_index(index: Dict[str, Any]) -> str:
    ...
    if options_str:
        return f"__table_args__ = (sa.Index('{name}', {', '.join(args)}, {options_str}),)"
    else:
...
def _generate_validator(field_name: str, field_info: Dict[str, Any]) -> List[str]:
    ...
    if field_info.get("min_length") or field_info.get("max_length"):
        ...
        if min_length and max_length:
            ...
            lines.append(f"            raise ValueError(f'{field_name} must be at least {min_length} characters')")
            lines.append(f"        if v is not None and len(v) > {max_length}:")
            lines.append(f"            raise ValueError(f'{field_name} must be at most {max_length} characters')")
        elif min_length:
            lines.append(f"        if v is not None and len(v) < {min_length}:")
            lines.append(f"            raise ValueError(f'{field_name} must be at least {min_length} characters')")
        elif max_length:
    ...
    if field_info.get("regex"):
        ...
        lines.append(f"            raise ValueError(f'{field_name} must match pattern {regex}')")
    ...
    if field_info.get("ge") is not None or field_info.get("le") is not None:
        ...
        if ge is not None and le is not None:
            ...
            lines.append(f"            raise ValueError(f'{field_name} must be at least {ge}')")
            lines.append(f"        if v is not None and v > {le}:")
            lines.append(f"            raise ValueError(f'{field_name} must be at most {le}')")
        elif ge is not None:
            lines.append(f"        if v is not None and v < {ge}:")
            lines.append(f"            raise ValueError(f'{field_name} must be at least {ge}')")
        elif le is not None:
