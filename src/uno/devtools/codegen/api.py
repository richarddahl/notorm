...
def _generate_get_all_endpoint(
    ...
    if include_pagination:
        if include_filtering:
            lines.append(f"    return await repository.get_paginated(params=params, filters=filters)")
        else:
...
def _generate_get_by_id_endpoint(
    ...
    lines.append(f'        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="{model_name} not found")')
...
def _generate_update_endpoint(
    ...
    # Implementation
    if include_validation:
        lines.append(f"    updated_item = await repository.update(id, **item.dict(exclude_unset=True))")
    else:
