...
def _generate_repository_imports(
    ...
    # Import base class
    if module_name:
        imports.append(f"from {module_name}.database.repository import {base_repository_class}")
...
def _generate_crud_methods(model_name: str, id_type: str, include_docstrings: bool) -> List[str]:
    ...
    if include_docstrings:
        ...
        lines.append('        """')
    lines.append(f"        query = select(self.model_type).where(self.model_type.id == id)")
    ...
    if include_docstrings:
        ...
        lines.append('')
        lines.append('        Returns:')
        lines.append(f'            True if the {model_name} was deleted, False otherwise')
        lines.append('        """')
    lines.append(f"        obj = await self.get_by_id(id)")
    lines.append(f"        if obj is None:")
...
def _generate_query_methods(model_name: str, include_docstrings: bool) -> List[str]:
    ...
    # Get all method
    lines.append("")
    lines.append(f"    async def get_all(self, limit: Optional[int] = None, offset: int = 0) -> List[T]:")
    if include_docstrings:
        lines.append(f'        """Get all {model_name}s.')
        lines.append('')
        lines.append('        Args:')
        lines.append(f'            limit: Optional limit on the number of {model_name}s to return')
...
def _generate_bulk_methods(model_name: str, include_docstrings: bool) -> List[str]:
    ...
    # Bulk create method
    lines.append("")
    lines.append(f"    async def bulk_create(self, items: List[Dict[str, Any]]) -> List[T]:")
    if include_docstrings:
        lines.append(f'        """Create multiple {model_name}s in a single operation.')
        lines.append('')
        lines.append('        Args:')
        lines.append(f'            items: List of dictionaries with {model_name} attributes')
        lines.append('')
        lines.append('        Returns:')
    ...
    # Bulk update method
    lines.append("")
    lines.append(f"    async def bulk_update(self, items: List[Dict[str, Any]], key_field: str = 'id') -> int:")
    if include_docstrings:
    ...
    lines.append(f"        stmt = stmt.where(getattr(self.model_type, key_field) == bindparam(f'b_{key_field}'))")
    lines.append("")
    ...
    lines.append(f"            return 0")
    lines.append("")
    lines.append(f"        stmt = delete(self.model_type).where(self.model_type.id.in_(ids))")
...
def _generate_filter_methods(filters: List[Dict[str, Any]], include_docstrings: bool) -> List[str]:
    ...
    for filter_def in filters:
        ...
        for field in fields:
            ...
            if optional:
                ...
                lines.append(f"            query = query.where(self.model_type.{field_name} {operator} {field_name})")
            else:
                lines.append(f"        query = query.where(self.model_type.{field_name} {operator} {field_name})")
