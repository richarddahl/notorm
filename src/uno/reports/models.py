...
report_template__field = Table(
    ...
    Index("ix_report_template_id__field_id", "report_template_id", "report_field_definition_id"),
)
...
class ReportTemplateModel(DefaultModelMixin, UnoModel):
    ...
    description: Mapped[str] = mapped_column(
        doc="Description of the report template"
    )
    ...
    format_config: Mapped[Dict[str, Any]] = mapped_column(
        JSONB,
        default={},
        doc="JSON configuration for output format"
    )
    parameter_definitions: Mapped[Dict[str, Any]] = mapped_column(
        JSONB,
        default={},
        doc="User parameters the report accepts"
    )
    cache_policy: Mapped[Dict[str, Any]] = mapped_column(
        JSONB,
        default={},
        doc="How report results are cached"
    )
    version: Mapped[str] = mapped_column(
        default="1.0.0",
        doc="For template versioning"
    )
...
class ReportFieldDefinitionModel(DefaultModelMixin, UnoModel):
    ...
    field_config: Mapped[Dict[str, Any]] = mapped_column(
        JSONB,
        default={},
        doc="Configuration specific to field_type"
    )
    
    # Presentation
    order: Mapped[int] = mapped_column(
        default=0,
        doc="Display order of the field"
    )
    format_string: Mapped[Optional[str]] = mapped_column(
        nullable=True,
        doc="Format string for the field value"
    )
    conditional_formats: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSONB,
        nullable=True,
        doc="Conditional formatting rules"
    )
    is_visible: Mapped[bool] = mapped_column(
        default=True,
        doc="Whether the field is visible in the report"
    )
...
class ReportTriggerModel(DefaultModelMixin, UnoModel):
    ...
    trigger_type: Mapped[str] = mapped_column(
        VARCHAR(50),
        doc="Type of trigger (manual, scheduled, event, query)"
    )
    trigger_config: Mapped[Dict[str, Any]] = mapped_column(
        JSONB,
        default={},
        doc="Configuration specific to trigger_type"
    )
    
    # For scheduled triggers
    schedule: Mapped[Optional[str]] = mapped_column(
        nullable=True,
        doc="Cron-style schedule expression"
    )
    
    # For event triggers
    event_type: Mapped[Optional[str]] = mapped_column(
        nullable=True,
        doc="Type of event that triggers the report"
    )
    ...
    last_triggered: Mapped[Optional[datetime]] = mapped_column(
        nullable=True,
        doc="When this trigger was last activated"
    )
...
class ReportOutputModel(DefaultModelMixin, UnoModel):
    ...
    output_type: Mapped[str] = mapped_column(
        VARCHAR(50),
        doc="Type of output (file, email, webhook, notification)"
    )
    output_config: Mapped[Dict[str, Any]] = mapped_column(
        JSONB,
        default={},
        doc="Configuration specific to output_type"
    )
    
    # Format
    format: Mapped[str] = mapped_column(
        VARCHAR(50),
        doc="Format of the output (csv, pdf, json, html, excel, text)"
    )
    format_config: Mapped[Dict[str, Any]] = mapped_column(
        JSONB,
        default={},
        doc="Configuration specific to format"
    )
    
    # Status
    is_active: Mapped[bool] = mapped_column(
        default=True,
        doc="Whether this output is active"
    )
...
class ReportExecutionModel(DefaultModelMixin, UnoModel):
    ...
    trigger_type: Mapped[str] = mapped_column(
        VARCHAR(50),
        doc="Type of trigger that initiated execution"
    )
    ...
    parameters: Mapped[Dict[str, Any]] = mapped_column(
        JSONB,
        default={},
        doc="Parameters provided for this execution"
    )
    ...
    row_count: Mapped[Optional[int]] = mapped_column(
        nullable=True,
        doc="Number of rows in the result"
    )
    ...
    result_hash: Mapped[Optional[str]] = mapped_column(
        nullable=True,
        doc="Hash of the result data for caching"
    )
...
class ReportOutputExecutionModel(DefaultModelMixin, UnoModel):
    ...
    completed_at: Mapped[Optional[datetime]] = mapped_column(
        nullable=True,
        doc="When output delivery completed"
    )
    error_details: Mapped[Optional[str]] = mapped_column(
        nullable=True,
        doc="Error details if output delivery failed"
    )
    
    # Result details
    output_location: Mapped[Optional[str]] = mapped_column(
        nullable=True,
        doc="Location of the output (URL, file path, etc.)"
    )
