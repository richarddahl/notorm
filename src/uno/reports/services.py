...
class ReportTemplateService(UnoServiceProtocol, ReportTemplateServiceProtocol):
    ...
    async def create_template(
        ...
        try:
            ...
            return await self.template_repository.get_by_id(
                created_template.id, self.session
            )
    ...
    async def update_template(
        ...
        try:
            ...
            template_result = await self.template_repository.get_by_id(
                template_id, self.session
            )
            ...
            update_result = await self.template_repository.update(
                existing_template, self.session
            )
    ...
    async def clone_template(
        ...
        try:
            ...
            template_data = source_template.model_dump(
                exclude={"id", "fields", "triggers", "outputs", "executions"}
            )
            template_data["name"] = new_name

            # Create the new template
            new_template = ReportTemplate(**template_data)
            create_result = await self.template_repository.create(
                new_template, self.session
            )
...
class ReportFieldService(UnoServiceProtocol, ReportFieldServiceProtocol):
    ...
    async def update_field(
        ...
        try:
            ...
            for key, value in field_data.items():
                if key not in ["parent_field", "child_fields", "templates"] and hasattr(
                    existing_field, key
                ):
            ...
            available_fields.append(
                {
                    ...
                    "field_config": {"table": base_object_type, "column": "id"},
                }
            )
            ...
            available_fields.append(
                {
                    ...
                    "field_type": ReportFieldType.DB_COLUMN,
                    "field_config": {"table": base_object_type, "column": "created_at"},
                }
            )
            ...
            available_fields.append(
                {
                    ...
                    "field_config": {"attribute_type_id": "status"},
                }
            )
    ...
    async def list_fields_by_template(
        ...
        try:
            return await self.field_repository.list_by_template(
                template_id, self.session
            )
        except Exception as e:
            return Failure(
...
class ReportExecutionService(UnoServiceProtocol, ReportExecutionServiceProtocol):
        ...
        try:
            ...
            if parameters:
                for param_name, param_def in template.parameter_definitions.items():
                    if param_name not in parameters and param_def.get(
                        "required", False
                    ):
                        return Failure(
                            ReportError(
            ...
            # Save the execution
            execution_result = await self.execution_repository.create(
            ...
            outputs_result = await self.output_repository.list_by_template(
                template_id, self.session
            )
            ...
            for output in outputs_result.value:
                if output.is_active:
                    ...
                    await self.output_execution_repository.create(
                        output_execution, self.session
                    )

            # Update execution status to in_progress
            await self.execution_repository.update_status(
                execution.id, ReportExecutionStatus.IN_PROGRESS, session=self.session
            )
    ...
    async def get_execution_status(self, execution_id: str) -> Result[Dict[str, Any]]:
        ...
        try:
            # Get the execution
            execution_result = await self.execution_repository.get_by_id(
                execution_id, self.session
            )
            if execution_result.is_failure:
                return execution_result
            ...
            output_executions_result = (
                await self.output_execution_repository.list_by_execution(
                    execution_id, self.session
                )
            )
    ...
    async def cancel_execution(self, execution_id: str) -> Result[bool]:
        ...
        try:
            ...
            status_result = await self.execution_repository.update_status(
                execution_id, ReportExecutionStatus.CANCELLED, session=self.session
            )
    ...
    async def get_execution_result(
        self, execution_id: str, format: Optional[str] = None
    ) -> Result[Any]:
        ...
        try:
            ...
            template_result = await self.template_repository.get_by_id(
                execution.report_template_id, self.session
            )
            if template_result.is_failure:
                return template_result
            ...
            if self.field_repository:
                fields_result = await self.field_repository.list_by_template(
                    template.id, self.session
                )
        ...
        try:
            ...
            if template_id:
                return await self.execution_repository.list_by_template(
                    template_id, status=status, limit=limit, session=self.session
                )
...
class ReportTriggerService(UnoServiceProtocol, ReportTriggerServiceProtocol):
    ...
    async def list_triggers_by_template(
        ...
        try:
            ...
            return await self.trigger_repository.list_by_template(
                template_id, self.session
            )
        except Exception as e:
            return Failure(
    ...
    async def update_trigger(
        ...
        try:
            ...
            trigger_result = await self.trigger_repository.get_by_id(
                trigger_id, self.session
            )
    ...
    async def handle_event(
        ...
        try:
            ...
            triggers_result = await self.trigger_repository.list_by_event_type(
                event_type, self.session
            )
            ...
            for trigger in triggers:
                ...
                if (
                    trigger.entity_type
                    and event_data.get("entity_type") != trigger.entity_type
                ):
                    continue
                ...
                if execution_result.is_success:
                    ...
                    await self.trigger_repository.update_last_triggered(
                        trigger.id, datetime.utcnow(), self.session
                    )
                else:
                    self.logger.error(
    ...
    async def check_query_triggers(self) -> Result[List[str]]:
        ...
        try:
            ...
            triggers = [
                ReportTrigger.from_orm(model) for model in result.scalars().all()
            ]
    ...
    async def process_scheduled_triggers(self) -> Result[List[str]]:
        ...
        try:
            ...
            triggers_result = (
                await self.trigger_repository.list_active_scheduled_triggers(
                    self.session
                )
            )
            if triggers_result.is_failure:
                    ...
                    try:
                        if trigger.schedule.startswith("interval:"):
                            ...
                            if len(parts) >= 3:
                                ...
                                if unit == "minutes":
                                    is_due = now >= trigger.last_triggered + timedelta(
                                        minutes=value
                                    )
                                elif unit == "hours":
                                    is_due = now >= trigger.last_triggered + timedelta(
                            ...
                            self.logger.warning(
                                f"Cron schedules not implemented: {trigger.schedule}"
                            )
                    except Exception as e:
                        self.logger.error(
                            f"Error parsing schedule {trigger.schedule}: {str(e)}"
                        )
...
class ReportOutputService(UnoServiceProtocol, ReportOutputServiceProtocol):
    ...
    async def list_outputs_by_template(
        ...
        try:
            ...
            return await self.output_repository.list_by_template(
                template_id, self.session
            )
        except Exception as e:
            return Failure(
    ...
    async def update_output_config(
        ...
        try:
            ...
            output_result = await self.output_repository.get_by_id(
                output_id, self.session
            )
            ...
            for key, value in output_data.items():
                if key not in ["report_template", "output_executions"] and hasattr(
                    existing_output, key
                ):
    ...
    async def deliver_report(self, execution_id: str, output_id: str) -> Result[bool]:
        ...
        try:
            ...
            stmt = select(ReportOutputExecutionModel).where(
                and_(
            ...
            if output_execution_model is None:
                ...
                output_execution = ReportOutputExecution.from_orm(
                    output_execution_model
                )
            ...
            if output.output_type == ReportOutputType.FILE:
                ...
                output_location = (
                    output.output_config.get("path", "/tmp")
                    + f"/{execution_id}.{output.format.lower()}"
                )
                self.logger.info(
                    f"Would write {output_size} bytes to {output_location}"
                )
            ...
            complete_result = (
                await self.output_execution_repository.complete_output_execution(
                    output_execution.id, output_location, output_size, self.session
                )
            )
