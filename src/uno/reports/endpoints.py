...
async def get_output_service(
    ...
    execution_repo = ReportExecutionRepository(session)
    output_execution_repo = ReportOutputExecutionRepository(session)
    return ReportOutputService(session, template_repo, output_repo, execution_repo, output_execution_repo, field_repository=field_repo)
...
async def update_template(
    ...
    result = await service.update_template(template_id, template_data.dict(exclude_unset=True))
    template = handle_result(result, f"Template with ID {template_id} not found")
...
@router.post("/templates/{template_id}/clone", response_model=dict, status_code=status.HTTP_201_CREATED)
...
@router.post("/templates/{template_id}/fields", response_model=dict, status_code=status.HTTP_201_CREATED)
...
async def get_available_fields(
    object_type: str = Path(..., description="The object type to get available fields for"),
    service: ReportFieldService = Depends(get_field_service)
):
...
@router.post("/templates/{template_id}/triggers", response_model=dict, status_code=status.HTTP_201_CREATED)
...
async def update_trigger(
    ...
    """Update a trigger."""
    result = await service.update_trigger(trigger_id, trigger_data.dict(exclude_unset=True))
...
@router.post("/templates/{template_id}/outputs", response_model=dict, status_code=status.HTTP_201_CREATED)
...
async def update_output(
    ...
    result = await service.update_output_config(output_id, output_data.dict(exclude_unset=True))
...
async def get_execution_result(
    execution_id: str = Path(..., description="The ID of the execution"),
    format: Optional[str] = Query(None, description="Optional format to return the result in"),
    service: ReportExecutionService = Depends(get_execution_service)
):
...
@router.post("/executions/{execution_id}/outputs/{output_id}/deliver", response_model=dict)
