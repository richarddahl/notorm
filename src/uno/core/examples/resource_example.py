...
from uno.core.asynchronous.task_manager import get_async_manager, run_application
    ...
    async def advanced_example():
        ...
        async with PooledSessionOperationGroup() as op_group:
            ...
            task1 = await op_group.run_operation(
                session, lambda s: s.execute("SELECT 3 as test3")
            )
            task2 = await op_group.run_operation(
                session, lambda s: s.execute("SELECT 4 as test4")
            )
