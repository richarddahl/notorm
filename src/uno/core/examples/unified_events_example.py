...
from uno.core.unified_events import (
    DomainEvent, EventBus, EventHandler, EventPublisher, InMemoryEventStore,
    EventPriority, EventSubscriber, EventHandlerScanner, event_handler,
    initialize_events, get_event_bus, get_event_publisher, get_event_store,
...
async def send_shipping_notification(event: OrderShippedEvent) -> None:
    ...
    print(f"Order {event.order_id} has been shipped! Tracking number: {event.tracking_number}")
...
def process_order_cancellation(event: OrderCancelledEvent) -> None:
    ...
    print(f"Order {event.order_id} has been cancelled. Reason: {event.reason or 'Not specified'}")
...
class OrderService:
    ...
    async def ship_order(self, order_id: str, tracking_number: str) -> None:
        ...
        self.logger.info(f"Shipped order {order_id} with tracking number {tracking_number}")
...
class AuditService:
    ...
    async def get_user_history(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        order_events = []
        for event_type in ["order_created_event", "order_shipped_event", "order_cancelled_event"]:
...
async def main():
    ...
    bob_order = await order_service.create_order(
        user_id=bob_id,
        items=[
            {"product_id": "prod-3", "name": "SuperWidget", "price": 49.99, "quantity": 1}
        ],
        topic="orders.new"
    )
    ...
    for event in history:
        ...
        if event_type == "user_created_event":
            ...
            print(f"- {timestamp}: Order created: {event.get('order_id')} (${event.get('total_amount', 0):.2f})")
        elif event_type == "order_shipped_event":
            print(f"- {timestamp}: Order shipped: {event.get('order_id')} (Tracking: {event.get('tracking_number')})")
