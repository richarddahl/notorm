...
from uno.core.errors.catalog import register_error, get_all_error_codes, get_error_code_info, ErrorCatalog
...
async def simple_demonstration():
    ...
    for category in ['VALIDATION', 'CONFLICT', 'NOT_FOUND', 'RESOURCE']:
        if category in errors_by_category:
            ...
            for error in errors_by_category[category][:3]:  # Show up to 3 examples
                print(f"  - {error.code}: {error.message_template} (HTTP {error.http_status_code})")
    ...
    print("The source code contains full implementations of all these patterns working together.")
