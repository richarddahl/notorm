...
from uno.core.cqrs import (
    ...
    initialize_mediator, get_mediator, execute_command, execute_query
)
from uno.core.events import (
    Event, EventPublisher, EventPriority, event_handler,
    initialize_events, get_event_publisher, publish_event
)
...
class OrderCommandHandler(BaseCommandHandler[CreateOrderCommand, str]):
    ...
    async def handle(self, command: CreateOrderCommand) -> str:
        ...
        total_amount = sum(item.get("price", 0) * item.get("quantity", 0) for item in command.items)
...
class OrderStatusCommandHandler:
    ...
    def __init__(self, orders: Dict[str, Dict[str, Any]], event_publisher: EventPublisher):
        """
        Initialize the handler.
    ...
    async def approve_order(self, command: ApproveOrderCommand) -> bool:
        ...
        # Add event
        self.add_event(OrderStatusChangedEvent(
...
class OrderQueryHandler:
    ...
    async def get_orders_by_customer(self, query: GetOrdersByCustomerQuery) -> List[Dict[str, Any]]:
        """
    ...
    async def get_orders_by_status(self, query: GetOrdersByStatusQuery) -> List[Dict[str, Any]]:
        """
...
class NotificationService:
    ...
    async def notify_order_created(self, event: OrderCreatedEvent) -> None:
        ...
        print(f"Notification: Order {event.order_id} created for customer {event.customer_id}")
...
class InventoryService:
    ...
    async def reserve_inventory(self, event: OrderCreatedEvent) -> None:
        ...
        for item in event.items:
            ...
            print(f"Inventory: Reserved {quantity} units of product {product_id} for order {event.order_id}")
    
    @event_handler(OrderStatusChangedEvent, priority=EventPriority.HIGH)
    async def update_inventory_on_status_change(self, event: OrderStatusChangedEvent) -> None:
        """
        ...
        if event.new_status in [OrderStatus.CANCELLED, OrderStatus.REJECTED]:
            ...
            print(f"Inventory: Released reserved inventory for order {event.order_id} due to {event.new_status.value}")
        ...
        # Commit inventory reduction if order is shipped
            ...
            print(f"Inventory: Committed inventory reduction for order {event.order_id}")
...
async def main():
    ...
    create_command = CreateOrderCommand(
        ...
        items=[
            {"product_id": "product1", "name": "Product 1", "price": 19.99, "quantity": 2},
            {"product_id": "product2", "name": "Product 2", "price": 29.99, "quantity": 1}
        ],
        shipping_address={
            "street": "123 Main St",
            "city": "Anytown",
    ...
    print(f"Order details: ID={order['id']}, Status={order['status']}, Amount=${order['total_amount']:.2f}")
    ...
    create_command = CreateOrderCommand(
        ...
        items=[
            {"product_id": "product3", "name": "Product 3", "price": 99.99, "quantity": 1}
        ],
        shipping_address={
            "street": "456 Oak St",
            "city": "Othertown",
    ...
    # Reject the order
    reject_command = RejectOrderCommand(
        order_id=order_id2,
    ...
    for order in customer_orders:
        print(f"  - Order {order['id']}: Status={order['status']}, Amount=${order['total_amount']:.2f}")
