...
class UserDatabaseParticipant(TransactionParticipant):
    ...
    async def commit(self, transaction_id: str) -> Result[bool, Error]:
        ...
        try:
            ...
            self.logger.info(f"Committed user database for transaction {transaction_id}")
            return Result.ok(True)
        except Exception as e:
    ...
    async def rollback(self, transaction_id: str) -> Result[bool, Error]:
        ...
        try:
            ...
            del self._prepared_tx[transaction_id]
            self.logger.info(f"Rolled back user database for transaction {transaction_id}")
            return Result.ok(True)
        except Exception as e:
...
class OrderDatabaseParticipant(TransactionParticipant):
    ...
    async def prepare(self, transaction_id: str) -> Result[bool, Error]:
        ...
        try:
            await self.db.begin_transaction()
            self._prepared_tx[transaction_id] = True
            self.logger.info(f"Prepared order database for transaction {transaction_id}")
            return Result.ok(True)
        except Exception as e:
    ...
    async def commit(self, transaction_id: str) -> Result[bool, Error]:
        ...
        try:
            await self.db.commit_transaction()
            del self._prepared_tx[transaction_id]
            self.logger.info(f"Committed order database for transaction {transaction_id}")
            return Result.ok(True)
        except Exception as e:
    ...
    async def rollback(self, transaction_id: str) -> Result[bool, Error]:
        ...
        try:
            await self.db.rollback_transaction()
            del self._prepared_tx[transaction_id]
            self.logger.info(f"Rolled back order database for transaction {transaction_id}")
            return Result.ok(True)
        except Exception as e:
...
class UserOrderService:
    ...
    async def create_user_and_order(
        ...
        user_participant_id = uow.register_participant("user_db", UserDatabaseParticipant(self.user_db))
        order_participant_id = uow.register_participant("order_db", OrderDatabaseParticipant(self.order_db))
        event_participant_id = uow.register_participant("event_store", EventStoreParticipant(self.event_store, "events"))
        ...
        try:
            ...
            async with uow:
                ...
                event_participant.add_events(uow.transaction.transaction_id, [user_event, order_event])
                
                # The transaction will be committed when the context exits
            
            self.logger.info(f"Successfully created user {user_id} and order {order_id}")
...
async def run_example():
    ...
    try:
        ...
        result = await service.create_user_and_order(
            ...
            total_amount=99.99,
            items=[
                {"product_id": "prod-1", "name": "Product 1", "price": 49.99, "quantity": 1},
                {"product_id": "prod-2", "name": "Product 2", "price": 25.00, "quantity": 2},
            ]
        )
