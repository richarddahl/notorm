...
class User:
    ...
    @classmethod
    async def create(cls, email: str, username: str, event_store: PostgresEventStore) -> 'User':
    ...
    async def change_email(self, new_email: str, event_store: PostgresEventStore) -> None:
    ...
    async def deactivate(self, reason: Optional[str], event_store: PostgresEventStore) -> None:
...
async def run_example():
    ...
    try:
        ...
        logger.info(f"Loaded user from events: {loaded_user.username} ({loaded_user.email})")
        ...
        for event in events:
            logger.info(f"Event: {event.event_type}, Version: {event.aggregate_version}")
        
        # Get all UserCreated events
        created_events = await event_store.get_events_by_type(UserCreated.get_event_type())
