        ...
        if "SELECT" in query and "users" in query:
            ...
            if user_id == "not_found":
                ...
                return MockResult([{
                    ...
                    "email": f"user{user_id}@example.com" if user_id else "user@example.com",
                }])
...
async def trace_with_decorator(iterations: int = 3) -> Dict[str, Any]:
    ...
    for i in range(iterations):
        ...
        span.add_event(
            f"iteration_{i}",
            attributes={"iteration": i, "timestamp": time.time()}
        )
...
async def trace_database_operations() -> Dict[str, Any]:
    ...
    @trace_db
    async def create_user(session: MockDatabaseSession, user_data: Dict[str, Any]) -> Dict[str, Any]:
        """
...
app.add_middleware(
    create_request_middleware(app, excluded_paths=["/health"])
)
...
async def get_user_endpoint(user_id: str, session: MockDatabaseSession = Depends(get_db_session)):
...
async def create_user_endpoint(
    ...
    session: MockDatabaseSession = Depends(get_db_session)
):
...
async def run_examples():
    ...
    try:
        ...
        logger.info("Completed context manager example", extra={"result": context_result})
        ...
        logger.info("Completed database operations example", extra={"result": db_result})
