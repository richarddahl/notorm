...
def get_applied_migrations() -> List[Migration]:
    ...
    """
    return [m for m in migration_registry.values() if m.status == MigrationStatus.APPLIED]
...
def get_pending_migrations() -> List[Migration]:
    ...
    """
    return [m for m in migration_registry.values() if m.status == MigrationStatus.PENDING]
...
class Migrator:
    ...
    async def apply_migrations(
        ...
        if migrations is None:
            ...
            migrations_dict = {
                mid: m for mid, m in migration_registry.items()
                if mid not in applied_ids
            }
        else:
            # Use provided migrations
            migrations_dict = {
                m.id: m for m in migrations
        ...
        if target and target in migration_registry:
            ...
            collect_deps(target)
            migrations_dict = {
                mid: m for mid, m in migrations_dict.items()
                if mid in target_deps
            }
        ...
        for migration in ordered_migrations:
            ...
            if len(applied_migrations) >= self.config.batch_size:
                self.logger.info(f"Reached batch size limit of {self.config.batch_size} migrations")
                break
    ...
    async def revert_migrations(
        ...
        if target and target in migration_registry:
            ...
            def collect_deps(mid):
                target_deps.add(mid)
                for dep_id in self._build_dependency_graph(migration_registry).get(mid, set()):
                    if dep_id in migration_registry and dep_id not in target_deps:
                        collect_deps(dep_id)
        ...
        if self.config.allow_missing_down:
            ...
            for migration in ordered_migrations:
                if (
                    isinstance(migration, migration_registry['SqlMigration']) and 
                    migration.down_sql is None
                ) or (
                    isinstance(migration, migration_registry['PythonMigration']) and 
                    migration.down_func is None
                ):
                    self.logger.warning(
