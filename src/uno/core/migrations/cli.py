...
from uno.core.migrations.providers import (
    ...
    ModuleMigrationProvider,
    discover_migrations,
)
from uno.core.migrations.tracker import (
    set_migration_tracker,
    DatabaseMigrationTracker,
    FileMigrationTracker,
)
...
def setup_common_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--schema", default="public", help="Database schema name (default: public)"
    )
...
def create_parser() -> argparse.ArgumentParser:
    ...
    migrate_parser.add_argument(
        "--steps", type=int, default=0, help="Number of migrations to apply (0 for all)"
    )
    migrate_parser.add_argument(
        "--fake",
        action="store_true",
    ...
    create_parser.add_argument(
        "--directory", "-d", required=True, help="Directory to create the migration in"
    )
    create_parser.add_argument(
    ...
    info_parser.add_argument(
        "migration_id", nargs="?", help="Optional migration ID to show information for"
    )
...
""".format(
                    name=args.name, created_at=datetime.datetime.now().isoformat()
                )
...
async def show_migration_status(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        tracker = DatabaseMigrationTracker(
            connection=connection, schema_name=args.schema, table_name=args.table
        )
        ...
        print(
            f"Discovered {len(migrations)} migrations, {len(applied_migrations)} applied"
        )
        print()
        ...
        if args.migration_id:
            ...
            for migration in migrations:
                if migration.id == args.migration_id:
                    ...
                    print(
                        f"Status: {'Applied' if migration.id in applied_migrations else 'Pending'}"
                    )
            ...
            for migration in sorted(migrations, key=lambda m: m.id):
                ...
                print(
                    f"{migration.id:<30} {status:<10} {applied_at:<25} {migration.name:<30}"
                )
