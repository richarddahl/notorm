...
@asynccontextmanager
async def timeout(
    seconds: Optional[float], operation: str = "Operation"
) -> AsyncIterator[None]:
    """
...
class AsyncLock(AbstractAsyncContextManager[None]):
    ...
    def __init__(self, name: str = None, logger: logging.Logger | None = None):
        """
    ...
    async def acquire(self, timeout: Optional[float] = None) -> bool:
        ...
        try:
            ...
            try:
                ...
                raise TimeoutError(
                    operation=f"Lock acquisition '{self.name}'", timeout=timeout
                )
    ...
    def owner_info(self) -> dict[str, Any] | None:
        ...
        return {
            ...
            "depth": self._depth,
            "locked_for": (
                time.time() - (self._locked_at or 0) if self._locked_at else 0
            ),
        }
...
class AsyncSemaphore(AbstractAsyncContextManager[None]):
    ...
    def __init__(
        self, value: int = 1, name: str = None, logger: logging.Logger | None = None
    ):
        """
    ...
    async def acquire(self, timeout: Optional[float] = None) -> bool:
        ...
        try:
            ...
            try:
                ...
                raise TimeoutError(
                    operation=f"Semaphore acquisition '{self.name}'", timeout=timeout
                )
...
class Limiter:
    ...
    def __init__(
        ...
        self.max_concurrent = (
            self.limit
        )  # For compatibility with enhanced async modules
    ...
    def _get_task_name(self) -> str:
        ...
        if frame:
            try:
                for _ in range(
                    3
                ):  # Skip frames for _get_task_name, acquire, and context manager
                    if frame.f_back:
                        frame = frame.f_back
