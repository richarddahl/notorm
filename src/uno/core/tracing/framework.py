...
class LoggingSpanExporter(SpanExporter):
    ...
    async def export_spans(self, spans: list[Span]) -> None:
        ...
        for span in spans:
            ...
            self.logger.info(f"Exported span: {span.name}", extra={"span": span_dict})
...
class TracingConfig:
    ...
    def from_config(cls, config: ConfigProtocol) -> "TracingConfig":
        ...
        try:
            default_attrs = (
                json.loads(default_attrs_str)
                if isinstance(default_attrs_str, str)
                else {}
            )
...
def extract_context(headers: dict[str, str]) -> Optional[PropagationContext]:
    ...
    if trace_id:
        ...
        return PropagationContext(trace_id=trace_id, span_id=str(uuid.uuid4()))
    ...
    async def dispatch(
        ...
        if context:
            ...
            try:
                ...
                current_span.attributes["http.response_content_length"] = (
