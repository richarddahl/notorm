        ...
        async def wrapper() -> HealthCheckResult:
            ...
            try:
                ...
                if isinstance(result, HealthCheckResult):
                    ...
                    return HealthCheckResult(
                        status=status, message=message, check_duration_ms=duration_ms
                    )
                elif isinstance(result, tuple) and len(result) == 2:
                    # Function returns a tuple of (bool, message)
                    status = (
                        HealthStatus.HEALTHY if result[0] else HealthStatus.UNHEALTHY
                    )

                    return HealthCheckResult(
                        status=status, message=result[1], check_duration_ms=duration_ms
                    )
