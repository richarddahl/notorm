...
from uno.core.health.framework import (
    HealthRegistry, HealthStatus, get_health_registry, HealthConfig,
    HealthCheckResult
)
...
class AlertRule(BaseModel):
    ...
    # Targeting
    check_id: Optional[str] = None  # Specific check ID
    check_name: Optional[str] = None  # Check name (supports wildcard patterns)
    group: Optional[str] = None  # Check group
    ...
    def matches(self, check_id: str, check: Dict[str, Any], result: HealthCheckResult) -> bool:
        """
...
class EmailAlertAction(AlertAction):
    ...
    async def send(self, alert: Alert) -> bool:
        ...
        if not self.config.smtp_server or not self.config.email_from or not self.config.email_to:
...
class AlertManager:
    ...
    async def process_health_report(self, report: Dict[str, Any]) -> None:
        ...
        try:
                    ...
                    if status == HealthStatus.HEALTHY:
                        ...
                        for rule in self.rules:
                            if rule.status == HealthStatus.HEALTHY and (
                                rule.check_id == check_id or
                                rule.check_name == check_info.get("name") or
                                rule.group == check_info.get("group") or
                                set(rule.tags).intersection(set(check_info.get("tags", [])))
                            ):
            ...
            if overall_status in (HealthStatus.UNHEALTHY, HealthStatus.DEGRADED):
                ...
                if self.alert_history and time.time() - self.alert_history[0].timestamp < 60:
                    return
            ...
            self.logger.error(f"Error processing health report: {str(e)}\n{traceback.format_exc()}")
    ...
    async def _send_alert(self, alert: Alert) -> None:
        ...
        if (
            self.config.throttle_seconds > 0 and
            self.alert_history and
            alert.check_id == self.alert_history[0].check_id and
            time.time() - self.alert_history[0].timestamp < self.config.throttle_seconds
        ):
            return
        ...
        for action in self.actions:
            try:
                ...
                self.logger.error(f"Error sending alert with {action.__class__.__name__}: {str(e)}")
...
async def setup_health_alerting(
    config: Optional[AlertConfig] = None
) -> AlertManager:
    """
