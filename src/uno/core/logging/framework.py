...
from uno.core.errors.framework import (
    ...
    ErrorDetail,
    FrameworkError,
    log_error as framework_log_error,
)
    ...
    def from_config(cls, config: ConfigProtocol) -> "LogConfig":
        ...
        return cls(
            level=LogLevel.from_string(
                config.get("logging.level", LogLevel.INFO.value)
            ),
            ...
            include_exception_traceback=config.get(
                "logging.include_exception_traceback", True
            ),
...
class StructuredLogger:
    ...
    def exception(
        self, msg: str, *args: Any, exc_info: Any = True, **kwargs: Any
    ) -> None:
