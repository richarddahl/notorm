...
class ProtocolValidationError(Exception):
    ...
    def __init__(
        ...
        message_parts = [
            f"Class '{cls.__name__}' does not properly implement protocol '{protocol.__name__}'"
        ]
...
def validate_protocol(cls: Type[Any], protocol: Any) -> None:
    ...
    if missing_attributes or type_mismatches:
        raise ProtocolValidationError(
            cls, protocol, missing_attributes, type_mismatches
        )
...
def find_protocol_implementations(
    module_or_package: Union[ModuleType, str], protocol: Any
) -> list[Type[Any]]:
    """
    ...
    protocol_attrs = (
        _get_protocol_attributes(protocol)
    ...
    for name, obj in inspect.getmembers(module_or_package):
        if (
            inspect.isclass(obj)
            and obj.__module__ == module_or_package.__name__
            and not inspect.isabstract(obj)
        ):

            # First check for specially marked classes
            if hasattr(obj, "__implemented_protocols__") and protocol in getattr(
                obj, "__implemented_protocols__", []
            ):
            ...
            try:
                ...
                # Last resort for runtime_checkable protocols: try instantiation and check
                    ...
                    hasattr(protocol, "_is_runtime_protocol")
                    and protocol._is_runtime_protocol
                ):
                    try:
        ...
        for _, submodule_name, is_pkg in pkgutil.iter_modules(
            ...
            try:
                ...
                implementations.extend(
                    find_protocol_implementations(submodule, protocol)
                )
            except ImportError:
...
def _get_protocol_attributes(protocol: Any) -> Set[str]:
    ...
    for attr_name, attr in inspect.getmembers(protocol):
        ...
        if (
            ...
            and not attr_name.startswith("_abc_")
            and not attr_name.startswith("_is_")
            and not attr_name.startswith("__pytest_")
            and not attr_name.startswith("_pytest_")
            and attr_name not in ("__module__", "__qualname__", "__parameters__")
        ):
...
def _are_types_compatible(concrete_type: Any, protocol_type: Any) -> bool:
    ...
    if getattr(protocol_type, "__origin__", None) is Union:
        ...
        if type(None) in protocol_type.__args__:
            ...
            if type(None) in getattr(concrete_type, "__args__", ()):
                ...
                protocol_inner = next(
                    (t for t in protocol_type.__args__ if t is not type(None)), Any
                )
                concrete_inner = next(
                    (t for t in concrete_type.__args__ if t is not type(None)), Any
                )
                return _are_types_compatible(concrete_inner, protocol_inner)

    # Handle generic types like List, Dict, etc.
    if (
        hasattr(protocol_type, "__origin__")
        and hasattr(concrete_type, "__origin__")
        and protocol_type.__origin__ == concrete_type.__origin__
    ):
    ...
    if (
        hasattr(protocol_type, "_is_protocol")
        and protocol_type._is_protocol
        and inspect.isclass(concrete_type)
    ):
        try:
    ...
    if protocol_type is Callable or (
        ...
        if concrete_type is Callable or (
            hasattr(concrete_type, "__origin__")
            and concrete_type.__origin__ is Callable
        ):
