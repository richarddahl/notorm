...
class Cache(Generic[K, V]):
    ...
    async def _evict_entries(self, needed_bytes: Optional[int] = None) -> int:
        ...
        if (
            (self.max_size is not None and len(self._cache) > self.max_size) or
            (self.max_bytes is not None and self._total_bytes > self.max_bytes) or
            (needed_bytes is not None)
        ):
            ...
            for key, entry in sorted_entries:
                ...
                if self.max_size is not None and len(self._cache) - len(entries_to_evict) <= self.max_size:
                    break
                
                if self.max_bytes is not None and self._total_bytes - freed_bytes <= self.max_bytes:
                    break
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        async with self._tag_lock, self._refresh_lock:
            stats.update({
                ...
                "tagged_keys": sum(len(keys) for keys in self._tag_to_keys.values()),
                "active_refreshes": sum(1 for task in self._refresh_tasks.values() if not task.done()),
            })
        ...
        async with self._cache_lock:
            ...
            for name, cache in self._caches.items():
                try:
                    ...
                    if count > 0:
                        self.logger.debug(f"Cleaned up {count} entries from cache '{name}'")
                except Exception as e:
            ...
            for name, cache in self._query_caches.items():
                try:
                    ...
                    if count > 0:
                        self.logger.debug(f"Cleaned up {count} entries from query cache '{name}'")
                except Exception as e:
                    self.logger.warning(f"Error cleaning up query cache '{name}': {str(e)}")
