...
from uno.core.monitoring.health import (
    HealthRegistry, HealthStatus, get_health_registry
)
from uno.core.monitoring.metrics import (
    MetricsRegistry, get_metrics_registry, MetricValue, MetricType
)
from uno.core.monitoring.events import (
    EventLogger, get_event_logger, EventLevel
)
from uno.core.resource_monitor import (
    ResourceMonitor, get_resource_monitor, ResourceHealth
)
...
class MonitoringDashboard:
    ...
    def _setup(self) -> None:
                    ...
                    if request.url.path.startswith(self.config.route_prefix):
                        ...
                        if api_key != self.config.api_key:
                            ...
                            return RedirectResponse(
                                url=f"{self.config.route_prefix}/login",
                                status_code=302
                            )
        ...
        async def metrics_api():
            ...
            for metric in metrics:
                ...
                simplified[name]["values"].append({
                    "value": metric.value if not isinstance(metric.value, list) else None,
                    "histogram": metric.value if isinstance(metric.value, list) else None,
                    "tags": metric.tags,
        ...
        async def resources_api(include_history: bool = False):
            """Get resources data."""
            return await self.resource_monitor.get_metrics(include_history=include_history)
            ...
            try:
                ...
                try:
                    ...
                    for metric in all_metrics:
                        if metric.name in ["http_requests_total", "http_error_rate", "system_cpu", "system_memory"]:
                            metrics[metric.name] = {
                                "value": metric.value,
                                "unit": metric.unit.name.lower()
                            }
