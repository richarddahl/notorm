...
from typing import (
    Any, AsyncContextManager, AsyncIterator, Callable, ContextManager, 
    Dict, Generic, List, Optional, Protocol, Type, TypeVar, Union
)
...
class DatabaseManagerProtocol(Protocol):
    ...
    def index_exists(self, index_name: str, schema: Optional[str] = None) -> bool:
        """
...
class QueryExecutorProtocol(Protocol, Generic[QueryResultT]):
    ...
    async def execute(self, query: str, params: Optional[Dict[str, Any]] = None) -> QueryResultT:
        """
        Execute a query.
    ...
    async def execute_many(self, query: str, params_list: List[Dict[str, Any]]) -> List[QueryResultT]:
        """
        Execute a query multiple times with different parameters.
