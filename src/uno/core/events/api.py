...
def create_subscription_router(subscription_manager: SubscriptionManager) -> APIRouter:
    ...
    def subscription_to_response(
        ...
        if subscription.metrics.invocation_count > 0:
            metrics_dict["success_rate"] = (
                subscription.metrics.success_count
                / subscription.metrics.invocation_count
            )
        else:
    ...
    def handle_error(result):
        if isinstance(result, Error):
            if isinstance(result.error, NotFoundError):
                ...
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST, detail=str(result.error)
                )
        return result
    ...
    async def get_subscriptions(
        ...
        active_only: bool = Query(
            False, description="Only return active subscriptions"
        ),
    ):
        """Get all subscriptions."""
        subscriptions = await subscription_manager.repository.get_subscriptions(
            event_type
        )
        ...
        return SubscriptionListResponse(items=response_items, total=len(response_items))
    ...
    async def get_event_types():
        ...
        return EventTypeListResponse(items=event_types, total=len(event_types))

    @router.get("/types/{name}", response_model=EventTypeResponse)
    async def get_event_type(
        name: str = Path(..., description="Name of the event type")
    ):
    ...
    @router.post(
        "/types", response_model=EventTypeResponse, status_code=status.HTTP_201_CREATED
    )
    async def register_event_type(event_type: EventTypeInfo):
        """Register a new event type."""
        registered = await subscription_manager.repository.register_event_type(
            event_type
        )
