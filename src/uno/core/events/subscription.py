...
class SubscriptionConfig(BaseModel):
    ...
    # Advanced settings
    filter_expression: Optional[str] = None  # Optional JSONATA or similar expression to filter events
...
class SubscriptionRepository(Generic[EventProtocol]):
    ...
    async def save(self):
        ...
        async with self._lock:
            try:
                data = {
                    "subscriptions": [sub.model_dump() for sub in self._subscriptions.values()],
                    "event_types": [et.model_dump() for et in self._event_types.values()]
                }
    ...
    async def update_subscription(self, subscription_id: str, config: SubscriptionConfig) -> Result[SubscriptionInfo]:
        """
        ...
        async with self._lock:
            if subscription_id not in self._subscriptions:
                return Error(NotFoundError(f"Subscription with ID {subscription_id} not found"))
    ...
    async def get_subscriptions(self, event_type: Optional[str] = None) -> List[SubscriptionInfo]:
        """
        ...
        """
        if event_type:
            return [sub for sub in self._subscriptions.values() if sub.event_type == event_type]
        else:
            return list(self._subscriptions.values())
    
    async def update_metrics(self, subscription_id: str, metrics: SubscriptionMetrics) -> Result[bool]:
        """
...
class SubscriptionManager(Disposable):
        ...
        try:
            ...
            async def metrics_wrapper(event: EventProtocol):
                ...
                self._events_processed_counter.inc(
                    labels={"event_type": event.event_type, "handler": subscription.handler_name, "status": "started"}
                )
                ...
                try:
                    ...
                    if asyncio.iscoroutinefunction(handler):
                        with self._processing_time_timer.time(
                            labels={"event_type": event.event_type, "handler": subscription.handler_name}
                        ):
                            await handler(event)
                    else:
                        with self._processing_time_timer.time(
                    ...
                    self._events_processed_counter.inc(
                        labels={"event_type": event.event_type, "handler": subscription.handler_name, "status": "success"}
                    )
                except Exception as e:
                    # Track failure
                    subscription.metrics.failure_count += 1
                    self._events_processed_counter.inc(
                        labels={"event_type": event.event_type, "handler": subscription.handler_name, "status": "failure"}
                    )
                    ...
                    if subscription.metrics.invocation_count == 1:
                        ...
                        subscription.metrics.avg_processing_time_ms = (
                            subscription.metrics.avg_processing_time_ms * (n - 1) / n + processing_time_ms / n
                        )
                        ...
                        if processing_time_ms > subscription.metrics.p95_processing_time_ms:
                            subscription.metrics.p95_processing_time_ms = (
                                0.95 * subscription.metrics.p95_processing_time_ms + 0.05 * processing_time_ms
                            )
                        if processing_time_ms > subscription.metrics.p99_processing_time_ms:
                            subscription.metrics.p99_processing_time_ms = (
                                0.99 * subscription.metrics.p99_processing_time_ms + 0.01 * processing_time_ms
                            )
                    
                    # Periodically update metrics in repository
                    if subscription.metrics.invocation_count % 10 == 0:
                        asyncio.create_task(self.repository.update_metrics(
            ...
            self._subscription_gauge.set(
                len([s for s in await self.repository.get_subscriptions(subscription.event_type) if s.is_active]),
    ...
    async def unregister_handler(self, subscription_id: str) -> Result[bool]:
        ...
        if subscription_id not in self._handlers:
            return Error(NotFoundError(f"No active handler for subscription {subscription_id}"))
    ...
    async def create_subscription(self, config: SubscriptionConfig) -> Result[SubscriptionInfo]:
        """
        Create a new subscription.
    ...
    async def update_subscription(self, subscription_id: str, config: SubscriptionConfig) -> Result[SubscriptionInfo]:
        ...
        elif was_active and is_active and (
            updated.event_type != current.event_type or
            updated.handler_module != current.handler_module or
            updated.handler_name != current.handler_name or
            updated.handler_function != current.handler_function
        ):
    ...
    async def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_subscriptions": len([sub for sub in subscriptions if sub.is_active]),
            ...
            "invocations": {
                ...
                "success_rate": total_success / total_invocations if total_invocations > 0 else 1.0
            },
