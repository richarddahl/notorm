...
class DomainEvent(BaseModel):
    ...
    event_type: str = Field(default_factory=lambda: _cls_name_to_event_type(DomainEvent.__name__))
        ...
        try:
            ...
            if not isinstance(e, UnoError):
                raise UnoError(
                    ...
                    context={"handler": self.handler_name, "cause": str(e), "event_id": event.event_id}
                )
            raise
...
class EventBus:
    ...
    def subscribe(
        ...
        handler_name = handler.__class__.__name__ if isinstance(handler, EventHandler) else getattr(handler, "__name__", str(handler))
        self.logger.debug(
    ...
    def unsubscribe(
        ...
        def matches_criteria(subscription: EventSubscription) -> bool:
            ...
            if isinstance(handler, EventHandler) and isinstance(sub_handler, EventHandler):
            ...
            # Check topic pattern
            if topic_pattern is not None and subscription.topic_pattern != compiled_pattern:
                return False
            
            return True
        
        # Remove matching subscriptions
        self._subscriptions = [s for s in self._subscriptions if not matches_criteria(s)]
    ...
    async def publish(self, event: DomainEvent) -> None:
        ...
        self.logger.debug(f"Publishing event: {event_type_name} (topic={topic}, id={event.event_id})")
    ...
    async def publish_async(self, event: DomainEvent) -> None:
        ...
        self.logger.debug(f"Publishing event {event_type_name} (id={event.event_id}, topic={topic}) asynchronously")
    ...
    async def publish_many_async(self, events: List[DomainEvent]) -> None:
        """
...
class EventPublisher:
    ...
    def collect_many(self, events: List[DomainEvent]) -> None:
        """
...
class InMemoryEventStore(EventStore[E]):
    ...
    async def get_events_by_aggregate_id(
        ...
        Returns:
            List of events for the aggregate
        """
        filtered_events = [e for e in self.events if getattr(e, "aggregate_id", None) == aggregate_id]
        ...
        for name, obj in inspect.getmembers(module):
            ...
            if inspect.isfunction(obj) and hasattr(obj, "__event_handler__"):
                ...
                if event_type is not None:
                    ...
                    count += 1
                    self.logger.debug(f"Registered function handler {obj.__name__} for {event_type.__name__}")
                            ...
                            try:
                                if isinstance(param_type, type) and hasattr(param_type, "__init__"):
                                    ...
                                    topic_pattern = getattr(
                                        handle_method, "__event_topic_pattern__", 
                                    ...
                                    count += 1
                                    self.logger.debug(f"Registered class handler {obj.__name__} for {param_type.__name__}")
                            except TypeError:
        ...
        for name, method in inspect.getmembers(instance, inspect.ismethod):
            if hasattr(method, "__event_handler__"):
                ...
                if event_type is not None:
                    ...
                    count += 1
                    self.logger.debug(f"Registered method handler {instance.__class__.__name__}.{name} for {event_type.__name__}")
...
def collect_events(events: List[DomainEvent]) -> None:
    """
    Collect multiple events for later batch publishing.
