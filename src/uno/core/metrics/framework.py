...
class MetricUnit(str, Enum):
    ...
    def from_string(cls, unit_str: str) -> "MetricUnit":
        ...
        try:
            ...
            raise ValueError(
                f"Invalid metric unit: {unit_str}. Valid units are: {valid_units}"
            )
...
class MetricsConfig:
    ...
    def from_config(cls, config: ConfigProtocol) -> MetricsConfig:
        ...
        try:
            default_tags = (
                json.loads(default_tags_str)
                if isinstance(default_tags_str, str)
                else {}
            )
    ...
    def decorator(func: F) -> F:
        ...
        async def async_wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            timer = await registry.get_or_create_timer(
                name=timer_name, description=description, tags=merged_tags
            )
...
def with_metrics_context(
    func: Optional[F] = None, **context: Any
        ...
        async with self._lock:
            if key not in self._counters:
                self._counters[key] = Counter(
                    name=name, description=description, unit=unit, tags=merged_tags
                )
        ...
        async with self._lock:
            if key not in self._timers:
                self._timers[key] = Timer(
                    name=name, description=description, tags=merged_tags
                )
...
class MetricsMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        ...
        async def metrics():
            ...
            return PlainTextResponse(
                content=self.registry.get_prometheus_metrics(), media_type="text/plain"
            )
    ...
    async def dispatch(
        ...
        tags = {"method": method, "path": path_template}
        ...
        try:
            ...
            # Track error
            error_tags = {**tags, "error_type": exc.__class__.__name__}
            await self.error_counter.increment()

            # Log the error
            self.logger.error(
                f"Error processing request {method} {path}: {str(exc)}", exc_info=exc
            )
