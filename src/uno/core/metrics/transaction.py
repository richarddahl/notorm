...
class TransactionMetricsTracker:
    ...
    async def get_transaction_statistics(self) -> dict[str, float | int]:
        ...
        return {
            ...
            "success_rate": (
                len(succeeded) / len(recent_transactions)
                if recent_transactions
                else 0.0
            ),
            "avg_queries_per_transaction": statistics.mean(
                [t.query_count for t in recent_transactions]
            ),
...
class TransactionContext:
    ...
    async def rollback_to_savepoint(self, savepoint: Any) -> None:
        ...
        self.logger.debug(
            f"Rolled back to savepoint in transaction: {self.transaction_id}"
        )
