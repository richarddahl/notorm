...
It demonstrates how to use BaseError, context, and the Result pattern.
"""
...
# Basic example endpoint that demonstrates BaseError usage
...
async def basic_error_example(
    ...
    if error_type == "validation":
        raise ValidationError(
            message=error_message, field="example_field", value="invalid_value"
        )
        ...
        raise EntityNotFoundError(entity_type="ExampleEntity", entity_id="example-123")
    elif error_type == "internal":
        raise BaseError(message=error_message, error_code=ErrorCode.INTERNAL_ERROR)
...
async def context_example(
    ...
    return {
        ...
        "context_example": {"user_id": user_id, "operation": operation},
    }
...
async def from_exception_example(
    ...
    if result.is_success:
        ...
        # Convert the Result.error to a BaseError
        error = result.error
        if isinstance(error, ZeroDivisionError):
            # Convert specific errors to BaseError
            raise BaseError(
