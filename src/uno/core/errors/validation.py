...
from uno.core.base.error import BaseError, ErrorCode, ValidationError
...
class ValidationContext:
    ...
    def add_error(self, field: str, message: str, value: Any = None) -> None:
        ...
        full_path = (
            ".".join(self.current_path + [field])
            if field
            else ".".join(self.current_path)
        )

        self.errors.append({"field": full_path, "message": message, "value": value})
...
def validate_fields(
    ...
    for field in required_fields:
        if (
            ...
            or (isinstance(data[field], str) and not data[field])
        ):
            context.add_error(
                field=field,
    ...
    for field, field_validators in validators.items():
        if field in data and data[field] is not None:
            ...
            for validator in field_validators:
                ...
                if error_message:
                    context.add_error(field=field, message=error_message, value=value)
