...
class ConfigTypeMismatchError(BaseError):
...
class ComponentInitializationError(BaseError):
...
class DependencyResolutionError(BaseError):
...
class DependencyCycleError(BaseError):
...
class ObjectInvalidError(BaseError):
...
class ObjectPropertyError(BaseError):
...
# Protocol errors
class ProtocolValidationError(BaseError):
...
class InterfaceMethodError(BaseError):
