...
from typing import Any, Dict, List, Optional, Set, Tuple, Union, TypeVar, Generic, Type, cast, Callable
import logging
import asyncio
import time
...
class QueryMetrics:
    ...
    def record_execution(self, time_ms: float, row_count: int, from_cache: bool = False) -> None:
        """
...
class QueryPlan:
    ...
    def _get_optimal_hints(self) -> QueryHints:
        ...
        elif self.pattern in (QueryPattern.FULL_TEXT_SEARCH, QueryPattern.COMPLEX_FILTER):
    ...
    async def find_by_id(
        ...
        if use_cache:
            ...
            if model and load_relations:
                ...
                model = await loader.load_relationships(model, load_relations, self.session)
        else:
            # Execute directly
            result = await self.query_builder.execute(
                query=query,
    ...
    async def find_all(
        ...
        if use_cache:
            ...
            if result and load_relations:
                ...
                loader = RelationshipLoader(self.model_class)
                result = await loader.load_relationships_batch(result, load_relations, self.session)
            ...
            if models and load_relations:
                ...
                # Create loader and load relationships in batch
                loader = RelationshipLoader(self.model_class)
                models = await loader.load_relationships_batch(models, load_relations, self.session)
    ...
    async def get_metrics(self) -> Dict[str, Dict[str, Any]]:
        ...
        for pattern, pattern_metrics in self.metrics.items():
            ...
            metrics[pattern.value] = {
                ...
                "min_time_ms": pattern_metrics.min_time if pattern_metrics.min_time != float('inf') else 0,
                ...
                "cache_hit_rate": pattern_metrics.cache_hits / pattern_metrics.execution_count if pattern_metrics.execution_count > 0 else 0,
            }
