...
from typing import Any, Dict, Generic, List, Optional, Type, TypeVar, cast, Union, Callable, get_type_hints
...
class EntityApi(Generic[EntityT]):
    ...
    def _register_routes(self) -> None:
        ...
        async def get_entity_by_id(
            ...
            try:
                ...
                if result.is_success and result.output is None:
                    return JSONResponse(
                        status_code=status.HTTP_404_NOT_FOUND,
                        content={"code": "NOT_FOUND", "message": f"{self.entity_type.__name__} not found"}
                    )
        ...
        @self.router.get(
            ...
            response_model=PaginatedResponse[self.response_model] if self.response_model else None,
            tags=self.tags,
        )
    ...
    def _register_aggregate_routes(self) -> None:
        ...
        async def update_aggregate(
            ...
            try:
                ...
                # Execute the update operation
                result = await self.service.update(entity_id, version, entity_data, context)
...
def create_dto_for_entity(
    ...
    for field_name, field_type in get_type_hints(entity_type).items():
        ...
        # Add field to DTO
        entity_fields[field_name] = (field_type, ... if optional is None or field_name not in optional else None)
