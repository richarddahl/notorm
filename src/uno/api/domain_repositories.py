...
class InMemoryEndpointConfigRepository(EndpointConfigRepositoryProtocol):
    ...
    async def update(self, endpoint: EndpointConfig) -> Result[EndpointConfig]:
        ...
        if (
            hasattr(old_endpoint, "resource_id")
            and hasattr(endpoint, "resource_id")
            and old_endpoint.resource_id != endpoint.resource_id
        ):

            # Remove from old resource
            if (
                old_endpoint.resource_id
                and old_endpoint.resource_id in self.endpoints_by_resource
            ):
                if endpoint.id in self.endpoints_by_resource[old_endpoint.resource_id]:
                    self.endpoints_by_resource[old_endpoint.resource_id].remove(
                        endpoint.id
                    )
    ...
    async def delete(self, id: str) -> Result[bool]:
        ...
        if id in self.endpoints:
            ...
            if hasattr(endpoint, "resource_id") and endpoint.resource_id:
                resource_id = endpoint.resource_id
                if (
                    resource_id in self.endpoints_by_resource
                    and id in self.endpoints_by_resource[resource_id]
                ):
        ...
        try:
            ...
            for file_path in resource_dir.glob("*.json"):
                try:
                    ...
                    self.logger.error(
                        f"Error loading resource from {file_path}: {str(e)}"
                    )
        except Exception as e:
        ...
        if id in self.resources:
            ...
            if os.path.exists(file_path):
                try:
                    ...
                    self.logger.error(
                        f"Error deleting resource file {file_path}: {str(e)}"
                    )
