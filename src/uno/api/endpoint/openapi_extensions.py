...
class DocumentedCrudEndpoint(CrudEndpoint, DocumentedBaseEndpoint):
    ...
    def _add_default_documentation(self) -> None:
        """Add default documentation for CRUD operations."""
        entity_name = self.response_model.__name__.replace("DTO", "").replace("Response", "")
    ...
    def _get_operation_examples(self, operation: str) -> Dict[str, ResponseExample]:
        ...
        if operation in self.operation_examples:
            ...
            for status_code, data in operation_examples.items():
                if isinstance(data, dict) and "content" in data and "description" in data:
        ...
        if not examples:
            ...
            if operation == "create":
                ...
                examples["400"] = ResponseExample(
                    status_code=400,
                    example={"message": "Invalid input data", "code": "VALIDATION_ERROR"},
                ...
                examples["404"] = ResponseExample(
                    status_code=404,
                    example={"message": f"{entity_name} not found", "code": "NOT_FOUND"},
    ...
    def _get_operations(self) -> List[tuple]:
        ...
        if f"post:{self.path}" in self.operation_docs:
            operations.append((self.path, "post", self.operation_docs[f"post:{self.path}"]))
        
        # Get operation
        if f"get:{self.path}/{{id}}" in self.operation_docs:
            operations.append((f"{self.path}/{{id}}", "get", self.operation_docs[f"get:{self.path}/{{id}}"]))
        
        # List operation
        if f"get:{self.path}" in self.operation_docs:
            operations.append((self.path, "get", self.operation_docs[f"get:{self.path}"]))
        
        # Update operation
        if f"put:{self.path}/{{id}}" in self.operation_docs:
            operations.append((f"{self.path}/{{id}}", "put", self.operation_docs[f"put:{self.path}/{{id}}"]))
        
        # Delete operation
        if f"delete:{self.path}/{{id}}" in self.operation_docs:
            operations.append((f"{self.path}/{{id}}", "delete", self.operation_docs[f"delete:{self.path}/{{id}}"]))
...
class DocumentedQueryEndpoint(QueryEndpoint, DocumentedBaseEndpoint):
    ...
    def _add_default_documentation(self) -> None:
        """Add default documentation for query operation."""
        operation_name = self.service.__class__.__name__.replace("Service", "").replace("Query", "")
        
        self.document_operation(
            self.path,
            self.method,
            summary=f"Execute {operation_name} query",
            description=self.description or f"Executes the {operation_name} query and returns the results.",
    ...
    def _get_operation_examples(self) -> Dict[str, ResponseExample]:
        ...
        if not examples:
            ...
            examples["400"] = ResponseExample(
                status_code=400,
                example={"message": "Invalid query parameters", "code": "VALIDATION_ERROR"},
...
class DocumentedCommandEndpoint(CommandEndpoint, DocumentedBaseEndpoint):
    ...
    def _add_default_documentation(self) -> None:
        """Add default documentation for command operation."""
        operation_name = self.service.__class__.__name__.replace("Service", "").replace("Command", "")
        
        self.document_operation(
            self.path,
            self.method,
            summary=f"Execute {operation_name} command",
            description=self.description or f"Executes the {operation_name} command with the provided data.",
...
class DocumentedFilterableCqrsEndpoint(FilterableCqrsEndpoint, DocumentedCqrsEndpoint):
    ...
    def _add_filter_documentation(self) -> None:
        ...
        for i, query_handler in enumerate(self.queries):
            if hasattr(query_handler, "supports_filtering") and query_handler.supports_filtering:
                path = f"{query_handler.path}"
                method = query_handler.method
                operation_name = getattr(query_handler, "name", f"Query{i+1}")
