...
class SecureCrudEndpoint(CrudEndpoint):
    ...
    def _register_create_route(self) -> None:
        ...
        @self.router.post(
            ...
            dependencies=[Depends(RequirePermissions(self.create_permissions))] if self.create_permissions else None,
        )
        async def create(
    ...
    def _register_get_route(self) -> None:
        ...
        @self.router.get(
            f"{self.path}/{{id}}",
            response_model=self.response_model,
            dependencies=[Depends(RequirePermissions(self.read_permissions))] if self.read_permissions else None,
        )
        async def get(
            id: str,
            user_context: UserContext = Depends(get_user_context),
    ...
    def _register_update_route(self) -> None:
        ...
        @self.router.put(
            f"{self.path}/{{id}}",
            response_model=self.response_model,
            dependencies=[Depends(RequirePermissions(self.update_permissions))] if self.update_permissions else None,
        )
        async def update(
            id: str,
    ...
    def _register_delete_route(self) -> None:
        ...
        @self.router.delete(
            ...
            dependencies=[Depends(RequirePermissions(self.delete_permissions))] if self.delete_permissions else None,
        )
        async def delete(
            id: str,
            user_context: UserContext = Depends(get_user_context),
