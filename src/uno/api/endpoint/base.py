    ...
    def _register_command_route(self) -> None:
        ...
        if self.response_model:
            @handler(
                self.path,
                response_model=self.response_model,
                status_code=status.HTTP_201_CREATED if self.method == "post" else status.HTTP_200_OK,
            )
            async def command(data: self.command_model) -> ResponseModel:
                result = await self.service.execute(data)
                success_status = status.HTTP_201_CREATED if self.method == "post" else status.HTTP_200_OK
