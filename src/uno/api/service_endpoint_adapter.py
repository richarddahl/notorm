...
import inspect
from typing import (
    Any, Dict, List, Optional, Type, TypeVar, Generic, Union, Protocol, 
    Callable, Awaitable, cast, get_type_hints, get_origin, get_args
)
...
def convert_to_pydantic_model(
    ...
    return create_model(
        model_name,
        **{field: (typ, ...) for field, typ in annotations.items()}
    )
