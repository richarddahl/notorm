...
from uno.core.unified_events import UnoDomainEvent, event_handler
...
from uno.domain.unified_services import (
    ...
    DomainServiceFactory, initialize_service_factory
)
...
from uno.api.service_endpoint_factory import (
    DomainServiceEndpointFactory, get_domain_service_endpoint_factory
)
...
class User(Entity):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._events: List[UnoDomainEvent] = []
    ...
    def deactivate(self) -> None:
        ...
        # Record an event
        self._events.append(UserDeactivatedEvent(
...
class UserDeactivatedEvent(UnoDomainEvent):
    """Event raised when a user is deactivated."""
    username: str


class OrderItemAddedEvent(UnoDomainEvent):
...
class SearchUsersService(ReadOnlyDomainService[SearchUsersInput, UserList, InMemoryUnitOfWork]):
    ...
    async def _execute_internal(self, input_data: SearchUsersInput) -> Result[UserList]:
        ...
        if input_data.username_contains:
            users = [u for u in users if input_data.username_contains.lower() in u.username.lower()]
            
        if input_data.email_contains:
            users = [u for u in users if input_data.email_contains.lower() in u.email.lower()]
...
class DeactivateUserService(DomainService[DeactivateUserInput, UserOutput, InMemoryUnitOfWork]):
    """Service for deactivating users."""
    
    async def _execute_internal(self, input_data: DeactivateUserInput) -> Result[UserOutput]:
        """Deactivate a user."""
        # Get the user
...
class CreateOrderService(DomainService[CreateOrderInput, OrderOutput, InMemoryUnitOfWork]):
...
class AddOrderItemService(DomainService[OrderItemInput, OrderOutput, InMemoryUnitOfWork]):
    """Service for adding items to an order."""
    
    async def _execute_internal(self, input_data: OrderItemInput) -> Result[OrderOutput]:
...
class PlaceOrderService(DomainService[PlaceOrderInput, OrderOutput, InMemoryUnitOfWork]):
    """Service for placing orders."""
    
    async def _execute_internal(self, input_data: PlaceOrderInput) -> Result[OrderOutput]:
        """Place an order."""
        # Get the order
        order = await self.uow.orders.get(input_data.order_id)
        if not order:
...
async def handle_user_deactivated(event: UserDeactivatedEvent) -> None:
    ...
    print(f"User {event.username} has been deactivated (aggregate ID: {event.aggregate_id})")


@event_handler(OrderPlacedEvent)
async def handle_order_placed(event: OrderPlacedEvent) -> None:
    """Handle order placed events."""
    print(f"Order {event.aggregate_id} has been placed by user {event.user_id} "
