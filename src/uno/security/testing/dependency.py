                        ...
                        for vuln_name, vuln_data in data["vulnerabilities"].items():
                            ...
                            if isinstance(via, list) and via:
                                if isinstance(via[0], dict):
                                    ...
                                    vuln_title = via[0].get("title", f"Vulnerability in {vuln_name}")
                                    vuln_description = via[0].get("description", "No description available")
                                else:
                                    vuln_id = vuln_name
                            ...
                            vulnerabilities.append(Vulnerability(
                                ...
                                recommendation=vuln_data.get("recommendation", "Update to a non-vulnerable version")
                            ))
                except json.JSONDecodeError:
