    ...
    def decrypt(self) -> 'EncryptedField[T]':
        ...
        if self.value is not None:
            if isinstance(self.value, str) and self.value.startswith("json:"):
                ...
                elif isinstance(self.value, float) or type(self.value).__name__ == 'float':
                    self.value = cast(T, float(decrypted))
                elif isinstance(self.value, bool) or type(self.value).__name__ == 'bool':
...
class FieldEncryption:
    ...
    def encrypt_field(
        ...
        field = EncryptedField(value=value, field_name=field_name, entity_type=entity_type)
