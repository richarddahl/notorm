...
class Property(BaseModel):
    ...
    def create_filter_field_sql(self) -> str:
        ...
        1. Inserts a new filter field into the `uno.filterfield` table with the specified attributes.
           - If a conflict occurs on the combination of `label` and `graph_type`, the insertion is ignored.
        2. Inserts a relationship between the filter field and a table type into the `uno.filterfield_ObjectType` table.
        ...
        return textwrap.dedent(
            ...
            INSERT INTO uno.filterfield_ObjectType (
                filterfield_id,
                object_type_id,
                direction
            )
            SELECT f.id, t.id, '{EdgeDirection.FROM.name}'
            FROM uno.filterfield f
...
class Vertex(BaseModel):
    ...
    def create_filter_field_sql(self) -> str:
        ...
        1. Inserts a new filter field into the `uno.filterfield` table with the specified attributes.
           - If a conflict occurs on the combination of `label` and `graph_type`, the insertion is ignored.
        2. Inserts a new association between the filter field and a table type into the `uno.filterfield_ObjectType` table.
