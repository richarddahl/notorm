...
from sqlalchemy import (
    ...
    DateTime, ForeignKey, Table, JSON, Enum
)
...
class ProductModel(UnoModel, Base):
    ...
    # Relationships
    variants = relationship("ProductVariantModel", back_populates="product", 
                           cascade="all, delete-orphan")
    images = relationship("ProductImageModel", back_populates="product",
                         cascade="all, delete-orphan")
    categories = relationship("CategoryModel", secondary=product_category_association,
                            back_populates="products")


class ProductVariantModel(UnoModel, Base):
...
class ProductImageModel(UnoModel, Base):
...
class CategoryModel(UnoModel, Base):
    ...
    children = relationship("CategoryModel", 
                          backref=relationship("CategoryModel", remote_side=[id]))
    products = relationship("ProductModel", secondary=product_category_association,
