...
from uno.domain.repository import SQLAlchemyAggregateRepository, SQLAlchemyRepository
...
from uno.examples.ecommerce_app.catalog.repository.models import (
    ProductModel, CategoryModel, ProductVariantModel, ProductImageModel,
...
class ProductRepository(SQLAlchemyAggregateRepository[Product, ProductModel]):
    ...
    def _to_entity(self, model: ProductModel) -> Product:
        ...
        if model.weight_value is not None and model.weight_unit is not None:
            product.weight = Weight(
                value=Decimal(str(model.weight_value)),
                unit=model.weight_unit
            )
        
        # Add dimensions if available
        if (model.dimensions_length is not None and 
            model.dimensions_width is not None and 
            model.dimensions_height is not None):
    ...
    async def add(self, entity: Product) -> Product:
        ...
        stmt = select(ProductModel).where(ProductModel.id == entity.id).options(
    ...
    def _specification_to_criteria(self, specification: Specification[Product]) -> Optional[BinaryExpression]:
        ...
        if isinstance(specification, CompositeSpecification):
            if hasattr(specification, "left") and hasattr(specification, "right"):
                ...
                if hasattr(specification, "__class__") and specification.__class__.__name__ == "AndSpecification":
                    return and_(left_expr, right_expr)
                elif hasattr(specification, "__class__") and specification.__class__.__name__ == "OrSpecification":
            ...
            if hasattr(specification, "specification"):
                ...
                if hasattr(specification, "__class__") and specification.__class__.__name__ == "NotSpecification":
                    return not_(expr)
...
class CategoryRepository(SQLAlchemyRepository[Category, CategoryModel]):
    ...
    def _specification_to_criteria(self, specification: Specification[Category]) -> Optional[BinaryExpression]:
        """Convert a specification to SQLAlchemy criteria."""
        # Handle SqlAlchemySpecification
        if isinstance(specification, SqlAlchemySpecification):
            return specification.to_expression()
