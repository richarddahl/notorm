...
from uno.domain.specifications import AndSpecification, OrSpecification, NotSpecification

from uno.examples.ecommerce_app.catalog.domain.entities import Product, Category, ProductVariant, ProductImage
from uno.examples.ecommerce_app.catalog.domain.value_objects import ProductStatus, Dimensions, Weight, Inventory
...
from uno.examples.ecommerce_app.catalog.repository.specifications import (
    ProductByStatusSpecification, ProductByCategorySpecification, 
    ProductByPriceRangeSpecification, ProductByNameSpecification
)
...
class ProductService(DomainService[Union[ProductCreateCommand, ProductUpdateCommand], Product, UnitOfWork]):
    """Service for working with products."""
    
    async def _execute_internal(self, input_data: Union[ProductCreateCommand, ProductUpdateCommand]) -> Result[Product]:
    ...
    async def _update_product(self, command: ProductUpdateCommand) -> Result[Product]:
        ...
        try:
            ...
            if product.version != command.version:
                return Failure(f"Concurrency conflict: expected version {command.version}, found {product.version}")
            ...
            if any([
                ...
                new_inventory = Inventory(
                    quantity=command.inventory_quantity if command.inventory_quantity is not None else product.inventory.quantity,
                    reserved=command.inventory_reserved if command.inventory_reserved is not None else product.inventory.reserved,
                    backorderable=command.inventory_backorderable if command.inventory_backorderable is not None else product.inventory.backorderable,
                    restock_threshold=command.inventory_restock_threshold if command.inventory_restock_threshold is not None else product.inventory.restock_threshold
                )
            ...
            if command.category_ids is not None:
                ...
                for cat_id in list(product.category_ids):  # Create a copy to avoid modifying during iteration
...
class ProductQueryService(ReadOnlyDomainService[Union[ProductListQuery, ProductDetailQuery], Union[List[Product], Product], UnitOfWork]):
    """Service for querying products."""
    
    async def _execute_internal(self, input_data: Union[ProductListQuery, ProductDetailQuery]) -> Result[Union[List[Product], Product]]:
    ...
    async def _list_products(self, query: ProductListQuery) -> Result[List[Product]]:
        ...
        try:
            ...
            if query.min_price is not None or query.max_price is not None:
                specifications.append(ProductByPriceRangeSpecification(
...
class CategoryService(DomainService[Union[CategoryCreateCommand, CategoryUpdateCommand], Category, UnitOfWork]):
    """Service for working with categories."""
    
    async def _execute_internal(self, input_data: Union[CategoryCreateCommand, CategoryUpdateCommand]) -> Result[Category]:
    ...
    async def _create_category(self, command: CategoryCreateCommand) -> Result[Category]:
    ...
    async def _update_category(self, command: CategoryUpdateCommand) -> Result[Category]:
...
class CategoryQueryService(ReadOnlyDomainService[CategoryListQuery, List[Category], UnitOfWork]):
    """Service for querying categories."""
    
    async def _execute_internal(self, input_data: CategoryListQuery) -> Result[List[Category]]:
    ...
    async def get_hierarchy(self) -> Result[List[Category]]:
        ...
        try:
            ...
            self.logger.error(f"Error getting category hierarchy: {str(e)}", exc_info=True)
            return Failure(str(e))