        ...
        if hasattr(entity, "get_changes") and callable(entity.get_changes):
            ...
            if "items" in changes:
                for item in entity.items:
                    ...
                    self.events.append(OrderItemAddedEvent(
                        order_id=entity.id,
...
class OrderService:
    ...
    async def add_item_to_order(self, order_id: str, product_id: str, quantity: int, price: float) -> Order:
        """
...
async def run_example():
    ...
    try:
        ...
        order = await order_service.add_item_to_order(
            order.id, "product-456", 2, 29.99
        )
        order = await order_service.add_item_to_order(
            order.id, "product-789", 1, 49.99
        )
