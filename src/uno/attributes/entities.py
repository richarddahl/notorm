...
class AttributeType(AggregateRoot[str]):
    ...
    def validate(self) -> None:
        ...
        if self.comment_required and not self.initial_comment:
            raise ValidationError("Initial comment is required when comment is required")
    ...
    def add_describable_type(self, meta_type_id: str, name: Optional[str] = None) -> None:
        """
...
class Attribute(AggregateRoot[str]):
    ...
    def validate(self) -> None:
        ...
        if self.attribute_type and self.attribute_type.comment_required and not self.comment:
    ...
    def add_value(self, value_id: str) -> None:
        ...
        if value_id not in self.value_ids:
            ...
            if self.attribute_type and not self.attribute_type.multiple_allowed and self.value_ids:
