...
class UnoFilterManager(FilterManagerProtocol):
        ...
        try:
            ...
            if len(self._filter_cache) > FILTER_CACHE_MAX_SIZE:
                ...
                self._filter_cache = dict(
                    sorted(self._filter_cache.items(), key=lambda x: hash(x[0]))[
                        :keep_count
                    ]
                )
        ...
        model_filter_dict = OrderedDict(
            {
                "limit": (
                    Optional[int],
                    Field(None, description="Maximum number of results to return"),
                ),
                "offset": (
                    Optional[int],
                        ...
                        model_filter_dict.update(
                            {
                                label_: (
                                    ...
                                    Field(
                                        None,
                                        description=f"Filter by {label} with {lookup} comparison",
                                    ),
                                )
                            }
                        )
    ...
    def validate_filter_params(
        ...
        if (
            FILTER_CACHE_ENABLED and use_cache and not use_or_condition
        ):  # Skip cache for OR conditions
            try:
        ...
        expected_params.update(
            ["limit", "offset", "order_by", "or"]
        )  # Add 'or' as acceptable prefix
        ...
        non_null_params = {
            k: v for k, v in filter_params.model_dump().items() if v is not None
        }

        # Process special parameters first (limit, offset, order_by)
        special_params = {
            k: v
            for k, v in non_null_params.items()
        ...
        for key, val in and_condition_params.items():
            ...
            if edge_upper not in self.filters.keys():
                context.add_error(
                    key, f"Invalid filter key: {key}", "INVALID_FILTER_KEY", val
                )
                continue
        ...
        if (
            ...
            and use_cache
            and not use_or_condition
            and not any(f.condition == "OR" for f in filters)
        ):
            try:
                model_key = self._generate_cache_key(model_class)
                # Convert parameters to a sorted, stringified representation for caching
        ...
        if param_name == "order_by":
            ...
            try:
                order_by_choices = [
                    name
                    for name in getattr(
                        model_class, "view_schema", model_class
                    ).model_fields.keys()
                ]
            except AttributeError:
