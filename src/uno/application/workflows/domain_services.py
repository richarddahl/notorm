...
class WorkflowDefService(UnoEntityService[WorkflowDef]):
    ...
    async def find_active_workflows(self) -> Result[List[WorkflowDef]]:
        ...
        try:
            ...
            return Failure(WorkflowServiceError(f"Error finding active workflows: {str(e)}"))
    
    async def get_workflow_with_relationships(self, workflow_id: str) -> Result[WorkflowDef]:
        """
        ...
        try:
            ...
            if self.trigger_repository:
                triggers_result = await self.trigger_repository.find_by_workflow(workflow_id)
            ...
            if self.condition_repository:
                conditions_result = await self.condition_repository.find_by_workflow(workflow_id)
            ...
            if self.action_repository:
                actions_result = await self.action_repository.find_by_workflow(workflow_id)
            ...
            if self.recipient_repository:
                recipients_result = await self.recipient_repository.find_by_workflow(workflow_id)
            ...
            if self.execution_repository:
                logs_result = await self.execution_repository.find_by_workflow(workflow_id)
    ...
    async def activate_workflow(self, workflow_id: str) -> Result[WorkflowDef]:
        ...
        try:
            ...
            workflow = workflow_result.value
            if not workflow:
                return Failure(WorkflowNotFoundError(f"Workflow {workflow_id} not found"))
    ...
    async def deactivate_workflow(self, workflow_id: str) -> Result[WorkflowDef]:
        ...
        try:
            ...
            return Failure(WorkflowServiceError(f"Error deactivating workflow: {str(e)}"))
...
class WorkflowTriggerService(UnoEntityService[WorkflowTrigger]):
    ...
    def __init__(
        ...
        super().__init__(WorkflowTrigger, repository, logger or logging.getLogger(__name__))
...
class WorkflowConditionService(UnoEntityService[WorkflowCondition]):
    ...
    def __init__(
        ...
        super().__init__(WorkflowCondition, repository, logger or logging.getLogger(__name__))
        self.workflow_service = workflow_service
    ...
    async def find_by_workflow(self, workflow_id: str) -> Result[List[WorkflowCondition]]:
        """
...
class WorkflowActionService(UnoEntityService[WorkflowAction]):
    ...
    def __init__(
        ...
        super().__init__(WorkflowAction, repository, logger or logging.getLogger(__name__))
        self.workflow_service = workflow_service
        ...
        try:
            ...
            if self.recipient_service:
                for action in actions:
                    recipients_result = await self.recipient_service.find_by_action(action.id)
                    if recipients_result.is_success:
    ...
    async def get_with_recipients(self, action_id: str) -> Result[WorkflowAction]:
        ...
        try:
            ...
            if self.recipient_service:
                recipients_result = await self.recipient_service.find_by_action(action_id)
                if recipients_result.is_success:
                    action.recipients = recipients_result.value
...
class WorkflowRecipientService(UnoEntityService[WorkflowRecipient]):
    ...
    def __init__(
        ...
        super().__init__(WorkflowRecipient, repository, logger or logging.getLogger(__name__))
        self.workflow_service = workflow_service
    ...
    async def find_by_workflow(self, workflow_id: str) -> Result[List[WorkflowRecipient]]:
        """
...
class WorkflowExecutionService(UnoEntityService[WorkflowExecutionRecord]):
    ...
    def __init__(
        ...
        super().__init__(WorkflowExecutionRecord, repository, logger or logging.getLogger(__name__))
        self.workflow_service = workflow_service
    ...
    async def create_execution_record(
        ...
        try:
            ...
            return Failure(WorkflowServiceError(f"Error creating execution record: {str(e)}"))
    ...
    async def update_execution_status(
        ...
        try:
            ...
            return Failure(WorkflowServiceError(f"Error updating execution status: {str(e)}"))
    
    async def find_by_workflow(self, workflow_id: str, limit: int = 100) -> Result[List[WorkflowExecutionRecord]]:
        """
        ...
        try:
            ...
            return Failure(WorkflowServiceError(f"Error finding execution records: {str(e)}"))