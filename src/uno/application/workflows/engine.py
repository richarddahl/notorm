...
class WorkflowExecutionActionError(WorkflowEngineError):
    ...
    def __init__(self, message: str, action_name: str | None = None, **context: Any):
        ...
        super().__init__(
            message=message, error_code=WorkflowErrorCode.WORKFLOW_ACTION_FAILED, **ctx
        )
...
class WorkflowEngine:
    ...
    def _register_default_handlers(self) -> None:
        ...
        self.register_condition_handler(
            WorkflowConditionType.FIELD_VALUE, self._handle_field_value_condition
        )
        self.register_condition_handler(
            WorkflowConditionType.TIME_BASED, self._handle_time_based_condition
        )
        self.register_condition_handler(
            WorkflowConditionType.ROLE_BASED, self._handle_role_based_condition
        )
        self.register_condition_handler(
            WorkflowConditionType.QUERY_MATCH, self._handle_query_match_condition
        )

        # Register action handlers
        self.register_action_handler(
            WorkflowActionType.NOTIFICATION, self._handle_notification_action
        )
        self.register_action_handler(
            WorkflowActionType.EMAIL, self._handle_email_action
        )
        self.register_action_handler(
            WorkflowActionType.WEBHOOK, self._handle_webhook_action
        )

        # Register recipient resolvers
        self.register_recipient_resolver(
            WorkflowRecipientType.USER, self._resolve_user_recipient
        )
        self.register_recipient_resolver(
            WorkflowRecipientType.ROLE, self._resolve_role_recipient
        )
        self.register_recipient_resolver(
            WorkflowRecipientType.GROUP, self._resolve_group_recipient
        )
    ...
    async def process_event(self, event: WorkflowEventModel) -> Result[Dict[str, Any]]:
        ...
        try:
            ...
            if not matching_workflows:
                self.logger.debug(
                    f"No matching workflows found for {event.table_name} {event.operation}"
                )
                return Success(
    ...
    async def _find_matching_workflows(
        ...
        async with self.db_manager.get_enhanced_session() as session:
            ...
            for match in matches:
                ...
                workflow = await self._get_workflow_with_components(
                    session, workflow_id
                )

                # Find the matching trigger
                matching_trigger = next(
                    (t for t in workflow.triggers if t.id == trigger_id), None
                )

                if matching_trigger and self._check_field_conditions(
                    matching_trigger, event
                ):
    ...
    def _check_field_conditions(
        self, trigger: WorkflowTrigger, event: WorkflowEventModel
    ) -> bool:
    ...
    async def _get_workflow_with_components(
        ...
        workflow_result = await session.execute(
            workflow_query, {"workflow_id": workflow_id}
        )
        ...
        triggers_result = await session.execute(
            triggers_query, {"workflow_id": workflow_id}
        )
        ...
        conditions_result = await session.execute(
            conditions_query, {"workflow_id": workflow_id}
        )
        ...
        actions_result = await session.execute(
            actions_query, {"workflow_id": workflow_id}
        )
        ...
        recipients_result = await session.execute(
            recipients_query, {"workflow_id": workflow_id}
        )
    ...
    async def _execute_workflow(
        ...
        try:
            ...
            conditions_result = await self._evaluate_conditions(
                workflow, event, execution_context
            )
    ...
    async def _create_execution_record(
        self, workflow_id: str, event: WorkflowEventModel
    ) -> WorkflowExecutionRecord:
    ...
    async def _evaluate_conditions(
        ...
        for condition in workflow.conditions:
            ...
            if evaluator:
                ...
                self.logger.debug(
                    f"Using advanced evaluator for condition {condition.id} ({condition.name or condition.condition_type})"
                )
                ...
                if not handler:
                    self.logger.warning(
                        f"No handler or evaluator registered for condition type: {condition.condition_type}"
                    )
                    continue
            ...
            if not result.value:
                ...
                self.logger.info(
                    f"Condition {condition.id} ({condition.name or condition.condition_type}) not met"
                )
    ...
    async def _execute_actions(
        ...
        from uno.workflows.executor import get_executor, ActionExecutionContext
        ...
        for action in workflow.actions:
            ...
            if executor:
                ...
                recipients_result = await self._resolve_recipients(
                    action, workflow, context
                )
                if recipients_result.is_failure:
                    self.logger.warning(
                        f"Failed to resolve recipients for action {action.id}: {recipients_result.error}"
                    )
                    recipients = []
                else:
                ...
                self.logger.info(
                    f"Executing action {action.id} ({action.name or action.action_type}) with executor"
                )
                action_result = await executor.execute(
                    action, execution_context, recipients
                )
                ...
                if not handler:
                    self.logger.warning(
                        f"No handler or executor registered for action type: {action.action_type}"
                    )
                    continue
                ...
                self.logger.info(
                    f"Executing action {action.id} ({action.name or action.action_type}) with legacy handler"
                )
                action_result = handler(action, event, context, recipients)

            if action_result.is_failure:
                self.logger.warning(
                    f"Action {action.id} ({action.name or action.action_type}) failed: {action_result.error}"
                )
    ...
    async def _resolve_recipients(
        ...
        for recipient in action.recipients:
            ...
            if resolver:
                ...
                self.logger.debug(
                    f"Using advanced resolver for recipient {recipient.id} ({recipient.recipient_type})"
                )
                result = await resolver.resolve(recipient, context)
            else:
                # Fall back to legacy resolver if no advanced resolver is available
                legacy_resolver = self._recipient_resolvers.get(
                    recipient.recipient_type
                )
                if not legacy_resolver:
                    self.logger.warning(
                        f"No resolver for recipient type: {recipient.recipient_type}"
                    )
                    continue
            ...
            if result.is_failure:
                self.logger.warning(
                    f"Failed to resolve recipient {recipient.id}: {result.error}"
                )
                continue
    ...
    def _handle_field_value_condition(
        ...
        try:
            ...
            if not config or not config.get("field"):
                return Failure(
                    WorkflowConditionError(
                        "Field value condition missing field configuration"
                    )
                )
            ...
            if operator == "eq":
                ...
                return Failure(
                    WorkflowConditionError(f"Unsupported operator: {operator}")
                )

        except Exception as e:
            return Failure(
                WorkflowConditionError(
                    f"Error evaluating field value condition: {str(e)}"
                )
            )
    ...
    async def _handle_query_match_condition(
        ...
        try:
            ...
            return Failure(
                WorkflowConditionError(
                    f"Error evaluating query match condition: {str(e)}"
                )
            )
    ...
    def _handle_notification_action(
        ...
        try:
            ...
            title = config.get(
                "title",
                f"Notification from {action.workflow.name if action.workflow else 'workflow'}",
            )
            ...
            self.logger.info(
                f"Recipients: {', '.join([r.username for r in recipients])}"
            )

            return Success(
                {"title": title, "message": message, "recipient_count": len(recipients)}
            )

        except Exception as e:
            return Failure(
                WorkflowActionError(f"Error executing notification action: {str(e)}")
            )
...
class WorkflowEventHandler(EventHandler):
    ...
    async def handle(self, event: UnoEvent) -> None:
...
class PostgresWorkflowEventListener:
    ...
    async def start(self) -> None:
        ...
        self.logger.info(
            f"Starting PostgreSQL workflow event listener on channel '{self.channel}'"
        )
        self._stop_requested = False
    ...
    async def _listen_for_events(self) -> None:
        ...
        async with self.db_manager.get_enhanced_session() as session:
            ...
            try:
                ...
                self.logger.info(
                    f"Listening for database events on channel '{self.channel}'"
                )
                ...
                while not self._stop_requested:
                    ...
                    try:
                        ...
                        self.logger.error(
                            f"Failed to parse notification payload: {message.payload}"
                        )
                    except Exception as e:
                ...
                self.logger.info(
                    "PostgreSQL workflow event listener task was cancelled"
                )
                raise
            except Exception as e:
                self.logger.exception(
                    f"Error in PostgreSQL workflow event listener: {e}"
                )
            finally:
                # Stop listening
                await conn.execute(f"UNLISTEN {self.channel};")
                self.logger.info(
                    f"Stopped listening for database events on channel '{self.channel}'"
                )
