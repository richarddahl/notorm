...
class JobManager:
    ...
    async def get_queue(self, queue_name: str) -> JobQueue:
        ...
        if queue_name not in self.queues:
            self.queues[queue_name] = JobQueue(
                queue_name=queue_name, storage=self.storage
            )
    ...
    async def cancel_job(self, job_id: str) -> Result[bool]:
        ...
        if job.status in [JobStatus.RUNNING, JobStatus.COMPLETED, JobStatus.FAILED]:
            return Result.failure(
                f"Job {job_id} cannot be cancelled in status {job.status.name}"
            )
    ...
    async def retry_job(self, job_id: str) -> Result[bool]:
        ...
        if job.status not in [JobStatus.FAILED, JobStatus.CANCELLED]:
            return Result.failure(
                f"Job {job_id} cannot be retried in status {job.status.name}"
            )
    ...
    async def get_failed_jobs(
        self, queue_name: str | None = None, limit: int = 100, offset: int = 0
    ) -> Result[list[Job]]:
        """Get failed jobs.
        ...
        return await self.storage.get_jobs_by_status(
            [JobStatus.FAILED], queue_name, limit, offset
        )
    ...
    async def get_running_jobs(
        ...
        """
        return await self.storage.get_jobs_by_status(
            [JobStatus.RUNNING], queue_name, limit, offset
        )
    ...
    async def get_pending_jobs(
        ...
        """
        return await self.storage.get_jobs_by_status(
            [JobStatus.PENDING], queue_name, limit, offset
        )
    ...
    async def get_completed_jobs(
        ...
        """
        return await self.storage.get_jobs_by_status(
            [JobStatus.COMPLETED], queue_name, limit, offset
        )
    ...
    async def _cleanup_loop(self) -> None:
        ...
        while self.running:
            try:
                ...
                result = await self.storage.mark_stalled_jobs_as_failed(
                    self.stall_timeout
                )
