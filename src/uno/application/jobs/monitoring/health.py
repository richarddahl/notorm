...
class JobQueueHealthCheck(HealthCheck):
    ...
    async def check_health(self) -> Dict[str, Any]:
        ...
        try:
            # Check queue length
            queue_length_result = await self.job_manager.get_queue_length(self.queue_name)
            ...
            # Check failed jobs
            failed_jobs_result = await self.job_manager.get_failed_jobs(self.queue_name, limit=0)
...
class JobSystemHealthChecker:
    ...
    def _setup_health_checks(self) -> None:
        ...
        queue_names_result = asyncio.get_event_loop().run_until_complete(queue_names_task)
    ...
    async def _check_loop(self) -> None:
        ...
        while self.running:
            try:
                ...
                try:
                    await asyncio.wait_for(self._stop_event.wait(), timeout=min_interval.total_seconds())
                ...
                for check in self.health_checks:
                    if not hasattr(check, 'last_check_time') or now - check.last_check_time >= check.check_interval:
                        ...
                        if result["status"] != HealthStatus.HEALTHY:
                            self.logger.warning(f"Health check {check.name} is {result['status']}: {result['message']}")
            except Exception as e:
    ...
    async def get_system_health(self) -> Dict[str, Any]:
        ...
        for check in self.health_checks:
            try:
                ...
                self.logger.exception(f"Error running health check {check.name}: {str(e)}")
                results[check.name] = {
                    "status": HealthStatus.UNHEALTHY,
