...
class JobMetrics:
    ...
    def record_job_failed(
        self, queue: str, task: str, worker: str, error_type: str
    ) -> None:
    ...
    def record_wait_time(
        self, queue: str, task: str, priority: str, seconds: float
    ) -> None:
            ...
            try:
                ...
                try:
                    await asyncio.wait_for(
                        ...
                        timeout=self.collection_interval.total_seconds(),
                    )
    ...
    async def _collect_queue_metrics(self) -> None:
        ...
        try:
            ...
            if not queue_names_result.is_success:
                self.logger.error(
                    f"Failed to get queue names: {queue_names_result.error}"
                )
                return
            ...
            for queue_name in queue_names:
                # Pending jobs
                pending_result = await self.job_manager.storage.get_queue_length(
                    queue_name
                )
                if pending_result.is_success:
                    self.metrics.set_queue_length(
                        queue_name, "pending", pending_result.value
                    )
                ...
                if running_result.is_success:
                    self.metrics.set_queue_length(
                        queue_name, "running", len(running_result.value)
                    )
                ...
                if failed_result.is_success:
                    self.metrics.set_queue_length(
                        queue_name, "failed", len(failed_result.value)
                    )
                ...
                if completed_result.is_success:
                    self.metrics.set_queue_length(
                        queue_name, "completed", len(completed_result.value)
                    )
        except Exception as e:
