...
class OneTimeSchedule(Schedule):
    ...
    def is_due(self, current_time: datetime) -> bool:
        ...
        return (
            current_time >= self.run_time and 
            (current_time - self.run_time).total_seconds() < 1
        )
...
class DailySchedule(Schedule):
        ...
        for t in self.times:
            try:
                ...
                if not (0 <= int(hour) <= 23 and 0 <= int(minute) <= 59):
                    raise ValueError(f"Invalid time: {t}. Must be in format 'HH:MM' (24-hour).")
            except Exception:
                raise ValueError(f"Invalid time: {t}. Must be in format 'HH:MM' (24-hour).")
    ...
    def get_next_n_run_times(self, after: datetime, n: int) -> List[datetime]:
        ...
        while len(result) < n:
            ...
            current = next_run + timedelta(minutes=1)  # Advance time to get the next run
    ...
    def __init__(
        ...
        for day in days:
            if isinstance(day, str):
                ...
                raise ValueError(f"Invalid day format: {day}. Must be a string or integer.")
        ...
        try:
            ...
            if not (0 <= int(hour) <= 23 and 0 <= int(minute) <= 59):
                raise ValueError(f"Invalid time: {time}. Must be in format 'HH:MM' (24-hour).")
        except Exception:
            raise ValueError(f"Invalid time: {time}. Must be in format 'HH:MM' (24-hour).")
...
class MonthlySchedule(Schedule):
    ...
    def __init__(
        ...
        """
        # Validate days
        self.days = sorted([int(day) for day in days])  # Sort days for consistent ordering
        for day in self.days:
            if not (1 <= day <= 31):
    ...
    def get_next_run_time(self, after: datetime) -> datetime:
        ...
        if remaining_days and (current_day < remaining_days[0] or 
                             (current_day == remaining_days[0] and current_time < self.time)):
...
class ScheduleTemplate:
    ...
    def create_schedule(self, schedule: Optional[Schedule] = None, **overrides: Any) -> Dict[str, Any]:
        ...
        if schedule is None and self.schedule is None:
            raise ValueError("No schedule provided and template has no default schedule")
