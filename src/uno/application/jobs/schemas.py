...
from uno.jobs.dtos import (
    ...
    PriorityEnum, StatusEnum, JobErrorDto, ScheduleIntervalDto
)
...
class ScheduleSchemaManager:
    ...
    def filter_params_to_query_params(self, params: ScheduleFilterParams) -> Dict[str, Any]:
        """Convert filter parameters to query parameters for the repository."""
        query_params = {}
...
class TaskSchemaManager:
    ...
    def task_to_dto(self, task: Dict[str, Any]) -> TaskInfoDto:
        ...
        timeout = None
        if "options" in task and "timeout" in task["options"] and task["options"]["timeout"] is not None:
            timeout = int(task["options"]["timeout"].total_seconds())
        
        retry_delay = 60
        if "options" in task and "retry_delay" in task["options"] and task["options"]["retry_delay"] is not None:
...
class QueueSchemaManager:
    ...
    def queues_to_list_dto(self, queues: List[Tuple[str, int, bool]]) -> QueueListDto:
        ...
        items = [self.queue_to_dto(name, size, is_paused) for name, size, is_paused in queues]
