...
class Worker(ABC):
        ...
        try:
            ...
            try:
                ...
                for mw in self.middleware:
                    modified_args, modified_kwargs = await mw.before_execution(
                ...
                for mw in task_middleware:
                    args, kwargs = await mw.before_task(
                        task_func, args, kwargs, job_context
                    )
                ...
                for mw in task_middleware:
                    result = await mw.after_task(
                        task_func, args, kwargs, result, job_context
                    )

                # Apply worker middleware for result
                for mw in self.middleware:
                    result = await mw.after_execution(
                        job, task_func, args, kwargs, result
                    )
    ...
    def _update_stats(
        self, success: bool, processing_time_ms: float, queue_wait_time_ms: float
    ) -> None:
