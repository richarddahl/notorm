...
class AsyncWorker(Worker):
    ...
    async def shutdown(self, wait: bool = True) -> None:
        ...
        for task in [self.main_task, self.heartbeat_task, 
                    self.metrics_task, self.healthcheck_task]:
    ...
    async def process_job(self, job: Job) -> None:
        ...
        if job.created_at and job.started_at:
            queue_wait_time_ms = (job.started_at - job.created_at).total_seconds() * 1000
        ...
        try:
            ...
            await self.queue.fail(job.id, {
                ...
                "message": str(e),
                "traceback": getattr(e, "__cause__", None) and 
                            getattr(e.__cause__, "__traceback__", None) and 
                            traceback.format_exception(None, e.__cause__, e.__cause__.__traceback__)
            }, retry=retry)
    ...
    async def _process_loop(self) -> None:
        ...
        while self.running:
            ...
            try:
                ...
                capacity = min(self.max_concurrent - self._stats["active_jobs"], self.batch_size)
                if capacity <= 0:
    ...
    async def check_health(self) -> Dict[str, Any]:
        ...
        if self.failure_count > self.max_failures * 0.8:
            ...
            health["warnings"] = [f"High failure count: {self.failure_count}/{self.max_failures}"]
    ...
    async def get_metrics(self) -> Dict[str, Any]:
        ...
        if metrics["jobs_processed_total"] > 0:
            metrics["error_rate"] = metrics["jobs_failed"] / metrics["jobs_processed_total"]
        else:
        ...
        if self._stats["processing_time_ms"]:
            ...
            if len(processing_times) >= 20:  # Only calculate percentiles with enough data
                metrics["processing_time_p95_ms"] = statistics.quantiles(processing_times, n=20)[18]  # 95th percentile
                metrics["processing_time_p99_ms"] = statistics.quantiles(processing_times, n=100)[98]  # 99th percentile
        ...
        if self._stats["queue_wait_time_ms"]:
            ...
            if len(wait_times) >= 20:  # Only calculate percentiles with enough data
                metrics["queue_wait_time_p95_ms"] = statistics.quantiles(wait_times, n=20)[18]  # 95th percentile
