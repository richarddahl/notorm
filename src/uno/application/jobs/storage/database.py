...
class DatabaseJobStorage(JobStorageProtocol):
    ...
    async def delete_job(self, job_id: str) -> Result[bool]:
        ...
        try:
            async with self._lock:
                ...
                delete_query = self.jobs_table.delete().where(self.jobs_table.c.id == job_id)
    ...
    async def enqueue_job(self, job: Job) -> Result[bool]:
        ...
        try:
            ...
            if not get_result.is_success:
                return Result.failure(f"Failed to check if job exists: {get_result.error}")
    ...
    async def get_jobs_by_status(
        ...
        try:
            ...
            query = select([self.jobs_table]).where(self.jobs_table.c.status.in_(status_values))
            
            if queue_name:
                query = query.where(self.jobs_table.c.queue_name == queue_name)
                
            query = query.order_by(self.jobs_table.c.updated_at.desc()).limit(limit).offset(offset)
            ...
            return Result.failure(f"Failed to get jobs by status from database: {str(e)}")
    ...
    async def add_schedule(self, schedule_def: ScheduleDefinition) -> Result[str]:
        ...
        try:
            async with self._lock:
                ...
                await session.execute(self.schedules_table.insert().values(**schedule_data))
                await session.commit()
    ...
    async def get_schedule(self, schedule_id: str) -> Result[Optional[ScheduleDefinition]]:
        ...
        try:
            ...
            query = select([self.schedules_table]).where(self.schedules_table.c.id == schedule_id)
            result = await session.execute(query)
            ...
            return Result.failure(f"Failed to retrieve schedule from database: {str(e)}")
    ...
    async def get_schedule_by_name(self, name: str) -> Result[Optional[ScheduleDefinition]]:
        ...
        try:
            ...
            query = select([self.schedules_table]).where(self.schedules_table.c.name == name)
            result = await session.execute(query)
            schedule_data = result.fetchone()
    ...
    async def delete_schedule(self, schedule_id: str) -> Result[bool]:
        ...
        try:
            async with self._lock:
                ...
                delete_query = self.schedules_table.delete().where(self.schedules_table.c.id == schedule_id)
                await session.execute(delete_query)
                await session.commit()
    ...
    async def get_due_schedules(self) -> Result[List[ScheduleDefinition]]:
        ...
        try:
            ...
            return Result.failure(f"Failed to get due schedules from database: {str(e)}")
    ...
    async def update_schedule_run_time(self, schedule_id: str, last_run: datetime, next_run: datetime) -> Result[bool]:
        ...
        try:
            ...
            return Result.failure(f"Failed to update schedule run times in database: {str(e)}")
    ...
    async def get_all_schedules(self) -> Result[List[ScheduleDefinition]]:
        ...
        try:
            ...
            return Result.failure(f"Failed to get all schedules from database: {str(e)}")
    ...
    async def mark_stalled_jobs_as_failed(self, stall_timeout: timedelta) -> Result[int]:
        ...
        try:
            ...
            return Result.failure(f"Failed to mark stalled jobs as failed in database: {str(e)}")
    ...
    def _job_from_row(self, row: Dict[str, Any]) -> Job:
        ...
        retry_delay = timedelta(seconds=row['retry_delay']) if row['retry_delay'] is not None else None
        timeout = timedelta(seconds=row['timeout']) if row['timeout'] is not None else None
