...
class RedisJobStorage(JobStorageProtocol):
    ...
    def __init__(
        self, redis_url: str = "redis://localhost:6379/0", prefix: str = "uno:jobs"
    ):
    ...
    def _deserialize_job(self, data: dict) -> Job:
        ...
        retry_delay = (
            timedelta(seconds=float(data["retry_delay"]))
            if data.get("retry_delay")
            else None
        )
        timeout = (
            timedelta(seconds=float(data["timeout"])) if data.get("timeout") else None
        )
        is_scheduled = (
            bool(int(data["is_scheduled"])) if "is_scheduled" in data else False
        )
    ...
    def _serialize_schedule(self, schedule_def: ScheduleDefinition) -> dict:
        ...
        return {
            ...
            "timeout": (
                schedule_def.timeout.total_seconds() if schedule_def.timeout else None
            ),
            "created_at": (
                schedule_def.created_at.isoformat()
                if schedule_def.created_at
                else datetime.now(datetime.UTC).isoformat()
            ),
            "updated_at": (
                schedule_def.updated_at.isoformat()
                if schedule_def.updated_at
                else datetime.now(datetime.UTC).isoformat()
            ),
            "last_run_at": (
                schedule_def.last_run_at.isoformat()
                if schedule_def.last_run_at
                else None
            ),
            "next_run_at": (
                schedule_def.next_run_at.isoformat()
                if schedule_def.next_run_at
                else None
            ),
        ...
        try:
            ...
            async with self._lock:
                ...
                if job.status == JobStatus.PENDING:
                    ...
                    )  # Use created_at timestamp as score for FIFO
        ...
        try:
            ...
            if not current_job_result.is_success:
                return Result.failure(
                    f"Failed to get current job state: {current_job_result.error}"
                )
    ...
    async def delete_job(self, job_id: str) -> Result[bool]:
        ...
        try:
            ...
            if not job_result.is_success:
                return Result.failure(
                    f"Failed to get job for deletion: {job_result.error}"
                )
    ...
    async def enqueue_job(self, job: Job) -> Result[bool]:
        ...
        try:
            ...
            if not job_exists_result.is_success:
                return Result.failure(
                    f"Failed to check if job exists: {job_exists_result.error}"
                )
            ...
            for priority in sorted(Priority, key=lambda p: p.value):
                ...
                async with self._lock:
                    job_ids_with_scores = await self._redis.zrangebyscore(
                        queue_key, min="-inf", max=now, start=0, num=1, withscores=True
                    )
    ...
    async def get_jobs_by_status(
        ...
        try:
            ...
            # Sort by updated_at
            all_matching_jobs.sort(
                key=lambda j: j.updated_at or datetime.min, reverse=True
            )
    ...
    async def get_schedule_by_name(
        ...
        try:
            ...
            return Result.failure(
                f"Failed to get schedule by name from Redis: {str(e)}"
            )
    ...
    async def update_schedule(self, schedule_def: ScheduleDefinition) -> Result[bool]:
        ...
        try:
            ...
            if not current_schedule_result.is_success:
                return Result.failure(
                    f"Failed to get current schedule: {current_schedule_result.error}"
                )
            ...
            if (
                schedule_def.schedule.schedule_type
                != current_schedule.schedule.schedule_type
                or schedule_def.schedule.to_dict()
                != current_schedule.schedule.to_dict()
                or schedule_def.next_run_at is None
            ):
                schedule_def.next_run_at = schedule_def.schedule.next_run_time(
            ...
            async with self._lock:
                ...
                if not schedule_def.enabled or (
                    current_schedule.enabled and not schedule_def.enabled
                ):
    ...
    async def delete_schedule(self, schedule_id: str) -> Result[bool]:
        ...
        try:
            ...
            if not schedule_result.is_success:
                return Result.failure(
                    f"Failed to get schedule for deletion: {schedule_result.error}"
                )
            ...
            if schedule is None:
                return Result.success(
                    True
                )  # Schedule doesn't exist, consider it deleted
    ...
    async def update_schedule_run_time(
        ...
        try:
            ...
            if not schedule_result.is_success:
                return Result.failure(
                    f"Failed to get schedule: {schedule_result.error}"
                )
            ...
            return Result.failure(
                f"Failed to update schedule run times in Redis: {str(e)}"
            )
                ...
                for priority in Priority:
                    ...
                    if job_ids:
                        ...
                        for job_id in job_ids:
                            ...
                            pipeline.srem(
                                self._get_status_index_key(JobStatus.PENDING), job_id
                            )

                            # Add to CANCELLED status set
                            pipeline.sadd(
                                self._get_status_index_key(JobStatus.CANCELLED), job_id
                            )
                ...
                for priority in Priority:
                    ...
                    if job_ids:
                        ...
                        for job_id in job_ids:
                            ...
                            pipeline.sadd(
                                self._get_status_index_key(JobStatus.PAUSED), job_id
                            )
        ...
        try:
            ...
            async with self._lock:
                ...
                for priority in Priority:
                    ...
                    paused_jobs = await self._redis.zrange(
                        temp_key, 0, -1, withscores=True
                    )
    ...
    async def mark_stalled_jobs_as_failed(
        ...
        try:
            ...
            return Result.failure(
                f"Failed to mark stalled jobs as failed in Redis: {str(e)}"
            )
