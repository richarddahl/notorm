...
class ValueService(ValueServiceProtocol):
    ...
    async def create_value(
        ...
        try:
            ...
            if validation_result.is_failure:
                return Failure(
                    ValueValidationError(
                        reason=str(validation_result.error), value=value
                    )
                )

            # Create value object
            value_obj = value_type(value=value, name=name or str(value))
    ...
    async def get_value_by_id(
        self, value_type: Type[ValueObj], value_id: str
        ...
        try:
            ...
            if value_obj is None:
                return Failure(
                    ValueNotFoundError(
                        value_id=value_id, value_type=value_type.__name__
                    )
                )
            ...
            self.logger.error(
                f"Error getting {value_type.__name__} by ID {value_id}: {e}"
            )
            return Failure(
                ValueServiceError(
                    reason=str(e),
    ...
    async def create_attachment(self, file_path: str, name: str) -> Result[Attachment]:
        """
        ...
        try:
            ...
            attachment = Attachment(file_path=file_path, name=name)
            ...
            return Failure(
                ValueServiceError(
                    reason=str(e), operation="create_attachment", file_path=file_path
                )
            )

    async def validate_value(
        self, value_type: Type[ValueObj], value: ValueType
    ) -> Result[bool]:
        """
        ...
        try:
            ...
            if value_type == BooleanValue and isinstance(value, bool):
                ...
                valid_type = True
            elif value_type == DecimalValue and (
                isinstance(value, decimal.Decimal) or isinstance(value, float)
            ):
                valid_type = True
            elif (
            ...
            if not valid_type:
                ...
                return Failure(
                    ValueTypeMismatchError(
                        expected_type=expected_type, actual_type=type(value).__name__
                    )
                )
            ...
            return Failure(ValueValidationError(reason=str(e), value=value))

    async def convert_value(
        self, value: Any, target_type: Type[ValueObj]
    ) -> Result[ValueType]:
        """
