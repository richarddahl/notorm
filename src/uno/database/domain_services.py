...
from typing import Protocol, Generic, TypeVar, Dict, List, Any, Optional, Union, Type, AsyncIterator, Iterator, Tuple, cast
import logging
import time
...
from uno.database.entities import (
    DatabaseId, ConnectionConfig, Transaction, TransactionId, TransactionIsolationLevel,
    QueryStatistics, QueryId, QueryPlan, IndexRecommendation, IndexType,
    ConnectionPoolConfig, PoolStatistics, CachedResult, CacheKey, CacheConfig, CacheStrategy,
    OptimizationConfig, OptimizationLevel, QueryComplexity, QueryRewrite, OptimizerMetricsSnapshot,
...
class QueryOptimizerServiceProtocol(Protocol):
    ...
    async def get_optimizer_metrics(self) -> UnoResult[OptimizerMetricsSnapshot, ErrorDetails]:
        """
    ...
    async def create_database(
        ...
        try:
            ...
            try:
                ...
                if config.db_schema:
                    await db_conn.execute(f"CREATE SCHEMA IF NOT EXISTS {config.db_schema}")
...
class QueryExecutionService:
    ...
    async def execute_query(
        ...
        if request.use_cache and self.cache_service:
            ...
            if isinstance(cached_result, Success) and cached_result.value is not None:
                ...
                return Success(QueryResponse(
                    ...
                    row_count=len(cached_result.value) if isinstance(cached_result.value, list) else 1,
        ...
        if request.optimize and self.optimizer_service:
            ...
            optimization_result = await self.optimizer_service.optimize_query(optimization_request)
            
            if isinstance(optimization_result, Success):
                # Use optimized query if available
                optimized_query = optimization_result.value.optimized_query or request.query
                query_plan = optimization_result.value.plan_after
        
        try:
            # Execute the query
            result = await self.session_repository.fetch_all(optimized_query, request.parameters)
...
class QueryOptimizerService:
    ...
    def __init__(
        ...
        recommendation_repository: Optional[IndexRecommendationRepositoryProtocol] = None,
    ...
    async def optimize_query(
        ...
        if request.level != OptimizationLevel.NONE:
            ...
            if request.level in [OptimizationLevel.AGGRESSIVE, OptimizationLevel.MAXIMUM]:
        ...
        if request.include_plan and response.optimized_query:
            plan_result = await self.analyze_query(response.optimized_query, request.parameters)
            ...
            if isinstance(plan_result, Success):
                ...
                if response.plan_before and response.plan_after:
                    if response.plan_before.estimated_cost > 0:
                        improvement = (response.plan_before.estimated_cost - response.plan_after.estimated_cost) / response.plan_before.estimated_cost
                        estimated_improvement = max(0, improvement * 100)  # as percentage
        ...
        if request.generate_recommendations and self.recommendation_repository:
            ...
            for table in tables:
                recommendation = self._generate_index_recommendation(table, request.query)
    ...
    async def analyze_query(
        ...
        try:
            ...
            if not explain_output or not isinstance(explain_output, dict):
                return Failure(ErrorDetails(
                    code="INVALID_EXPLAIN_OUTPUT",
                    message="Invalid EXPLAIN output"
                ))
    ...
    async def get_index_recommendations(
        ...
        result = await self.recommendation_repository.list_recommendations(table_name, limit)
        return result
    ...
    def _generate_index_recommendation(self, table_name: str, query: str) -> Optional[IndexRecommendation]:
        """
        ...
        where_pattern = rf"where\s+([a-zA-Z0-9_\s\.=<>!]+{table_name}[a-zA-Z0-9_\s\.=<>!]+)"
...
class QueryCacheService:
    ...
    async def get_cache_statistics(self) -> UnoResult[Dict[str, Any], ErrorDetails]:
        ...
        try:
            ...
            if isinstance(size_result, Failure):
                self.logger.error(f"Error getting cache size: {size_result.error.message}")
    ...
    def _determine_query_complexity(self, query: str) -> QueryComplexity:
        ...
        has_aggregation = any(keyword in lower_query for keyword in ["group by", "having", "count(", "sum(", "avg(", "min(", "max("])
        ...
        if has_window_functions or (has_subqueries and has_aggregation) or join_count >= 3:
...
class TransactionService:
    ...
    async def transaction_context(
        ...
        Yields:
            A transaction object
        """
        async with self.transaction_repository.transaction(isolation_level, read_only) as transaction:
            yield transaction
...
class ConnectionPoolService:
    ...
    async def get_pool_statistics(self) -> UnoResult[PoolStatistics, ErrorDetails]:
        ...
        try:
            if not self.engine_factory or not hasattr(self.engine_factory, "engine"):
                return Failure(ErrorDetails(
                    code="NO_ENGINE",
                    message="No SQLAlchemy engine available"
                ))
            ...
            if not hasattr(engine, "pool"):
                return Failure(ErrorDetails(
                    code="NO_POOL",
                    message="Engine does not have a connection pool"
                ))
    ...
    async def reset_pool(self) -> UnoResult[None, ErrorDetails]:
        ...
        try:
            ...
            return Failure(ErrorDetails(
                code="POOL_RESET_ERROR",
                message=f"Error resetting pool: {str(e)}"
            ))