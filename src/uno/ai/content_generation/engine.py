...
class ContentEngine:
    ...
    async def summarize(
        ...
        result = {
            ...
            "reduction_ratio": len(processed_content) / len(text) if len(text) > 0 else 0
        }
    ...
    async def _retrieve_context(
        ...
        if strategy == RAGStrategy.VECTOR_ONLY:
            ...
            relationship_keywords = ["related", "connection", "linked", "associated", "between"]
            has_relationship_focus = any(keyword in query.lower() for keyword in relationship_keywords)
        ...
        try:
            ...
            for row in rows:
                ...
                results.append({
                    ...
                    "entity_type": entity_type,
                    "content": content,
                    "metadata": {k: v for k, v in graph_node.items() if k not in ["id", "type", "content"]},
                    "source": "graph"
                })
    ...
    def _format_prompt(
        ...
        formatted_prompt += "You are an AI assistant that generates high-quality content. "
        ...
        if context:
            ...
            for i, item in enumerate(context):
                ...
                formatted_prompt += f"[{i+1}] Source: {entity_id}\nContent: {content}\n\n"
