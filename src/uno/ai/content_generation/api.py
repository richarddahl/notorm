...
from uno.ai.content_generation.engine import (
    ContentEngine, ContentType, ContentMode, ContentFormat, RAGStrategy
)
...
class IndexContentRequest(BaseModel):
    ...
    graph_nodes: Optional[List[Dict[str, Any]]] = Field(
        default=None, 
        description="Additional nodes to add to the graph"
    )
...
class GenerateContentRequest(BaseModel):
    ...
    context_entity_ids: Optional[List[str]] = Field(
        default=None, 
        description="Optional specific entity IDs to include as context"
    )
    context_entity_types: Optional[List[str]] = Field(
        default=None, 
        description="Optional entity types to search for context"
    )
    rag_strategy: Optional[RAGStrategy] = Field(
        default=None, 
        description="Strategy for retrieval (overrides default)"
    )
...
class SummarizeRequest(BaseModel):
    ...
    bullet_points: bool = Field(
        default=False, 
        description="Whether to generate bullet points"
    )
...
class ContentResponse(BaseModel):
    ...
    context_count: Optional[int] = Field(
        default=None, 
        description="Number of context items used"
    )
    context_sources: Optional[List[str]] = Field(
        default=None, 
        description="Sources of context used"
    )
...
class SummaryResponse(BaseModel):
    ...
    reduction_ratio: float = Field(..., description="Ratio of summary to original length")
