...
class ConstructionPipeline(BaseModel):
    ...
    relationship_extraction_method: RelationshipExtractionMethod = (
        RelationshipExtractionMethod.DEPENDENCY_PARSING
    )
...
class KnowledgeConstructor:
        ...
        try:
            ...
            async with self.pool.acquire() as conn:
                ...
                try:
                    ...
                    self.logger.info(
                        f"Graph {self.config.graph_name} created or already exists"
                    )
                except Exception as e:
    ...
    def _initialize_nlp_components(self) -> None:
        ...
        if EntityExtractionMethod.SPACY_NER in [
            self.config.default_pipeline.entity_extraction_method
        ] or RelationshipExtractionMethod.DEPENDENCY_PARSING in [
            self.config.default_pipeline.relationship_extraction_method
        ]:
            if SPACY_AVAILABLE:
                try:
                    import spacy
                ...
                self.logger.warning(
                    "SpaCy is not available. Some extraction methods will not work."
                )

        # Initialize Transformers if available and needed
        if EntityExtractionMethod.TRANSFORMER_NER in [
            self.config.default_pipeline.entity_extraction_method
        ] or RelationshipExtractionMethod.TRANSFORMER_RE in [
            self.config.default_pipeline.relationship_extraction_method
        ]:
            if TRANSFORMERS_AVAILABLE:
                try:
                    ...
                    self.logger.info(
                        f"Loading Transformer NER model: {self.config.transformer_model}"
                    )
                ...
                self.logger.warning(
                    "Transformers are not available. Some extraction methods will not work."
                )
    ...
    async def construct_knowledge_graph(
        ...
        try:
            ...
            if self.config.deduplication_enabled:
                all_entities = self._deduplicate_entities(
                    all_entities, pipeline.similarity_threshold
                )
            ...
            result = ConstructionResult(
                ...
                metadata={
                    ...
                    "relationship_types": list(
                        set(rel.type for rel in all_relationships)
                    ),
    ...
    async def extract_knowledge(
        self, text_source: TextSource, pipeline: Optional[ConstructionPipeline] = None
    ) -> ExtractionResult:
        """
        ...
        try:
            ...
            self.logger.error(
                f"Failed to extract knowledge from text source {text_source.id}: {e}"
            )
            return ExtractionResult(
                source_id=text_source.id, metadata={"error": str(e)}
            )

    async def _extract_entities(
        self, text_source: TextSource, pipeline: ConstructionPipeline
    ) -> list[Entity]:
        """
        ...
        if pipeline.entity_extraction_method == EntityExtractionMethod.RULE_BASED:
            ...
            if self.spacy_nlp:
                ...
                self.logger.warning(
                    "SpaCy NLP not available, falling back to rule-based extraction"
                )
                entities = self._extract_entities_rule_based(text_source)

        elif (
            pipeline.entity_extraction_method == EntityExtractionMethod.TRANSFORMER_NER
        ):
            if self.transformer_ner:
                entities = self._extract_entities_transformer(text_source)
            else:
                self.logger.warning(
                    "Transformer NER not available, falling back to rule-based extraction"
                )
                entities = self._extract_entities_rule_based(text_source)

        # Filter by confidence threshold
        entities = [
            e for e in entities if e.confidence >= pipeline.entity_confidence_threshold
        ]
        ...
        try:
            ...
            for token in ner_results:
                ...
                # Continuation of an entity
                    ...
                    entity = entities[-1]
                    entity.text += " " + token["word"].lstrip(
                        "##"
                    )  # Handle BERT tokenization
                    entity.end_char = token["end"]
                    entity.confidence = (
                        entity.confidence + token["score"]
    ...
    async def _extract_relationships(
        ...
        # Select extraction method
        if (
            pipeline.relationship_extraction_method
            == RelationshipExtractionMethod.PATTERN_BASED
        ):
            relationships = self._extract_relationships_pattern_based(
            ...
            if self.spacy_nlp:
                relationships = self._extract_relationships_dependency_parsing(
                    text_source, entities
                )
            else:
                self.logger.warning(
                    "SpaCy NLP not available, falling back to pattern-based extraction"
                )
                relationships = self._extract_relationships_pattern_based(
                    text_source, entities
                )
            ...
            pipeline.relationship_extraction_method
            == RelationshipExtractionMethod.TRANSFORMER_RE
        ):
            if self.transformer_re:
                relationships = self._extract_relationships_transformer(
                    text_source, entities
                )
            else:
                self.logger.warning(
                    "Transformer RE not available, falling back to pattern-based extraction"
                )
                relationships = self._extract_relationships_pattern_based(
                    text_source, entities
                )
        ...
        relationships = [
            ...
            if r.confidence >= pipeline.relationship_confidence_threshold
        ]

        # Filter by relationship types if specified
        if pipeline.relationship_types:
            relationships = [
                r for r in relationships if r.type in pipeline.relationship_types
            ]

        return relationships

    def _extract_relationships_pattern_based(
        self, text_source: TextSource, entities: list[Entity]
    ) -> list[Relationship]:
    ...
    def _extract_relationships_dependency_parsing(
        ...
        for sent in doc.sents:
                            ...
                            if token in ent:
                                ...
                                for entity in entities:
                                    if (
                                        ...
                                        and entity.end_char >= ent.end_char
                                    ):
                                        subjects.append(entity)
                                        break
                                break

                elif token.dep_ in ("dobj", "pobj") and (
            ...
            for subject in subjects:
                for obj in objects:
                    relationship_id = (
                        f"{text_source.id}_relationship_{relationship_id_counter}"
                    )
                    relationship_id_counter += 1
    ...
    def _extract_relationships_transformer(
        ...
        self.logger.warning(
            "Transformer-based relationship extraction not fully implemented"
        )
        return []
    ...
    def _deduplicate_entities(
        self, entities: list[Entity], similarity_threshold: float
    ) -> list[Entity]:
        """
                ...
                for entity in entities:
                    ...
                    try:
                        node_id = await conn.fetchval(
                            f"SELECT * FROM cypher('{self.config.graph_name}', $1, $2, $3) as (id agtype);",
                            cypher,
                            {},
                            True,
                        )
        ...
        try:
            async with self.pool.acquire() as conn:
                ...
                result = await conn.fetch(
                    f"SELECT * FROM cypher('{self.config.graph_name}', $1, $2, $3) as (result agtype);",
                    cypher_query,
                    {},
                    True,
                )
