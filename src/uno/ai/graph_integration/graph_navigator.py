...
class GraphNavigator:
        ...
        try:
            ...
            async with self.pool.acquire() as conn:
                ...
                result = await conn.fetchval(
                    f"SELECT * FROM cypher('{self.config.graph_name}', $1, $2, $3) as (path agtype);",
                    cypher,
                    {},
                    True,
                )
                ...
                path_result = PathResult(
                    ...
                    score=1.0,
                    metadata={"traversal_mode": traversal_mode, "max_depth": max_depth},
                )
            ...
            async with self.pool.acquire() as conn:
                ...
                for row in result:
                    ...
                    path_result = PathResult(
                        ...
                        length=len(relationships),
                        score=1.0
                        / (len(relationships) + 1),  # Shorter paths score higher
                        metadata={
    ...
    async def extract_subgraph(
        ...
        try:
            ...
            async with self.pool.acquire() as conn:
                ...
                # Execute Cypher query
                result = await conn.fetchval(
                    f"SELECT * FROM cypher('{self.config.graph_name}', $1, $2, $3) as (subgraph agtype);",
                    cypher,
                    {},
                    True,
                )
                ...
                if not center_node:
                    self.logger.error(
                        f"Center node {center_node_id} not found in subgraph result"
                    )
                    return None
    ...
    async def find_similar_nodes(
        ...
        try:
            ...
            async with self.pool.acquire() as conn:
                ...
                # Execute Cypher query
                result = await conn.fetch(
                    f"SELECT * FROM cypher('{self.config.graph_name}', $1, $2, $3) as (node agtype, similarity agtype);",
                    cypher,
                    {},
                    True,
                )
        ...
        try:
            ...
            if reasoning_type == "causal":
                ...
                cypher = self._build_shortest_path_query(
                    ...
                    rel_types,
                    max_depth,
                    self.config.default_node_filter,
                    self.config.default_path_constraint,
                )
                ...
                path_result = PathResult(
                    ...
                    metadata={
                        ...
                        "explanation": self._generate_path_explanation(
                            nodes, relationships, reasoning_type
                        ),
                    },
                )
    ...
    async def detect_communities(
        ...
        try:
            ...
            async with self.pool.acquire() as conn:
                ...
                # Execute Cypher query
                result = await conn.fetch(
                    f"SELECT * FROM cypher('{self.config.graph_name}', $1, $2, $3) as (community agtype);",
                    cypher,
                    {},
                    True,
                )
        ...
        try:
                ...
                if strategy == "path":
                    ...
                    for i in range(len(relevant_nodes)):
                        for j in range(i + 1, len(relevant_nodes)):
                            ...
                            path = await self.find_shortest_path(
                                start_node_id=start_id, end_node_id=end_id, max_depth=3
                            )
                    ...
                    for node_id in relevant_nodes:
                        subgraph = await self.extract_subgraph(
                            center_node_id=node_id, max_depth=2, max_nodes=10
                        )
                    ...
                    for node_id in relevant_nodes[:2]:
                        subgraph = await self.extract_subgraph(
                            center_node_id=node_id, max_depth=1, max_nodes=5
                        )
            ...
            # If we have query embedding, use it to find semantically similar nodes
                ...
                async with self.pool.acquire() as conn:
                    ...
                    result = await conn.fetch(
                        f"SELECT * FROM cypher('{self.config.graph_name}', $1, $2, $3) as (node agtype);",
                        cypher,
                        {},
                        True,
                    )
                    ...
                    if len(relevant_node_ids) > 0:
                        ...
                        subgraph = await self.extract_subgraph(
                            center_node_id=start_id, max_depth=2, max_nodes=10
                        )
    ...
    def _generate_path_explanation(self, nodes, relationships, reasoning_type):
        ...
        if reasoning_type == "causal":
            ...
            for i, rel in enumerate(relationships):
                ...
                end_node = nodes[i + 1]
                explanation += (
                    f"- {start_node.get('properties', {}).get('name', start_node.get('id'))} "
                    + f"{rel.get('label')} "
    ...
    def _format_path_for_rag(self, path):
        ...
        text = (
            f"Path from {nodes[0].get('properties', {}).get('name', nodes[0].get('id'))} "
    ...
    def _build_shortest_path_query(
        self, start_id, end_id, rel_types, max_depth, node_filter, path_constraint
    ):
        ...
        if node_filter:
            if node_filter.labels:
                ...
                node_where.append(
                    f"ALL(label IN LABELS(node) WHERE label IN ['{node_labels}'])"
                )

            if node_filter.exclude_labels:
                exclude_labels = "|".join(node_filter.exclude_labels)
                node_where.append(
                    f"NONE(label IN LABELS(node) WHERE label IN ['{exclude_labels}'])"
                )
        ...
        if path_constraint:
            ...
            if path_constraint.required_nodes:
                req_nodes = ", ".join(
                    [f"'{node}'" for node in path_constraint.required_nodes]
                )
                path_where.append(
                    f"ALL(node IN [{req_nodes}] WHERE node IN NODES(path))"
                )

            if path_constraint.excluded_nodes:
                excl_nodes = ", ".join(
                    [f"'{node}'" for node in path_constraint.excluded_nodes]
                )
                path_where.append(
                    f"NONE(node IN [{excl_nodes}] WHERE node IN NODES(path))"
                )

            if path_constraint.required_relationships:
                req_rels = ", ".join(
                    [f"'{rel}'" for rel in path_constraint.required_relationships]
                )
                path_where.append(
                    f"ANY(rel IN RELATIONSHIPS(path) WHERE TYPE(rel) IN [{req_rels}])"
                )

            if path_constraint.excluded_relationships:
                excl_rels = ", ".join(
                    [f"'{rel}'" for rel in path_constraint.excluded_relationships]
                )
                path_where.append(
                    f"NONE(rel IN RELATIONSHIPS(path) WHERE TYPE(rel) IN [{excl_rels}])"
                )
    ...
    def _build_bidirectional_query(
        self, start_id, end_id, rel_types, max_depth, node_filter, path_constraint
    ):
    ...
    def _build_neighborhood_query(
        self, center_id, max_depth, rel_types, node_filter, max_nodes
    ):
        ...
        if node_filter:
            if node_filter.labels:
                node_labels = "|".join(node_filter.labels)
                node_where.append(
                    f"ANY(label IN LABELS(b) WHERE label IN ['{node_labels}'])"
                )

            if node_filter.exclude_labels:
                exclude_labels = "|".join(node_filter.exclude_labels)
                node_where.append(
                    f"NONE(label IN LABELS(b) WHERE label IN ['{exclude_labels}'])"
                )
    ...
    def _build_pagerank_subgraph_query(
        self, center_id, max_depth, rel_types, node_filter, max_nodes
    ):
    ...
    def _build_common_neighbors_query(
        self, node_id, top_k, min_similarity, rel_types, node_filter
    ):
    ...
    def _build_jaccard_similarity_query(
        self, node_id, top_k, min_similarity, rel_types, node_filter
    ):
    ...
    def _build_hierarchical_reasoning_query(
        self, start_id, end_id, max_depth, rel_types
    ):
