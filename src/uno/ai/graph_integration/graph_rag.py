...
from uno.ai.graph_integration.graph_navigator import GraphNavigator, PathResult, SubgraphResult
...
class GraphRAGService(Generic[T]):
    ...
    def _format_path_for_context(self, path: PathResult) -> str:
        ...
        start_name = nodes[0].get('properties', {}).get('name', nodes[0].get('id', 'Unknown'))
        end_name = nodes[-1].get('properties', {}).get('name', nodes[-1].get('id', 'Unknown'))
        ...
        for i, rel in enumerate(relationships):
            ...
            start_name = start_node.get('properties', {}).get('name', start_node.get('id', 'Unknown'))
            end_name = end_node.get('properties', {}).get('name', end_node.get('id', 'Unknown'))
    ...
    def _format_subgraph_for_context(self, subgraph: SubgraphResult) -> str:
        ...
        center_name = center_node.get('properties', {}).get('name', center_node.get('id', 'Unknown'))
...
async def create_graph_rag_service(
    ...
    from uno.ai.graph_integration.knowledge_constructor import create_knowledge_constructor
