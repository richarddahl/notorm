...
class IntelligentRecommendationService:
    ...
    async def get_recommendations(
        ...
        recommendation_set = RecommendationSet(
            ...
            metadata={
                "sources": [source.value for source in request.sources] if request.sources else ["all"],
    ...
    async def get_similar_items(
            ...
            try:
                async with self.pool.acquire() as conn:
                    ...
                    rows = await conn.fetch("""
                        ...
                        LIMIT $5
                    """, embedding_str, item_id, item_type, min_similarity, limit)
        ...
        if len(similar_items) < limit and self.embedding_service:
            for key, item in self.active_items.items():
                ...
                if any(result[0]["item_id"] == item["item_id"] and 
                       result[0]["item_type"] == item["item_type"] 
    ...
    async def _collect_context(self, request: RecommendationRequest) -> Dict[str, Any]:
        ...
        if self.embedding_service:
            ...
            for item in context["user_context"] + context["session_context"] + \
                       context["entity_context"] + context["specific_context"]:
    ...
    async def _generate_candidates(
        ...
        if RecommendationSource.BEHAVIOR in sources and request.user_id:
            behavior_candidates = await self._generate_behavior_candidates(request, context)
        ...
        if RecommendationSource.SIMILARITY in sources:
            similarity_candidates = await self._generate_similarity_candidates(request, context)
        ...
        if RecommendationSource.POPULARITY in sources:
            popularity_candidates = await self._generate_popularity_candidates(request, context)
        ...
        if RecommendationSource.COMPLEMENTARY in sources:
            complementary_candidates = await self._generate_complementary_candidates(request, context)
                        ...
                        item = RecommendationItem(
                            ...
                            reasons=[
                                RecommendationReason(
                                    ...
                                    confidence=min(0.6 + (row["user_count"] / 20.0), 0.95)
                                )
                            ]
                        )
    ...
    async def _generate_similarity_candidates(
        ...
        if not query_embeddings:
            for item in context["user_context"] + context["session_context"]:
                if (item.type == ContextType.QUERY or 
                    item.source == ContextSource.SEARCH or
                    item.source == ContextSource.CONTENT_GENERATION):
                    if item.id in context_embeddings:
                        query_embeddings.append(context_embeddings[item.id])
                        ...
                        item = RecommendationItem(
                            ...
                            reasons=[
                                RecommendationReason(
                                    ...
                                    confidence=min(0.5 + (row["user_count"] / 100.0), 0.9)
                                )
                            ]
                        )
    ...
    async def _generate_complementary_candidates(
        ...
        recent_items = set()
        for context_list in [context["specific_context"], context["session_context"], context["user_context"]]:
    ...
    async def _generate_expert_candidates(
        ...
        for context_list in [context["specific_context"], context["session_context"], context["user_context"]]:
            for item in context_list:
                if item.source == ContextSource.SEARCH and item.type == ContextType.QUERY:
                    ...
                    context_text += f"- Interaction with {item.entity_type}: {item.entity_id}\n"
        ...
        try:
            ...
            try:
                ...
                self.logger.error(f"Failed to parse expert recommendations: {e}\nResponse: {response}")
                ...
                "items": [
                    {
                        ...
                        "source": item.reasons[0].source.value if item.reasons else "unknown"
                    }
    ...
    async def _rank_candidates(
        ...
        for candidate in candidates:
            ...
            similarity_boost = candidate.similarity_score * 0.3 if candidate.similarity_score else 0
            ...
            # Combine scores
            candidate.metadata["rank_score"] = base_score + priority_boost + similarity_boost + confidence_boost
        
        # Sort by rank score (descending)
        ranked_candidates = sorted(
            candidates,
            key=lambda x: x.metadata.get("rank_score", 0),
            reverse=True
        )
    ...
    async def _generate_explanations(
        ...
        try:
            ...
            try:
                ...
                self.logger.error(f"Failed to parse explanations: {e}\nResponse: {response}")
