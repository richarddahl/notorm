...
class SharedEmbeddingService:
    ...
    async def embed_batch(
        ...
        if model_name not in self.embedding_models:
            if model_name in self.models:
                ...
                self.logger.warning(f"Model {model_name} not found, using default model")
                model_name = self.default_model_config.model_name
    ...
    async def nearest_neighbors(
        ...
        for i, embedding in enumerate(text_embeddings):
            similarity = self.compute_similarity(query_embedding, embedding, similarity_method)
        ...
        try:
            ...
            if model_type == EmbeddingType.SENTENCE_TRANSFORMER:
                ...
                if "model" not in config.additional_config:
                    raise ValueError("Custom model requires a 'model' in additional_config")
    ...
    async def _embed_with_huggingface(
        ...
        for start_idx in range(0, len(texts), config.batch_size):
            ...
            if config.device:
                encoded_input = {k: v.to(config.device) for k, v in encoded_input.items()}
            ...
            if config.pooling_strategy == "cls":
                ...
                raise ValueError(f"Unsupported pooling strategy: {config.pooling_strategy}")
            ...
            for retry in range(max_retries):
                try:
                    ...
                    for embedding_data in response.data:
                        all_embeddings.append(np.array(embedding_data.embedding, dtype=np.float32))
                    
                    break
                
                except Exception as e:
                    if retry < max_retries - 1:
                        self.logger.warning(f"OpenAI API error, retrying in {retry_delay}s: {e}")
                        await asyncio.sleep(retry_delay)
                        retry_delay *= 2
                    else:
                        self.logger.error(f"OpenAI API error after {max_retries} retries: {e}")
                        # Return zeros for failed embeddings
                        for _ in range(len(batch)):
                            all_embeddings.append(np.zeros(config.dimension, dtype=np.float32))
    ...
    async def _embed_with_custom_model(
        ...
        if "embed_function" not in config.additional_config:
            raise ValueError("Custom model requires an 'embed_function' in additional_config")
        ...
        if asyncio.iscoroutinefunction(embed_function):
            ...
            loop = asyncio.get_event_loop()
            return await loop.run_in_executor(None, embed_function, model, texts, config)
    ...
    async def enrich_rag_prompt(
        ...
        for idx, item in enumerate(filtered_context):
            ...
            if item.get('entity_type') and item.get('entity_id'):
                context_section += f"Entity: {item.get('entity_type')}/{item.get('entity_id')}\n"
