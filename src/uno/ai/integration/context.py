...
class UnifiedContextManager:
        ...
        if self.connection_string:
            ...
            async with self.pool.acquire() as conn:
                ...
                try:
                    ...
                    self.logger.warning(
                        "Vector extension not available, embedding search will be disabled"
                    )
    ...
    async def store_context(self, item: ContextItem) -> str:
        ...
        if not item.validity.expires_at and self.default_ttl > 0:
            item.validity.expires_at = datetime.datetime.now() + datetime.timedelta(
    ...
    async def get_context(
        self, context_id: str, mark_as_used: bool = True
    ...
    async def query_context(self, query: ContextQuery) -> list[ContextItem]:
        ...
        if self.pool and (not query.limit or len(results) < query.limit):
            db_results = await self._query_database(
                query, already_found=set(item.id for item in results)
            )
    ...
    async def update_context_relevance(
        self, context_id: str, relevance: Relevance
    ) -> bool:
        """
    ...
    async def get_user_context(
        ...
        """
        query = ContextQuery(
            user_id=user_id, limit=limit, source=source, valid_only=not include_expired
        )
    ...
    async def _query_database(
        self, query: ContextQuery, already_found: Set[str] = None
    ) -> list[ContextItem]:
        """
            ...
            async with self.pool.acquire() as conn:
                ...
                if query.valid_only:
                    ...
                    where_clauses.append(
                        f"(max_uses IS NULL OR current_uses < max_uses)"
                    )
                ...
                if query.min_relevance:
                    where_clauses.append(
                        f"relevance IN ({','.join(['$' + str(param_idx + i) for i in range(len(relevance_values))])})"
                    )
                    relevance_values = [
                        r.value
                        for r in Relevance
                        if r.value >= query.min_relevance.value
                    ]
                ...
                for key, value in query.metadata_filters.items():
                    where_clauses.append(
                        f"metadata->>'${param_idx}' = ${param_idx + 1}"
                    )
                    params.append(key)
    ...
    def _row_to_context_item(self, row) -> ContextItem:
        ...
        dependencies = (
            ...
            if row["dependencies"] and isinstance(row["dependencies"], str)
            else row["dependencies"] or []
        )
