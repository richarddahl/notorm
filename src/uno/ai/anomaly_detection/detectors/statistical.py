...
class StatisticalDetector(AnomalyDetector):
    ...
    def _train_zscore(self, values: np.ndarray) -> bool:
        ...
        if self.std == 0:
            self.logger.warning("Standard deviation is zero, Z-score detector may not be effective")
    ...
    def _train_moving_average(self, data: pd.DataFrame) -> bool:
        ...
        if not isinstance(data.index, pd.DatetimeIndex):
            self.logger.warning("Data index is not DatetimeIndex, moving average may not be effective")
            return False
    ...
    def _train_regression(self, data: pd.DataFrame) -> bool:
        ...
        # Ensure data is sorted by time
        if not isinstance(data.index, pd.DatetimeIndex):
            self.logger.warning("Data index is not DatetimeIndex, regression may not be effective")
            return False
        ...
        try:
            ...
            if self.include_seasonal and self.seasonality > 0:
                for period in [self.seasonality, self.seasonality // 2]:  # Fundamental and harmonic
    ...
    def _detect_moving_average(
        ...
        for window, window_data in self.moving_averages.items():
            ...
            try:
                closest_idx = self.training_data.index.get_indexer([timestamp], method='nearest')[0]
                ...
                self.logger.warning(f"Error detecting with moving average (window={window}): {e}")
        ...
        if anomalies:
            ...
            window, z_score, mean, std = next(
                (a for a in anomalies if a[0] == anomaly_window),
                anomalies[0]
            )
    ...
    def _get_description(self, value: float, z_score: float) -> str:
        ...
        return (f"Anomaly detected: {self.metric_name} value of {value:.2f} is {direction} "
    ...
    def _get_suggestion(self, value: float, z_score: float) -> str:
        ...
        if self.anomaly_type in [AnomalyType.SYSTEM_CPU, AnomalyType.SYSTEM_MEMORY, 
            ...
            return "Review user activity logs and investigate potential security concerns"
    ...
    def _get_description_iqr(
        ...
        """
        if value < lower_bound:
            return (f"Anomaly detected: {self.metric_name} value of {value:.2f} is below "
                    f"normal range ({lower_bound:.2f} to {upper_bound:.2f})")
        else:
            return (f"Anomaly detected: {self.metric_name} value of {value:.2f} is above "
                    f"normal range ({lower_bound:.2f} to {upper_bound:.2f})")
    ...
    def _get_suggestion_iqr(
        ...
        Returns:
            Suggestion for handling the anomaly
        """
        return self._get_suggestion(value, (value - (upper_bound + lower_bound) / 2) / self.iqr)
    
    def _get_description_ma(
        self,
        value: float,
        z_score: float,
        window: int
