...
class HybridDetector(AnomalyDetector):
    ...
    async def _detect_ensemble(
        ...
        if weighted_score >= self.ensemble_threshold:
            ...
            description = f"Ensemble anomaly detected: {self.metric_name} value of {value:.2f}\n"
    ...
    def _calculate_adaptive_weights(self) -> Dict[str, float]:
        ...
        for detector_id, perf in self.detector_performance.items():
            ...
            precision = perf["true_positives"] / (perf["true_positives"] + perf["false_positives"] + 1e-10)
            recall = perf["true_positives"] / (perf["true_positives"] + perf["false_negatives"] + 1e-10)
