...
class LearningBasedDetector(AnomalyDetector):
    ...
    def __init__(
        ...
        feature_columns: list[str] | None = None,  # Additional feature columns
    ...
    def _initialize_isolation_forest(self) -> None:
        ...
        if not self._sklearn_available:
            self.logger.warning(
                "scikit-learn not available, isolation forest cannot be used"
            )
            return
    ...
    def _initialize_one_class_svm(self) -> None:
        ...
        if not self._sklearn_available:
            self.logger.warning(
                "scikit-learn not available, one-class SVM cannot be used"
            )
            return
    ...
    async def _train_lstm(self, data: pd.DataFrame) -> bool:
        ...
        try:
            ...
            if not isinstance(data.index, pd.DatetimeIndex):
                self.logger.warning(
                    "Data index is not DatetimeIndex, time features may not be accurate"
                )
        ...
        try:
            ...
            if score > self.threshold_value:
                ...
                return AnomalyAlert(
                    ...
                    metric_value=value,
                    expected_range={"threshold": self.threshold_value},
    ...
    async def _detect_lstm(
        ...
        try:
            ...
            last_sequence = np.reshape(
                last_sequence, (1, self.sequence_length, X_recent_scaled.shape[1])
            )
            ...
            if mse > self.threshold_value:
                ...
                return AnomalyAlert(
                    ...
                    metadata={
                        ...
                        "threshold": self.threshold_value,
                        "predicted_value": (
                            float(prediction[0][0]) if prediction.size > 0 else None
                        ),
                    },
                )
    ...
    def _get_description_if(self, value: float, score: float) -> str:
        ...
        """
        return (
            f"Anomaly detected: {self.metric_name} value of {value:.2f} is anomalous "
    ...
    def _get_suggestion_if(self, value: float, score: float) -> str:
        ...
        if self.anomaly_type in [
            ...
            suggestion += (
                ". Review application logs and monitor related performance metrics."
            )
    ...
    def _get_description_svm(self, value: float, score: float) -> str:
        ...
        """
        return (
            f"Anomaly detected: {self.metric_name} value of {value:.2f} is outside normal boundaries "
    ...
    def _get_description_ae(self, value: float, error: float) -> str:
        ...
        """
        return (
            f"Anomaly detected: {self.metric_name} value of {value:.2f} is difficult to reconstruct "
    ...
    def _get_description_lstm(self, value: float, error: float) -> str:
        ...
        """
        return (
            f"Anomaly detected: {self.metric_name} value of {value:.2f} deviates from expected sequence pattern "
