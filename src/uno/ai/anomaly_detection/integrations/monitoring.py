...
class SystemMonitorIntegration:
    ...
    async def _fetch_historical_data(self, days: int) -> Dict[str, pd.DataFrame]:
        ...
        try:
            ...
            if self.metrics_source == "prometheus":
                historical_data = await self._fetch_prometheus_historical(start_time, end_time)
            elif self.metrics_source == "datadog":
                historical_data = await self._fetch_datadog_historical(start_time, end_time)
            elif self.metrics_source == "cloudwatch":
                historical_data = await self._fetch_cloudwatch_historical(start_time, end_time)
    ...
    async def _fetch_prometheus_historical(
        ...
        for metric_key in self.metrics_mapping:
            ...
            if metric_key == "cpu_usage":
                values = 0.3 + 0.2 * np.random.random(size=len(timestamps))  # 30-50% CPU usage
            elif metric_key == "memory_usage":
                values = (2.0 + 1.0 * np.random.random(size=len(timestamps))) * 1024 * 1024 * 1024  # 2-3GB
            elif metric_key == "disk_usage":
                values = 0.6 + 0.1 * np.random.random(size=len(timestamps))  # 60-70% disk usage
            elif metric_key == "network_traffic":
                values = (1.0 + 0.5 * np.random.random(size=len(timestamps))) * 1024 * 1024  # 1-1.5MB
            elif metric_key == "error_rate":
                values = 0.01 + 0.02 * np.random.random(size=len(timestamps))  # 1-3% error rate
            elif metric_key == "latency":
                values = 0.1 + 0.1 * np.random.random(size=len(timestamps))  # 100-200ms latency
            else:
                continue
            ...
            df = pd.DataFrame({
                ...
                self.metrics_mapping[metric_key]: values
            })
