...
class UserBehaviorIntegration:
        ...
        try:
            async with pool.acquire() as conn:
                ...
                if "login_frequency" in self.user_metrics_mapping:
                    ...
                    for i in range(5):
                        user_data.append({
                            self.user_metrics_mapping["login_frequency"]: np.random.poisson(3),  # Average 3 logins per day
                            "entity_id": f"user_{i}",
                            "entity_type": "user"
                        })
                ...
                if "transaction_amounts" in self.user_metrics_mapping:
                    ...
                    # Generate synthetic data for demonstration
                    for i in range(5):
                        for _ in range(np.random.poisson(2)):  # Average 2 transactions per user
                            user_data.append({
                                self.user_metrics_mapping["transaction_amounts"]: np.random.exponential(100),  # Average $100 transactions
                                "entity_id": f"user_{i}",
                                "entity_type": "user"
                            })
    ...
    async def _collect_kafka_user_data(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i in range(3):
                user_data.append({
                    self.user_metrics_mapping["access_patterns"]: hash(f"pattern_{np.random.randint(1, 10)}"),
    ...
    async def _collect_log_user_data(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if "session_duration" in self.user_metrics_mapping:
                for i in range(4):
                    user_data.append({
                        self.user_metrics_mapping["session_duration"]: np.random.exponential(15),  # Average 15 minutes
                        "entity_id": f"user_{np.random.randint(1, 20)}",
                        "entity_type": "user"
                    })
            ...
            for metric_key, metric_data in historical_data.items():
                if metric_key in self.user_metrics_mapping:
                    ...
                    for detector_id in detector_ids:
                        self.logger.info(f"Training detector {detector_id} on historical {metric_key} data")
    ...
    async def _fetch_historical_data(self, days: int) -> Dict[str, pd.DataFrame]:
        ...
        try:
            ...
            if self.data_source == "database":
                ...
                historical_data = self._generate_synthetic_historical(start_time, end_time)
            elif self.data_source == "logs":
    ...
    async def _fetch_db_historical(
        ...
        try:
            ...
            self.logger.error(f"Error fetching historical user behavior data from database: {e}")
    ...
    def _generate_synthetic_historical(
        ...
        for metric_key in self.user_metrics_mapping:
            ...
            if metric_key == "login_frequency":
                ...
                values = np.random.poisson(3, size=len(timestamps))  # Average 3 logins per day
                ...
                # Transaction amounts
                values = np.random.exponential(100, size=len(timestamps))  # Average $100
            elif metric_key == "transaction_frequency":
                # Daily transaction frequency per user
                values = np.random.poisson(2, size=len(timestamps))  # Average 2 transactions per day
                ...
                values = np.random.exponential(15, size=len(timestamps))  # Average 15 minutes
            else:
                continue
            ...
            df = pd.DataFrame({
                ...
                'user_id': np.random.choice([f"user_{i}" for i in range(1, 21)], size=len(timestamps)),
