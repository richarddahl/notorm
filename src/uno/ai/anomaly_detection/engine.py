...
class AnomalyDetector:
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_detection_time_ms": np.mean(self.metrics["detection_time_ms"]) if self.metrics["detection_time_ms"] else None,
        }
...
class AnomalyDetectionEngine:
    ...
    async def register_detector(
        ...
        if save_config and self.pool:
            ...
            async with self.pool.acquire() as conn:
                ...
                """, detector_id, detector.anomaly_type.value, detector.strategy.value,
                    detector.metric_name, json.dumps(config), True)
    ...
    async def train_all_detectors(
        ...
        if tasks:
            ...
            for i, result in enumerate(training_results):
                ...
                if isinstance(result, Exception):
                    self.logger.error(f"Error training detector {detector_id}: {result}")
    ...
    async def get_alerts(
        ...
        if severities:
            placeholders = ", ".join([f"${param_index + i}" for i in range(len(severities))])
        ...
        if self.pool:
            try:
                async with self.pool.acquire() as conn:
                    ...
                    """, alert.id, alert.timestamp, alert.anomaly_type.value,
                        alert.detection_strategy.value, alert.severity.value,
                        alert.entity_id, alert.entity_type, alert.metric_name,
                        alert.metric_value, json.dumps(alert.expected_range),
                        alert.deviation_factor, alert.description, alert.suggestion,
                        json.dumps(alert.related_alerts), json.dumps(alert.metadata))
