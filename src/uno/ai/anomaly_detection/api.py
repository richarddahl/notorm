...
def create_anomaly_detection_router(
    engine: AnomalyDetectionEngine, path_prefix: str = "/anomalies"
) -> APIRouter:
    """
    ...
    async def delete_detector(detector_id: str):
        ...
        try:
            ...
            if not success:
                raise HTTPException(
                    status_code=404, detail=f"Detector not found: {detector_id}"
                )
    ...
    async def get_detectors():
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to get detectors: {str(e)}"
            )
    ...
    async def process_data_point(request: DataPointRequest):
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to process data point: {str(e)}"
            )
...
async def _train_detectors_task(
    ...
    try:
        if historical_data:
                ...
                for detector_id in detector_ids:
                    if detector_id in engine.detectors:
                        ...
                        if metric_name in data_frames:
                            await engine.train_detector(
                                detector_id, data_frames[metric_name]
                            )
            else:
            ...
            if engine.logger:
                engine.logger.info(
                    f"Training detectors with {days} days of stored data"
                )
...
def integrate_anomaly_detection(
    ...
    # Create router
    router = create_anomaly_detection_router(engine=engine, path_prefix=path_prefix)
