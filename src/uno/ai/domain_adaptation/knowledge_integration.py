...
class DomainKnowledgeManager:
        ...
        try:
            ...
            for item in items:
                try:
                    ...
                    if config.create_embeddings and self.embedding_service and not item.embedding:
                        text_to_embed = f"{item.name} {item.definition or ''} {item.content or ''}"
                    ...
                    if item.definition:
                        ...
                        for synonym in item.synonyms:
                            self.domain_glossaries[item.domain][synonym] = item.definition
    ...
    async def _load_from_file(self, config: KnowledgeImportConfig) -> List[KnowledgeItem]:
        """
                    ...
                    for i, row in enumerate(csv_reader):
                        ...
                        item = KnowledgeItem(
                            ...
                            metadata={k: v for k, v in row.items() if k not in [
                                'id', 'domain', 'source', 'type', 'name', 'definition',
                                'content', 'synonyms', 'importance'
    ...
    async def _load_from_url(self, config: KnowledgeImportConfig) -> List[KnowledgeItem]:
        """
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(config.source_url) as response:
                    if response.status != 200:
                        self.logger.error(f"Failed to fetch data from URL: {response.status}")
                        return items
    ...
    async def _load_from_database(self, config: KnowledgeImportConfig) -> List[KnowledgeItem]:
        """
        ...
        try:
            async with self.pool.acquire() as conn:
                ...
                existing = await conn.fetchval(
                    f"SELECT id FROM {self.knowledge_table} WHERE id = $1",
                    item.id
                )
    ...
    async def search_knowledge(
        ...
        if not results and domain in self.knowledge_cache:
            ...
            for item_id, item in self.knowledge_cache[domain].items():
                ...
                name_similarity = SequenceMatcher(None, query.lower(), item.name.lower()).ratio()
                def_similarity = 0.0
                if item.definition:
                    def_similarity = SequenceMatcher(None, query.lower(), item.definition.lower()).ratio()
                
                content_similarity = 0.0
                if item.content:
                    content_similarity = SequenceMatcher(None, query.lower(), item.content.lower()).ratio()
        ...
        if config.mode == PromptEnhancementMode.APPEND:
            ...
            for item in relevant_items:
                ...
                if config.include_relationships and item.relationships:
                    for rel_type, related_items in item.relationships.items():
                        knowledge_text += f"  - {rel_type}: {', '.join(related_items)}\n"
            ...
            for item in relevant_items:
                ...
                if config.include_relationships and item.relationships:
                    for rel_type, related_items in item.relationships.items():
                        system_message["content"] += f"  - {rel_type}: {', '.join(related_items)}\n"
            ...
            for item in relevant_items:
                ...
                if term in prompt:
                    if config.include_definitions and item.definition:
                        ...
                        enhanced_prompt = enhanced_prompt.replace(term, f"{term}{definition}")
            ...
            if "{{relationships}}" in template:
                ...
                for term, relations in knowledge_vars["relationships"].items():
                    ...
                    for rel_type, related_items in relations.items():
                        relationships_text += f"  - {rel_type}: {', '.join(related_items)}\n"
