...
class DomainEmbeddingAdapter:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            fine_tuned_path = os.path.join(self.config.output_dir, f"{self.config.domain}_model")
            if os.path.exists(fine_tuned_path):
                self.logger.info(f"Loading fine-tuned model from {fine_tuned_path}")
                self.model = await loop.run_in_executor(
                    None,
                    lambda: SentenceTransformer(fine_tuned_path)
                )
                self.is_fine_tuned = True
                
                # Load evaluation results if available
                eval_path = os.path.join(self.config.output_dir, f"{self.config.domain}_eval.json")
    ...
    async def prepare_contrastive_data(
        ...
        if train_test_split > 0:
            ...
            self.evaluator = EmbeddingSimilarityEvaluator.from_input_examples(
                ...
                name=f"{self.config.domain}_dev"
            )
        else:
    ...
    async def prepare_triplet_data(
        self,
        ...
        if train_test_split > 0:
            ...
            for anchor, positive, negative in triplets[train_size:]:
                dev_similarity_pairs.append(InputExample(texts=[anchor, positive], label=1.0))
                dev_similarity_pairs.append(InputExample(texts=[anchor, negative], label=0.0))
    ...
    async def fine_tune(self) -> Dict[str, Any]:
        ...
        if self.config.method == FineTuningMethod.CONTRASTIVE:
            ...
            if not teacher_model_path:
                self.logger.error("Teacher model path required for knowledge distillation")
                return {"error": "Teacher model path required for knowledge distillation"}
            
            teacher_model = SentenceTransformer(teacher_model_path)
            train_loss = losses.MSELoss(
                model=self.model,
        ...
        if warmup_steps <= 0:
            warmup_steps = int(len(train_dataloader) * self.config.training.num_epochs * 0.1)
        
        # Set up output directory
        output_path = os.path.join(self.config.output_dir, f"{self.config.domain}_model")
        
        # Set up checkpoint saving
        checkpoint_path = self.config.training.checkpoint_path
        if not checkpoint_path:
            checkpoint_path = os.path.join(self.config.output_dir, f"{self.config.domain}_checkpoints")
        ...
        try:
            ...
            if self.config.training.save_best_model and os.path.exists(output_path):
                self.model = await loop.run_in_executor(
                    None,
                    lambda: SentenceTransformer(output_path)
                )
    ...
    async def evaluate(
        ...
        test_evaluator = EmbeddingSimilarityEvaluator.from_input_examples(
            test_examples,
            name=f"{self.config.domain}_test",
        ...
        try:
            ...
            spearman_correlation = await loop.run_in_executor(
                None,
                lambda: test_evaluator(self.model)
            )
    ...
    async def export_model(self, export_path: Optional[str] = None) -> str:
        ...
        if not export_path:
            export_path = os.path.join(self.config.output_dir, f"{self.config.domain}_export")
