...
class InteractionCreate(BaseModel):
    ...
    interaction_type: str = Field("view", description="Interaction type (view, like, purchase, etc.)")
    rating: Optional[float] = Field(None, description="Optional explicit rating (0-5)")
    timestamp: Optional[str] = Field(None, description="Interaction timestamp (ISO format)")
    content: Optional[str] = Field(None, description="Item content for content-based recommendations")
...
class BatchInteractionCreate(BaseModel):
    ...
    interactions: List[InteractionCreate] = Field(..., description="List of interactions")
...
class RecommendationRequest(BaseModel):
    ...
    exclusions: Optional[List[str]] = Field(None, description="Items to exclude")
...
class Recommendation(BaseModel):
    ...
    sources: Optional[List[int]] = Field(None, description="Source algorithms (for hybrid recommenders)")
    metadata: Optional[Dict[str, Any]] = Field(None, description="Additional metadata")
...
def create_recommendation_router(
    ...
    async def create_interactions_batch(request: BatchInteractionCreate):
        ...
        try:
            interactions = [
                interaction.dict() for interaction in request.interactions
            ]
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Failed to create interactions batch: {str(e)}"
            )
...
def integrate_recommendations(
    ...
    # Create router
    router = create_recommendation_router(
        engine, 
