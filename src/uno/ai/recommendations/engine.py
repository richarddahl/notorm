...
class RecommendationAlgorithm(Generic[T], ABC):
    ...
    @abstractmethod
    async def recommend(
        self, user_id: str, limit: int = 10, exclusions: Optional[list[T]] = None
    ) -> list[dict[str, Any]]:
        """
...
class ContentBasedRecommender(RecommendationAlgorithm[T]):
    ...
    async def add_interaction(self, interaction: dict[str, Any]) -> None:
        ...
        timestamp = interaction.get(
            self.timestamp_field, datetime.now(datetime.UTC).isoformat()
        )
        ...
        if content:
            ...
            await self.vector_storage.store(
                ...
                embedding=embedding,
                metadata={"item_id": str(item_id), "item_type": item_type},
            )
    ...
    async def recommend(
        ...
        logger.debug(
            f"Generated {len(filtered_results)} recommendations for user {user_id}"
        )
...
class CollaborativeFilteringRecommender(RecommendationAlgorithm[T]):
    ...
    async def add_interaction(self, interaction: dict[str, Any]) -> None:
        ...
        if item_id not in self.item_metadata:
            self.item_metadata[item_id] = {"item_id": item_id, "item_type": item_type}
    ...
    async def _update_similarity_matrix(self) -> None:
        ...
        if len(valid_users) < 2:
            logger.warning(
                "Not enough users with interactions for similarity calculation"
            )
            return
    ...
    def _calculate_similarity(self, user1: str, user2: str) -> float:
        ...
        for item_id in set(items1.keys()) | set(items2.keys()):
            ...
            if self.use_timestamps and self.latest_timestamp != datetime.min:
                if (
                    user1 in self.user_metadata
                    and item_id in self.user_metadata[user1]
                    and "timestamp" in self.user_metadata[user1][item_id]
                ):
                    timestamp1 = self.user_metadata[user1][item_id]["timestamp"]
                    time_diff = (
                        self.latest_timestamp - timestamp1
                    ).total_seconds() / 86400.0  # days
                    decay = np.exp(-self.time_decay_factor * time_diff)
                    rating1 *= decay

                if (
                    user2 in self.user_metadata
                    and item_id in self.user_metadata[user2]
                    and "timestamp" in self.user_metadata[user2][item_id]
                ):
                    timestamp2 = self.user_metadata[user2][item_id]["timestamp"]
                    time_diff = (
                        self.latest_timestamp - timestamp2
                    ).total_seconds() / 86400.0  # days
                    decay = np.exp(-self.time_decay_factor * time_diff)
                    rating2 *= decay
    ...
    async def recommend(
        ...
        similar_users = sorted(
            self.user_similarity[user_id].items(), key=lambda x: x[1], reverse=True
        )
            ...
            [
                {
                    "item_id": item_id,
                    "score": score,
                    "item_type": self.item_metadata.get(item_id, {}).get(
                        "item_type", "item"
                    ),
                }
...
class HybridRecommender(RecommendationAlgorithm[T]):
    ...
    async def initialize(self) -> None:
        ...
        self.initialized = True
        logger.info(
            f"Initialized HybridRecommender with {len(self.algorithms)} algorithms"
        )
        ...
        for i, algorithm in enumerate(self.algorithms):
            try:
                ...
                recommendations = await algorithm.recommend(
                    user_id=user_id, limit=algo_limit, exclusions=exclusions
                )
                ...
                logger.error(
                    f"Error getting recommendations from algorithm {i}: {str(e)}"
                )
...
class RecommendationEngine(Generic[T]):
    ...
    async def recommend(
        ...
        if item_type:
            recommendations = [
                rec for rec in recommendations if rec.get("item_type") == item_type
            ][:limit]
