...
# SQL statement errors
class SQLStatementError(BaseError):
...
class SQLSyntaxError(BaseError):
...
# SQL emitter errors
class SQLEmitterError(BaseError):
...
class SQLEmitterInvalidConfigError(BaseError):
...
# SQL registry errors
class SQLRegistryClassNotFoundError(BaseError):
...
class SQLRegistryClassAlreadyExistsError(BaseError):
...
# SQL configuration errors
class SQLConfigError(BaseError):
...
class SQLConfigInvalidError(BaseError):
