...
class UserRepository(BaseRepository[UserModel]):
    ...
    async def find_by_role(
        ...
        """
        stmt = select(self.model_class).where(self.model_class.roles.any(id=role_id))
...
class GroupRepository(BaseRepository[GroupModel]):
    ...
    async def find_by_name(self, name: str, tenant_id: str) -> Optional[GroupModel]:
        """
        ...
        """
        stmt = select(self.model_class).where(
            and_(self.model_class.name == name, self.model_class.tenant_id == tenant_id)
        )
        result = await self.session.execute(stmt)
        return result.scalars().first()
    ...
    async def find_by_user(
        ...
        """
        stmt = select(self.model_class).where(self.model_class.users.any(id=user_id))
