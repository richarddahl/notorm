...
class ReportDataAggregator:
    ...
    async def get_aggregated_data(
        ...
        try:
            ...
            if not cache_key:
                cache_key = self._generate_cache_key(template_id, parameters, aggregations)
            ...
            execution_result = await self.execution_service.execute(template_id, parameters)
            if execution_result.is_failure():
                return Failure(AggregationError(f"Failed to execute report: {execution_result.error}"))
            
            execution = execution_result.value
            
            # Wait for completion if needed
            if execution.status not in [ReportExecutionStatus.COMPLETED, "completed"]:
                completion_result = await self.execution_service.wait_for_completion(execution.id, 60)
                if completion_result.is_failure():
                    return Failure(AggregationError(f"Failed to wait for report completion: {completion_result.error}"))
                execution = completion_result.value
            
            # Get the result data
            result_data = execution.data
            if not result_data:
                return Failure(AggregationError(f"No data in report execution {execution.id}"))
    ...
    async def get_multi_report_data(
        ...
        try:
            # Generate cache key if not provided
            if not cache_key:
                cache_key = self._generate_cache_key("multi", template_ids, date_range, filters)
                ...
                for template_id, task in completion_tasks:
                    try:
                        ...
                        if completion_result.is_success():
                            ...
                            dashboard_data[template_id] = {"error": str(completion_result.error)}
                    except Exception as e:
            ...
            return Failure(AggregationError(f"Error getting multi-report data: {str(e)}"))
    ...
    async def _execute_time_series(
        ...
        result = await loop.run_in_executor(
            self._executor,
            lambda: self._perform_time_series(df, date_column, value_column, group_by, frequency)
        )
    ...
    def _perform_time_series(
        ...
        if group_by:
            ...
            grouped = df.groupby(group_by + [pd.Grouper(key=date_column, freq=frequency)])
        ...
        time_series[date_column] = time_series[date_column].dt.strftime('%Y-%m-%dT%H:%M:%S')
    ...
    async def _execute_summary(
        ...
        if not all(col in df.columns for col in columns):
            ...
            self.logger.warning(f"Missing columns in data: {missing_cols}. Available: {available_cols}")
        ...
        result = await loop.run_in_executor(
            self._executor,
            lambda: self._perform_summary(df, columns)
        )
        
        return result
    
    def _perform_summary(
        self,
        df: pd.DataFrame,
        columns: List[str]
    ) -> Dict[str, Any]:
    ...
    def _generate_cache_key(self, *args) -> str:
        ...
        json_args = [json.dumps(arg, sort_keys=True) if not isinstance(arg, str) else arg for arg in args]
