...
class MessageRepository(Repository, MessageRepositoryProtocol):
    ...
    async def get_by_id(self, message_id: str) -> Optional[Message]:
        """Get a message by its ID."""
        message_model = await self.session.query(MessageModel).filter(
    ...
    async def get_messages_for_user(
        ...
        """Get messages for a specific user."""
        query = self.session.query(MessageModel).join(
            MessageUserModel, 
        ...
        message_models = await query.order_by(
            MessageModel.sent_at.desc()
    ...
    async def get_message_thread(
        ...
        offset = (page - 1) * page_size
        child_models = await self.session.query(MessageModel).filter(
            MessageModel.parent_id == parent_message_id
        ).order_by(
            MessageModel.sent_at.asc()
        ).offset(offset).limit(page_size).all()
    ...
    async def update(self, message: Message) -> Message:
        ...
        existing_users = await self.session.query(MessageUserModel).filter(
        ...
        for user in message.users:
            if user.user_id not in existing_user_ids:
                ...
                user_model = next((u for u in existing_users if u.user_id == user.user_id), None)
                if user_model:
    ...
    async def get_draft_messages_for_user(
        ...
        """Get draft messages for a specific user."""
        offset = (page - 1) * page_size
        message_models = await self.session.query(MessageModel).join(
            MessageUserModel, 
            MessageUserModel.message_id == MessageModel.id
        ).filter(
            MessageUserModel.user_id == user_id,
    ...
    def _model_to_entity(self, model: MessageModel) -> Message:
        ...
        meta_record_ids = [mr.id for mr in model.meta_records] if hasattr(model, 'meta_records') else []
