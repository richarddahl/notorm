...
class MessageDomainServiceProtocol(Protocol):
    ...
    async def update_message(
        ...
        meta_record_ids: Optional[List[str]] = None
    ) -> Message:
...
class MessageDomainService(DomainService, MessageDomainServiceProtocol):
    ...
    async def update_message(
        ...
        sender_id = next((user.user_id for user in message.users if user.is_sender), None)
    ...
    async def send_message(self, message_id: str) -> Message:
        ...
        has_recipients = any(
            not user.is_sender for user in message.users
        )
