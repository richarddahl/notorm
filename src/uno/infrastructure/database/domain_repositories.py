...
from typing import Generic, Protocol, TypeVar, List, Dict, Any, Optional, Union, Type, AsyncIterator, Iterator, cast
import logging
from datetime import datetime, UTC

from sqlalchemy import select, insert, update, delete, text, create_engine, MetaData, Table, Column
...
from uno.database.entities import (
    ...
    QueryStatistics, QueryId, QueryPlan, IndexRecommendation, 
    ConnectionPoolConfig, PoolStatistics, CachedResult, CacheKey, CacheConfig
)
...
class ModelRepositoryProtocol(Generic[ModelT, EntityT], Protocol):
    ...
    async def get_by_id(self, id: Union[str, DatabaseId]) -> UnoResult[Optional[EntityT], ErrorDetails]:
        """
        Get an entity by ID.
...
class SqlAlchemyDatabaseTransactionRepository:
    ...
    async def transaction(
        ...
        if isinstance(transaction_result, Failure):
            raise Exception(f"Failed to begin transaction: {transaction_result.error.message}")
        ...
        try:
            ...
            if isinstance(commit_result, Failure):
                raise Exception(f"Failed to commit transaction: {commit_result.error.message}")
        except Exception as e:
    ...
    async def get_by_id(self, id: Union[str, DatabaseId]) -> UnoResult[Optional[EntityT], ErrorDetails]:
        ...
        try:
            async with self.session_repository.get_session() as session:
                stmt = select(self.model_class).where(getattr(self.model_class, self.id_field) == id_value)
    ...
    async def update(self, entity: EntityT) -> UnoResult[EntityT, ErrorDetails]:
        ...
        try:
            ...
            if id_value is None:
                return Failure(ErrorDetails(
                    code="INVALID_ENTITY",
                    message=f"Entity has no ID value"
                ))
...
class InMemoryIndexRecommendationRepository:
    ...
    async def save_recommendation(
        ...
        for existing in self._recommendations:
            if (existing.table_name == recommendation.table_name and
                existing.column_names == recommendation.column_names and
                existing.index_type == recommendation.index_type):
                # Update existing recommendation
                existing.estimated_improvement = max(
                    existing.estimated_improvement,
    ...
    async def list_recommendations(
        ...
        sorted_recommendations = sorted(
            filtered,
            key=lambda r: (r.priority, r.estimated_improvement),
            reverse=True
        )
