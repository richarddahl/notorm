    ...
    def generate_report(
        ...
        report = {
            "time_range": {
                ...
                "duration_hours": (snapshots[-1].timestamp - snapshots[0].timestamp)
                / 3600,
            },
        ...
        if len(snapshots) > 1:
            ...
            report["trends"] = {
                ...
                "very_slow_query_change": latest.very_slow_query_count
                - first.very_slow_query_count,
                "avg_execution_time_change": latest.avg_execution_time
                - first.avg_execution_time,
                "index_recommendations_change": latest.index_recommendations
                - first.index_recommendations,
                "implemented_indexes_change": latest.implemented_indexes
                - first.implemented_indexes,
            }
            ...
            if first.slow_query_count > 0:
                report["trends"]["slow_query_pct_change"] = (
                    (latest.slow_query_count - first.slow_query_count)
                    / first.slow_query_count
                    * 100
                )

            if first.avg_execution_time > 0:
                report["trends"]["avg_execution_time_pct_change"] = (
                    (latest.avg_execution_time - first.avg_execution_time)
                    / first.avg_execution_time
                    * 100
                )
...
def track_query_performance(
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            current_time = time.time()
            if (
                not hasattr(track_query_performance, "_last_collection_time")
                or current_time - track_query_performance._last_collection_time >= 60
            ):
