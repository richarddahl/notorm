...
# Import error types
from uno.core.base.error import BaseError, ErrorContext
...
# Map PostgreSQL error codes to BaseError subclasses
PG_ERROR_MAPPING: Dict[str, Type[BaseError]] = {
    # Connection errors
...
def map_pg_exception_to_uno_error(
    ...
    default_error_class: Type[BaseError] = DatabaseQueryError,
    **context: Any,
) -> BaseError:
    """
    Map a PostgreSQL exception to a appropriate BaseError.
    ...
    Returns:
        An appropriate BaseError subclass instance
    """
    ...
    if error_code and error_code in PG_ERROR_MAPPING:
        ...
        if (
            error_class == DatabaseUniqueViolationError
            and "constraint_name" in error_details
        ):
        ...
        if (
            error_class == DatabaseForeignKeyViolationError
            and "constraint_name" in error_details
        ):
        ...
        if (
            error_class == DatabaseColumnNotFoundError
            and "column_name" in error_details
        ):
        ...
        if issubclass(error_class, DatabaseTransactionError):
            return error_class(reason=default_message or error_message, **error_context)

        # For deadlock detection
        if error_code == PostgresErrorCode.DEADLOCK_DETECTED:
            return DatabaseTransactionConflictError(
                reason="Deadlock detected", **error_context
            )

        # General case - create the error object with available info
        try:
            return error_class(
                message=default_message or error_message, **error_context
            )
        except TypeError:
    ...
    return default_error_class(reason=default_message or error_message, **error_context)
...
async def handle_pg_error(
    ...
    **kwargs: Any,
    ...
    Raises:
        BaseError: An appropriate BaseError if a PostgreSQL error occurs
    """
    try:
        return await coro(*args, **kwargs)
    except Exception as ex:
        # Rethrow BaseErrors as is
        if isinstance(ex, BaseError):
            raise

        # Map and raise PostgreSQL errors as BaseErrors
        mapped_error = map_pg_exception_to_uno_error(
            ex, default_message=error_message, operation=coro.__name__
        )
    ...
    def decorator(func: Callable[..., Awaitable[T]]) -> Callable[..., Awaitable[T]]:
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> T:
            return await handle_pg_error(
                func, *args, error_message=error_message, **kwargs
            )
...
def is_transient_error(error: Exception) -> bool:
    ...
    if (
        ...
        or is_deadlock_error(error)
        or is_serialization_error(error)
    ):
        return True
