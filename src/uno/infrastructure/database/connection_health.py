...
class ConnectionHealthMetrics:
    ...
    def record_error(self, error_type: str | None = None) -> None:
    ...
    def record_ping(self, latency: float) -> None:
        ...
        self.network_latency = (
            sum(self.ping_times) / len(self.ping_times) if self.ping_times else 0
        )
...
class HealthClassifier:
    ...
    def __init__(
        ...
        self.thresholds = {
            ...
            "network_latency": (
                ...
                network_latency_critical_threshold,
            ),
    ...
    def classify_metrics(
    ...
    ) -> Tuple[ConnectionHealthState, float, list[ConnectionIssue]]:
        """
        ...
        if age >= critical:
            ...
            severity = (
                (age - warning) / (critical - warning) * 0.4
            )  # Lower severity for age issues
            issues.append(
                ConnectionIssue(
                    connection_id="",  # Will be filled by caller
                    issue_type=ConnectionIssueType.RESOURCE_USAGE,
        ...
        score = (
            sum(scores.get(k, 1.0) * w for k, w in self.weights.items()) / total_weight
        )
...
class ConnectionHealthMonitor:
    ...
    def __init__(
        ...
        self._health_change_callbacks: list[
            Callable[
                [str, ConnectionHealthState, ConnectionHealthState], Awaitable[None]
            ]
        ] = []
        self._issue_detected_callbacks: list[
            Callable[[str, ConnectionIssue], Awaitable[None]]
        ] = []
        self._issue_resolved_callbacks: list[
            Callable[[str, ConnectionIssue], Awaitable[None]]
        ] = []
    ...
    def set_connection_provider(
        self, provider: Callable[[], Awaitable[AsyncConnection]]
    ) -> None:
        """
    ...
    def register_issue_detected_callback(
        self, callback: Callable[[str, ConnectionIssue], Awaitable[None]]
    ) -> None:
        """
    ...
    async def record_error(
        self, connection_id: str, error_type: str | None = None
    ) -> None:
        """
    ...
    async def record_transaction_end(
        self, connection_id: str, success: bool = True
    ) -> None:
        """
        Record the end of a transaction.
    ...
    async def ping_connection(
        self, connection: AsyncConnection, attempts: int = 3
    ) -> Tuple[bool, float]:
        """
            ...
            with contextlib.suppress(asyncio.TimeoutError):
                async with timeout(5.0):
                    ...
                    if len(rows) == 1:
                        ...
                        # Multiple rows
                        return {"rows": [dict(zip(keys, row)) for row in rows]}
        except Exception as e:
    ...
    async def check_connection_health(
        self, connection_id: str, connection: AsyncConnection
        ...
        if diagnostics.get("idle_in_transaction") and diagnostics[
            "idle_in_transaction"
        ].get("transaction_seconds"):
            transaction_seconds = float(
                diagnostics["idle_in_transaction"]["transaction_seconds"]
            )

            if transaction_seconds > 60.0:  # More than 1 minute idle in transaction
                severity = min(
                    1.0, transaction_seconds / 600.0
                )  # Scale up to 10 minutes
        ...
        # Check for locks
        if (
            diagnostics.get("transaction_locks")
            and "rows" in diagnostics["transaction_locks"]
        ):
        ...
        # Check for blocking
        if (
            diagnostics.get("blocking_sessions")
            and "rows" in diagnostics["blocking_sessions"]
        ):
            blocking_count = len(diagnostics["blocking_sessions"]["rows"])

            if blocking_count > 0:
                severity = min(
                    0.9, blocking_count / 5.0
                )  # Scale up to 5 blocked sessions
        ...
        async with self._lock:
            ...
            old_state = self._connection_states.get(
                connection_id, ConnectionHealthState.UNKNOWN
            )
    ...
    ) -> Optional[ConnectionHealthAssessment]:
        """
    ...
    async def get_issues(
        self, connection_id: str, include_resolved: bool = False
    ...
    async def resolve_issue(
        self, connection_id: str, issue_index: int, action: str
    ) -> bool:
        """
    ...
    async def start_monitoring(self) -> None:
        ...
        self._monitoring_task = asyncio.create_task(
            self._monitoring_loop(), name=f"health_monitor_{self.pool_name}"
        )

        self.logger.info(
            f"Started connection health monitoring for pool {self.pool_name}"
        )
    ...
    async def stop_monitoring(self) -> None:
        ...
        self.logger.info(
            f"Stopped connection health monitoring for pool {self.pool_name}"
        )
...
class ConnectionRecycler:
    ...
    async def mark_for_recycling(self, connection_id: str, reason: str) -> None:
        """
    ...
    async def start_recycling(self) -> None:
        ...
        self._recycling_task = asyncio.create_task(
            self._recycling_loop(), name=f"recycler_{self.health_monitor.pool_name}"
        )

        self.logger.info(
            f"Started connection recycling for pool {self.health_monitor.pool_name}"
        )
    ...
    async def stop_recycling(self) -> None:
        ...
        self.logger.info(
            f"Stopped connection recycling for pool {self.health_monitor.pool_name}"
        )
        ...
        for conn_id in connection_ids:
            ...
            if assessment.score < self.unhealthy_threshold:
                await self.mark_for_recycling(
                    conn_id, f"Unhealthy connection (score: {assessment.score:.2f})"
                )
                continue
            ...
            if age > self.age_threshold:
                await self.mark_for_recycling(
                    conn_id, f"Connection too old (age: {age:.1f}s)"
                )
                continue
            ...
            if metrics.query_count > self.usage_threshold:
                await self.mark_for_recycling(
                    conn_id, f"High query count (count: {metrics.query_count})"
                )
                continue
            ...
            if metrics.error_count > self.error_threshold:
                await self.mark_for_recycling(
                    conn_id, f"Too many errors (count: {metrics.error_count})"
                )
                continue
            ...
            if error_rate > self.error_rate_threshold:
                await self.mark_for_recycling(
                    conn_id, f"High error rate (rate: {error_rate:.1%})"
                )
                continue
