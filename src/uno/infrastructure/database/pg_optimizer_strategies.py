...
class PgOptimizationStrategies:
    ...
    async def get_table_statistics(self, table_name: str) -> OpResult[Dict[str, Any]]:
        ...
        try:
            ...
            if not stats_data:
                return Failure(
                    f"Table {table_name} not found or no statistics available"
                )
    ...
    async def recommend_table_maintenance(
        ...
        if (
            "table_bytes" in stats
            and "row_estimate" in stats
            and "avg_row_size" in stats
        ):
        ...
        if (
            "index_bytes" in stats
            and "table_bytes" in stats
            and stats["table_bytes"] > 0
        ):
    ...
    def recommend_covering_index(
        ...
        for table_name in query_plan.table_scans:
            ...
            key_columns = self._extract_filter_columns(
    ...
    def recommend_partial_index(
        ...
        for table_name in query_plan.table_scans:
            ...
            table_where_clause = self._extract_partial_index_condition(
    ...
    def _extract_filter_columns(
        ...
        if hasattr(self.optimizer, "_extract_filter_columns") and callable(
            self.optimizer._extract_filter_columns
        ):
        ...
        table_columns = [
            col["name"] for col in self.optimizer._table_info[table_name]["columns"]
        ]
    ...
    def _extract_output_columns(self, table_name: str, query_text: str) -> list[str]:
        ...
        select_match = re.search(
            r"SELECT\s+(.*?)\s+FROM", query_text, re.IGNORECASE | re.DOTALL
        )
        if not select_match:
            return columns
        ...
        alias_match = re.search(
            r"FROM\s+%s\s+(?:AS\s+)?(\w+)" % table_name, query_text, re.IGNORECASE
        )
    ...
    def _extract_expressions(self, query_text: str) -> list[dict[str, Any]]:
        ...
        date_matches = re.finditer(
            r"EXTRACT\(\s*(\w+)\s+FROM\s+(\w+)\.(\w+)\s*\)", query_text, re.IGNORECASE
        )
        ...
        concat_matches = re.finditer(r"(\w+)\.(\w+)\s*\|\|\s*(\w+)\.(\w+)", query_text)
    ...
    async def rewrite_for_pg_features(self, query: str) -> OpResult[QueryRewrite]:
        """
    ...
    async def _rewrite_json_functions(self, query: str) -> OpResult[QueryRewrite]:
        ...
        json_scalar_pattern = (
            r"JSON_EXTRACT_SCALAR\(\s*(\w+)\.(\w+)\s*,\s*['\"]([^'\"]+)['\"]\s*\)"
        )
    ...
    async def _rewrite_to_distinct_on(self, query: str) -> OpResult[QueryRewrite]:
        ...
        if not match:
            return Failure(
                "No suitable GROUP BY pattern found for DISTINCT ON optimization"
            )
        ...
        new_select = re.sub(r"(MIN|MAX)\(\s*(\w+\.\w+|\w+)\s*\)", r"\2", select_clause)
...
class PgQueryOptimizer(QueryOptimizer):
    ...
    async def recommend_indexes(self, query_plan, query_text=None):
        ...
        if query_text and self.config.recommend_indexes:
            ...
            covering_indexes = self.pg_strategies.recommend_covering_index(
                query_plan, query_text
            )
            recommendations.extend(covering_indexes)

            # Add partial indexes
            partial_indexes = self.pg_strategies.recommend_partial_index(
                query_plan, query_text
            )
            recommendations.extend(partial_indexes)

            # Add expression indexes
            expression_indexes = self.pg_strategies.recommend_expression_index(
                query_plan, query_text
            )
