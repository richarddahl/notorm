...
class DatabaseConnectionTimeoutError(BaseError):
...
class DatabaseConnectionPoolExhaustedError(BaseError):
...
class DatabaseQueryTimeoutError(BaseError):
...
class DatabaseQuerySyntaxError(BaseError):
...
class DatabaseTransactionRollbackError(BaseError):
...
class DatabaseTransactionConflictError(BaseError):
...
class DatabaseUniqueViolationError(BaseError):
...
class DatabaseForeignKeyViolationError(BaseError):
...
# Resource errors
class DatabaseResourceNotFoundError(BaseError):
...
class DatabaseResourceAlreadyExistsError(BaseError):
...
class DatabaseTableNotFoundError(BaseError):
...
class DatabaseColumnNotFoundError(BaseError):
...
# Session errors
class DatabaseSessionError(BaseError):
...
class DatabaseSessionExpiredError(BaseError):
...
class DatabaseNotSupportedError(BaseError):
