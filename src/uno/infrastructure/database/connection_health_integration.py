...
from typing import Dict, List, Set, Optional, Any, Callable, Awaitable, Tuple, Union, TypeVar, cast
import asyncio
import logging
import time
import contextlib
...
class HealthAwareConnectionPool(EnhancedConnectionPool):
    ...
    async def release(self, conn_id: str) -> None:
        ...
        async with self._pool_lock:
            if conn_id in self._connections:
                ...
                # Check health state
                health_state = self._health_states.get(conn_id, ConnectionHealthState.UNKNOWN)
    ...
    def get_health_metrics(self) -> Dict[str, Any]:
        ...
        metrics = {
            "health_states": {
                conn_id: state.value
                for conn_id, state in self._health_states.items()
            },
        }
