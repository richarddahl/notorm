...
class EncryptionManager:
    ...
    def _create_key_manager(self) -> Any:
        ...
        if key_management == KeyManagementType.VAULT:
            try:
                ...
                self.logger.warning(
                    "Vault key manager not available, falling back to local"
                )
            ...
            try:
                ...
                self.logger.warning(
                    "AWS KMS key manager not available, falling back to local"
                )
                from uno.security.encryption.local_key_manager import LocalKeyManager
            ...
            try:
                ...
                self.logger.warning(
                    "Azure Key Vault manager not available, falling back to local"
                )
                from uno.security.encryption.local_key_manager import LocalKeyManager
            ...
            try:
                ...
                self.logger.warning(
                    "GCP KMS key manager not available, falling back to local"
                )
                from uno.security.encryption.local_key_manager import LocalKeyManager
    ...
    def encrypt_field(
        self, field_name: str, value: str, entity_type: str | None = None
    ) -> str:
        """
