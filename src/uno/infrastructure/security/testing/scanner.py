    ...
    def _initialize_scanners(self) -> None:
        ...
        if self.config.enable_penetration_testing:
            try:
                ...
                self.scanners["penetration"] = PenetrationTester(
    ...
    def scan(self, target: str) -> dict[str, Any]:
        ...
        for scanner_name, scanner in self.scanners.items():
            try:
                ...
                self.logger.info(
                    f"Found {len(scanner_vulnerabilities)} vulnerabilities with {scanner_name} scanner"
                )
        ...
        if self.config.fail_build_on_high and severity_counts["high"] > 0:
            fail_conditions.append(
                f"Found {severity_counts['high']} high vulnerabilities"
            )
        if self.config.fail_build_on_medium and severity_counts["medium"] > 0:
            fail_conditions.append(
                f"Found {severity_counts['medium']} medium vulnerabilities"
            )
