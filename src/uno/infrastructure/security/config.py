...
class EncryptionConfig(BaseModel):
    ...
    algorithm: EncryptionAlgorithm = Field(
        EncryptionAlgorithm.AES_GCM, description="Encryption algorithm to use"
    )
    key_management: KeyManagementType = Field(
        KeyManagementType.LOCAL, description="Key management system"
    )
    key_rotation_days: int = Field(
        90, description="Number of days between key rotations"
    )
    data_at_rest_encryption: bool = Field(
        True, description="Enable encryption for data at rest"
    )
    data_in_transit_encryption: bool = Field(
        True, description="Enable encryption for data in transit (HTTPS)"
    )
...
class PasswordPolicyConfig(BaseModel):
    ...
    level: PasswordPolicyLevel = Field(
        PasswordPolicyLevel.STANDARD, description="Password policy level"
    )
    ...
    password_history: int = Field(
        5, description="Number of previous passwords to remember"
    )
    max_age_days: int = Field(90, description="Maximum password age in days")
    lockout_threshold: int = Field(
        5, description="Number of failed attempts before lockout"
    )
    lockout_duration_minutes: int = Field(30, description="Lockout duration in minutes")
...
class AuthenticationConfig(BaseModel):
    ...
    mfa_type: MFAType = Field(
        MFAType.TOTP, description="Multi-factor authentication type"
    )
    enable_sso: bool = Field(False, description="Enable single sign-on")
    ...
    remember_me_duration_days: int = Field(
        30, description="Remember me duration in days"
    )
...
class AuditingConfig(BaseModel):
    ...
    enabled: bool = Field(True, description="Enable security auditing")
    level: AuditLogLevel = Field(AuditLogLevel.STANDARD, description="Audit log level")
    ...
    include_user_agent: bool = Field(
        True, description="Include user agent in audit logs"
    )
    include_ip_address: bool = Field(
        True, description="Include IP address in audit logs"
    )
    include_request_id: bool = Field(
        True, description="Include request ID in audit logs"
    )
...
class WebSecurityConfig(BaseModel):
    ...
    enable_clickjacking_protection: bool = Field(
        True, description="Enable clickjacking protection"
    )
    enable_content_type_options: bool = Field(
        True, description="Enable content type options"
    )
    enable_referrer_policy: bool = Field(True, description="Enable referrer policy")
    referrer_policy: str = Field(
        "strict-origin-when-cross-origin", description="Referrer policy"
    )
    ...
    cors_max_age: int = Field(86400, description="CORS max age in seconds")  # 1 day
...
class SecurityTestingConfig(BaseModel):
    ...
    enable_dependency_scanning: bool = Field(
        True, description="Enable dependency scanning"
    )
    enable_static_analysis: bool = Field(True, description="Enable static analysis")
    enable_dynamic_analysis: bool = Field(
        False, description="Enable dynamic analysis (DAST)"
    )
    enable_penetration_testing: bool = Field(
        False, description="Enable penetration testing"
    )
    scan_frequency: str = Field("weekly", description="Scan frequency")
    fail_build_on_critical: bool = Field(
        True, description="Fail build on critical vulnerabilities"
    )
    fail_build_on_high: bool = Field(
        True, description="Fail build on high vulnerabilities"
    )
    ...
    security_scan_timeout: int = Field(
        1800, description="Security scan timeout in seconds"  # 30 minutes
    )
...
class SecurityConfig(BaseModel):
    ...
    authentication: AuthenticationConfig = Field(
        default_factory=AuthenticationConfig, description="Authentication configuration"
    )
    auditing: AuditingConfig = Field(
        default_factory=AuditingConfig, description="Auditing configuration"
    )
    web_security: WebSecurityConfig = Field(
        default_factory=WebSecurityConfig, description="Web security configuration"
    )
    ...
    def production_defaults(cls) -> "SecurityConfig":
        ...
        config.web_security.content_security_policy_level = (
    ...
    def development_defaults(cls) -> "SecurityConfig":
        ...
        # Web security defaults
        config.web_security.content_security_policy_level = (
            ContentSecurityPolicyLevel.BASIC
        )
