...
class OrderEntity(AggregateRoot):
    ...
    def add_item(self, item: OrderItemEntity) -> None:
        ...
        self.record_event(OrderItemAddedEvent(
            order_id=self.id,
            item_id=item.id,
...
class ProductSpecificationTranslator:
    ...
    def translate(self, spec: Specification[ProductEntity]) -> Dict[str, Any]:
        ...
        if isinstance(spec, ProductByPriceRangeSpecification):
            return {
                "price__gte": spec.min_price,
                "price__lte": spec.max_price
            }
...
async def product_repository_example(session: AsyncSession) -> None:
    ...
    # Batch operations
    products_to_add = [
        ProductEntity(id=0, name="Product 1", price=19.99, category="Books", sku="BK-001"),
        ProductEntity(id=0, name="Product 2", price=29.99, category="Books", sku="BK-002"),
        ProductEntity(id=0, name="Product 3", price=39.99, category="Books", sku="BK-003"),
    ]
...
async def unit_of_work_example(session: AsyncSession) -> None:
    ...
    async with uow:
        ...
        try:
            ...
            order.add_item(
                OrderItemEntity(id=1, product_id=product.id, quantity=1, price=product.price)
            )
...
async def get_product_repository(session: AsyncSession) -> Repository[ProductEntity, int]:
    """
