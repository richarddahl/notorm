...
class MemcachedCache(DistributedCache):
    ...
    def __init__(
        ...
        if not MEMCACHED_AVAILABLE:
            raise ImportError(
                "Memcached client is not available. Please install it with `pip install pymemcache aiomcache`."
            )
        ...
        async_server = (
            self._parse_host(hosts[0])
            if len(hosts) == 1
            else self._parse_host(random.choice(hosts))
        )
    ...
    async def set_async(self, key: str, value: Any, ttl: Optional[int] = None) -> bool:
        ...
        try:
            ...
            result = await self._async_client.set(
                full_key.encode(), serialized_value, exptime=ttl
            )
        ...
        try:
            ...
            if isinstance(memcached_stats, dict):
                ...
                for key, value in memcached_stats.items():
                    stats[
                        f"memcached_{key.decode() if isinstance(key, bytes) else key}"
                ...
                for i, server_stats in enumerate(memcached_stats):
                    if server_stats and isinstance(server_stats[1], dict):
                        for key, value in server_stats[1].items():
                            stats[
                                f"memcached_{i}_{key.decode() if isinstance(key, bytes) else key}"
                            ] = (value.decode() if isinstance(value, bytes) else value)
    ...
    def multi_set(self, mapping: Dict[str, Any], ttl: Optional[int] = None) -> bool:
        ...
        try:
            # Get full keys
            full_mapping = {
                self._get_full_key(key): value for key, value in mapping.items()
            }
