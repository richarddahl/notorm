            ...
            if invalidate_on_args:
                ...
                for arg_name in invalidate_on_args:
                    if isinstance(arg_name, int):
                        ...
                        if arg_name < len(args) and (
                            args[arg_name] is True or args[arg_name] == "invalidate"
                        ):
                        ...
                        if arg_name in params and (
                            params[arg_name] is True or params[arg_name] == "invalidate"
                        ):
                ...
                for arg_name in invalidate_on_args:
                    if isinstance(arg_name, int):
                        ...
                        if arg_name < len(args) and (
                            ...
                            await cache_manager.invalidate_pattern_async(
                                f"*{cache_key}*"
                            )
                            # Execute the function without caching
                            return await func(*args, **kwargs)
...
def invalidate_cache(key_pattern: str, region: str | None = None):
...
def async_invalidate_cache(key_pattern: str, region: str | None = None):
...
def cache_aside(
    get_from_cache: Callable[[Any], Any], save_to_cache: Callable[[Any, Any], None]
):
