...
class CacheManager:
    ...
    def get(self, key: str, default: Any = None) -> Any:
        ...
        full_key = get_cache_key(
            key,
            self.config.key_prefix,
            self.config.use_hash_keys,
    ...
    def set(self, key: str, value: Any, ttl: Optional[int] = None) -> bool:
        ...
        if self.invalidation_strategy and isinstance(
            self.invalidation_strategy, TimeBasedInvalidation
        ):
        ...
        if self.local_cache:
            try:
                local_ttl = (
                    min(ttl, self.config.local.ttl)
                    if ttl > 0
                    else self.config.local.ttl
                )
        ...
        if self.distributed_cache and self.config.use_multi_level:
            try:
                dist_ttl = (
                    min(ttl, self.config.distributed.ttl)
                    if ttl > 0
                    else self.config.distributed.ttl
                )
            ...
            try:
                ...
                if self.monitor:
                    self.monitor.record_error(
                        "distributed", "invalidate_pattern", str(e)
                    )
    ...
    async def check_health_async(self) -> Dict[str, bool]:
        ...
        if self.distributed_cache:
            try:
                health["distributed"] = (
                    await self.distributed_cache.check_health_async()
                )
            except Exception:
                health["distributed"] = False
    ...
    @contextmanager
    def cache_context(self, region: str | None = None):
        ...
        try:
            if region and region in self.config.regions:
                ...
                merged_config = CacheConfig(
                    **{**original_config.__dict__, **region_config}
                )
    ...
    @asynccontextmanager
    async def cache_context_async(self, region: str | None = None):
