...
class Scheduler:
    ...
    def __init__(
        ...
        self.logger = logger or logging.getLogger(f"uno.jobs.scheduler.{self.instance_id}")
    ...
    async def schedule(
        ...
        await self.storage.schedule_recurring_job(
            ...
            interval_seconds=getattr(schedule_obj, "interval", None) and 
                            getattr(schedule_obj, "interval").total_seconds(),
    ...
    async def trigger_event(
        ...
        for schedule in all_schedules:
            ...
            if schedule.get("type") == "event" and schedule.get("event_name") == event_name:
    ...
    async def get_statistics(self) -> Dict[str, Any]:
        ...
        stats["schedules_active"] = len([s for s in all_schedules if s.get("status") == "active"])
        stats["schedules_paused"] = len([s for s in all_schedules if s.get("status") == "paused"])
    ...
    async def _process_due_jobs(self, limit: int = 100) -> None:
        ...
        for schedule_id, job_data in due_jobs:
            try:
                ...
                self.logger.error(f"Error processing due job for schedule {schedule_id}: {e}")
