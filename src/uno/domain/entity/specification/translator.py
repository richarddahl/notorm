...
class InMemorySpecificationTranslator(SpecificationTranslator[T, list[T]]):
    ...
    def translate(self, specification: Specification[T]) -> list[T]:
        ...
        """
        return lambda entities: [
            e for e in entities if specification.is_satisfied_by(e)
        ]
    ...
    def _translate_attribute_specification(
        ...
        if specification.comparator.__name__ != "eq":
            if specification.comparator.__name__ == "lt":
                return {
                    f"{specification.attribute_name}__lt": specification.expected_value
                }
            elif specification.comparator.__name__ == "lte":
                return {
                    f"{specification.attribute_name}__lte": specification.expected_value
                }
            elif specification.comparator.__name__ == "gt":
                return {
                    f"{specification.attribute_name}__gt": specification.expected_value
                }
            elif specification.comparator.__name__ == "gte":
                return {
                    f"{specification.attribute_name}__gte": specification.expected_value
                }
            elif specification.comparator.__name__ == "contains":
                return {
                    f"{specification.attribute_name}__contains": specification.expected_value
                }
            elif specification.comparator.__name__ == "startswith":
                return {
                    f"{specification.attribute_name}__startswith": specification.expected_value
                }
            elif specification.comparator.__name__ == "endswith":
                return {
                    f"{specification.attribute_name}__endswith": specification.expected_value
                }
