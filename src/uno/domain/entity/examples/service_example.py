...
class ProductService(DomainService[Product, uuid.UUID]):
    ...
    async def get_products_by_category(self, category: str) -> Result[List[Product], str]:
        """
    ...
    async def update_inventory(self, product_id: uuid.UUID, quantity: int) -> Result[Product, str]:
        """
...
class OrderService(DomainServiceWithUnitOfWork[Order, uuid.UUID]):
    ...
    async def get_orders_by_customer(self, customer_id: uuid.UUID) -> Result[List[Order], str]:
        """
...
class OrderingApplicationService(ApplicationService[Any, str]):
    ...
    async def place_order(
        ...
        """
        self.log_request("place_order", {"customer_id": customer_id, "product_ids": product_ids})
    ...
    async def get_available_products_by_category(
        ...
        available_products = [
            ...
            for p in category_products if specification.is_satisfied_by(p)
        ]
        
        self.log_response("get_available_products_by_category", Success(available_products))
...
async def run_example():
    ...
    if result.is_success():
        ...
        for product in result.value:
            print(f"  - {product.name} (${product.price:.2f}, {product.inventory_count} in stock)")
    ...
    if result.is_success():
        ...
        for product in result.value:
            ...
            stock = f"{product.inventory_count} in stock" if product.inventory_count > 0 else "Out of stock"
    ...
    # Update order status
    if (order_result := await order_service.get_orders_by_customer(customer_id)).is_success():
        orders = order_result.value
        if orders:
            update_result = await order_service.update_order_status(orders[0].id, "shipped")
    ...
    if result.is_success():
        print(f"Updated inventory for {result.value.name}: {result.value.inventory_count}")
    ...
    factory = ServiceFactory(
        Product, 
        repository_factory=create_product_repo
    )
