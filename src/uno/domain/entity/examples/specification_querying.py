...
async def setup_repositories():
    ...
    electronics = Category(
        id=uuid4(), name="Electronics", description="Electronic devices and accessories"
    )

    clothing = Category(
        id=uuid4(), name="Clothing", description="Apparel and accessories"
    )

    books = Category(id=uuid4(), name="Books", description="Books and publications")
...
async def run_product_queries(repos):
    ...
    for product in target_products:
        print(
            f"  - {product.name}: ${product.price} (Inventory: {product.inventory_count})"
        )
...
async def run_order_queries(repos):
    ...
    for order in draft_orders:
        print(
            f"  - Order {order.id}: Customer {order.customer_id} - Items: {order.item_count}"
        )
    ...
    for order in recent_orders:
        print(
            f"  - Order {order.id}: {order.order_date.strftime('%Y-%m-%d')} - Status: {order.status.name}"
        )
    ...
    for order in high_value_orders:
        print(
            f"  - Order {order.id}: ${order.total_amount} - Items: {order.item_count}"
        )

    # Find delivered orders with high value
    delivered_high_value_spec = OrderStatusSpecification(OrderStatus.DELIVERED).and_(
        OrderValueSpecification(decimal.Decimal("1000"))
    )
    ...
    for order in delivered_high_value:
        print(
            f"  - Order {order.id}: ${order.total_amount} - Date: {order.order_date.strftime('%Y-%m-%d')}"
        )
...
async def run_customer_queries(repos):
    ...
    for customer in recent_customers:
        print(
            f"  - {customer.name} ({customer.email}) - Last order: {customer.last_order_date.strftime('%Y-%m-%d')}"
        )
