...
from typing import Dict, Any, Optional, List, TypeVar, Generic, Set, ClassVar, Type, cast
    ...
    def safe_post_init(self):
        ...
        for field_name, field_type in orig_annotations.items():
            if not hasattr(self, field_name) or getattr(self, field_name) is None:
                ...
                if field_type == Dict or isinstance(field_type, type) and issubclass(field_type, Dict):
                    setattr(self, field_name, {})
                # Handle list fields
                elif field_type == List or isinstance(field_type, type) and issubclass(field_type, List):
                    setattr(self, field_name, [])
                # Handle set fields
                elif field_type == Set or isinstance(field_type, type) and issubclass(field_type, Set):
...
class Entity(BaseModel, Generic[T_ID]):
    ...
    def __post_init__(self):
        ...
        for field_name, field_value in self.__annotations__.items():
            ...
            # Handle dictionary fields
            if field_value == Dict or isinstance(field_value, type) and issubclass(field_value, Dict):
                setattr(self, field_name, {})
            # Handle list fields
            elif field_value == List or isinstance(field_value, type) and issubclass(field_value, List):
                setattr(self, field_name, [])
            # Handle set fields
            elif field_value == Set or isinstance(field_value, type) and issubclass(field_value, Set):
                setattr(self, field_name, set())
