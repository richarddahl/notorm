...
class GraphPathQuery:
    ...
    async def execute(self, query: PathQuerySpecification) -> Tuple[List[str], QueryMetadata]:
        """
        ...
        metadata = QueryMetadata(
            query_path=query.path_expression,
        ...
        if self.track_performance and self.use_cache:
            ...
            result = await self.performance_tracker.track_query(
                query_key=str(hash(query.path_expression)),
                callback=query_fn
            )
    ...
    def _format_parameter_value(self, value: Any) -> str:
        ...
        if isinstance(value, str):
            ...
            items = [f"{k}: {self._format_parameter_value(v)}" for k, v in value.items()]
            return f"{{{', '.join(items)}}}"
        else:
            # Other types
            return str(value)
