...
from typing import (
    Any, Dict, Generic, List, Optional, Set, Tuple, Type, TypeVar, Union, Protocol,
    cast, Callable
)
...
from uno.core.cqrs import (
    ...
    MessageBus, CommandBus, QueryBus, Mediator, TransactionalCommandHandler
)
...
class ReadModelQueryHandlerBase(QueryHandler[TQuery, TResult], Generic[TQuery, TResult, TReadModel]):
    """
...
class GetByIdQueryHandler(ReadModelQueryHandlerBase[TQuery, Optional[TReadModel], TReadModel]):
    """
    ...
    def convert_result(self, read_model_result: Optional[TReadModel]) -> Optional[TReadModel]:
        """
...
class GraphQueryHandler(ReadModelQueryHandlerBase[TQuery, List[TReadModel], TReadModel]):
    """
    ...
    def to_read_model_query(self, query: TQuery) -> GraphQuery:
        ...
        if not hasattr(query, self.start_node_field):
            raise ValueError(f"Query {query} does not have a {self.start_node_field} field")
...
class EventSourcingReadModelCommandHandler(TransactionalCommandHandler[TCommand, TResult]):
    """
    ...
    def __init__(
        ...
        event_handlers: Optional[Dict[Type[DomainEvent], List[Callable[[DomainEvent], Any]]]] = None,
    ...
    @abstractmethod
    async def execute_command(self, command: TCommand, uow: AbstractUnitOfWork) -> TResult:
        """
...
def register_projection_event_handlers(
    event_bus: Any,
    projections: List[Tuple[Type[DomainEvent], Projection[TReadModel, Any], ReadModelRepository[TReadModel]]],
    config: Optional[ReadModelIntegrationConfig] = None
