...
from typing import Dict, List, Optional, Type, TypeVar, Generic, cast, Any, Set, Callable
...
class UnitOfWork(ABC):
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb) -> None:
        ...
        if exc_type:
            self.logger.error(f"Rolling back transaction due to {exc_type.__name__}: {exc_val}")
            await self.rollback()
            return
