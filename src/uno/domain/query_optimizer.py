...
class QueryPerformanceMetric:
    ...
    def _update_statistics(self) -> None:
        ...
        self.median_time = sorted_times[mid] if len(sorted_times) % 2 == 1 else \
...
class QueryPerformanceTracker:
    ...
    async def track_query(
        ...
        try:
            ...
            self.logger.error(f"Query error: {query_key} - Duration: {duration:.4f}s - Error: {e}")
            
            # Re-raise the exception
            raise
    
    def get_metrics(self, query_key: Optional[str] = None) -> Union[Dict[str, Any], List[Dict[str, Any]]]:
        """
    ...
    def reset_metrics(self, query_key: Optional[str] = None) -> None:
        """
...
class GraphQueryOptimizer:
    ...
    async def update_statistics(self) -> None:
        ...
        try:
            async with async_session() as session:
                ...
                self.logger.info(f"Updated graph statistics: {len(self.cardinality_stats)} entries")
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_update": self.last_stats_update.isoformat() if self.last_stats_update else None,
            "node_count": sum(count for label, count in self.cardinality_stats.items() 
                          if label.startswith("node:")),
            "relationship_count": sum(count for label, count in self.cardinality_stats.items() 
...
class MaterializedQueryView:
    ...
    async def refresh(self, force: bool = False) -> None:
        ...
        needs_refresh = (
            ...
            (now - self.last_refresh) > timedelta(seconds=self.refresh_interval)
        )
        ...
        if needs_refresh and not self.is_refreshing:
            ...
            try:
                ...
                self.logger.error(f"Error refreshing materialized view {self.name}: {e}")
