...
from uno.domain.events import (
    DomainEvent, EventBus, EventStore, EventPublisher, 
    get_event_bus, get_event_store
)
...
class EventStoreIntegration:
    ...
    async def get_events_by_aggregate(
        ...
        """
        try:
            events = await self.event_store.get_events_by_aggregate_id(aggregate_id, event_types)
            return events
        except Exception as e:
    ...
    @classmethod
    def create_event_store(cls, event_type: Type[T], schema: str = "public") -> PostgresEventStore[T]:
        """
...
def get_event_sourced_repository(
    ...
    integration: EventStoreIntegration = None
