            ...
            if self.model_class:
                ...
                if order_by:
                    for field in order_by:
                        if field.startswith("-"):
                            stmt = stmt.order_by(
                                getattr(self.model_class, field[1:]).desc()
                            )
                        else:
            ...
            if isinstance(entity, AggregateRoot):
                if self.model_class:
                    ...
                    stmt = select(self.model_class).where(
                        ...
                        == entity.version - 1,  # Version should be one less
                    )
                    result = await self.session.execute(stmt)
                    model = result.scalars().first()
                    if model is None:
                    ...
                    result = await self.session.execute(
                        stmt, {"id": entity.id, "version": entity.version - 1}
                    )
                    row = result.first()
                    if row is None:
                        raise ConcurrencyError(self._get_entity_type_name(), entity.id)
                ...
                if self.model_class:
                    ...
                    if model is None:
                        raise EntityNotFoundError(
                            self._get_entity_type_name(), entity.id
                        )
...
class SqlAlchemyAggregateRepository(
    AggregateRepository[A], SqlAlchemyRepository[A], Generic[A]
):
    """
...
class InMemoryRepository(Repository[T], Generic[T]):
    ...
    async def update(self, entity: T) -> T:
        ...
        if isinstance(entity, AggregateRoot) and isinstance(
            self.entities[entity.id], AggregateRoot
        ):
...
class InMemoryAggregateRepository(
    AggregateRepository[A], InMemoryRepository[A], Generic[A]
):
    """
    ...
    def __init__(
        self, aggregate_type: Type[A], logger: Optional[logging.Logger] = None
    ):
        """
