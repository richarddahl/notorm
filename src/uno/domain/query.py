...
class QueryExecutor(Generic[T, Q], ABC):
    ...
    def project(self, entity: T, include: Optional[List[str]] = None, exclude: Optional[List[str]] = None) -> Dict[str, Any]:
        """
...
class FilterQueryExecutor(QueryExecutor[T, Q]):
    ...
    async def execute(self, query: Q) -> QueryResult[T]:
        ...
        return QueryResult(
            ...
            total_pages=(len(filter_results.ids) + query.limit - 1) // query.limit if query.limit else 1
        )
...
class QueryService(Generic[T, Q]):
    ...
    async def query(self, query_params: Union[Q, Dict[str, Any]]) -> QueryResult[T]:
        ...
        query = query_params if isinstance(query_params, self.query_type) else self.query_type(**query_params)
