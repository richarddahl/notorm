...
import time
import hashlib
from typing import Dict, List, Any, Optional, TypeVar, Generic, Union, Set, Type, Tuple, Callable, Awaitable
...
class EnhancedQueryExecutor(Generic[T, Q]):
    ...
    async def execute(self, query: Q) -> Tuple[QueryResult[T], QueryMetadata]:
        ...
        metadata = QueryMetadata(
            query_path=self.entity_type.__name__,
        ...
        if cache_key in self.materialized_views:
            ...
            metadata.complete_execution(
                record_count=len(results.items),
