...
class DomainRouter(Generic[T, SvcT]):
    ...
    def _generate_schemas(self) -> None:
        ...
        if not self.create_dto:
            self.create_dto = create_model(f"{entity_name}Create", **field_info)
        ...
        if not self.update_dto:
            ...
            self.update_dto = create_model(f"{entity_name}Update", **update_field_info)
    ...
    def _register_endpoints(self) -> None:
        ...
        async def get_entity(
            id: str = Path(
                ..., description=f"The ID of the {self.entity_type.__name__}"
            ),
            service: SvcT = Depends(lambda: get_service(self.service_type)),
        ):
            """Get an entity by ID."""
            ...
            if isinstance(result, Result):
                ...
                raise HTTPException(
                    status_code=404, detail=f"{self.entity_type.__name__} not found"
                )
            return result.to_dict()
        ...
        async def delete_entity(
            ...
            if isinstance(result, Result):
                ...
                return {
                    "success": True,
                    "message": f"{self.entity_type.__name__} deleted",
                }
            elif not result:
                raise HTTPException(
                    status_code=404, detail=f"{self.entity_type.__name__} not found"
    ...
    def add_custom_endpoint(
        self, path: str, method: str, endpoint_function, response_model=None, **kwargs
    ):
        """
...
def domain_endpoint(
    entity_type: Type[T] = None, service_type: Type[SvcT] = None, response_model=None
):
    """
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            if isinstance(result, Result):
                if result.is_failure:
                    status_code = (
                        404 if "not found" in str(result.error).lower() else 400
                    )
                    raise HTTPException(
                        status_code=status_code, detail=str(result.error)
                    )
                return result.value
