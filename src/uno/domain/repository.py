...
class Repository(Generic[T], ABC):
    ...
    @abstractmethod
    async def get(
        self, id: str, load_relations: Optional[Union[bool, List[str]]] = None
    ) -> Optional[T]:
        """
        Get an entity by ID.
    ...
    async def load_relations(
        self, entity: T, relation_names: Optional[Union[bool, List[str]]] = True
    ) -> T:
        """
    ...
    async def load_relations_batch(
        self, entities: List[T], relation_names: Optional[Union[bool, List[str]]] = True
    ) -> List[T]:
        """
...
class UnoDBRepository(Repository[T]):
        ...
        if self._batch_ops is None and self.use_batch_operations:
            ...
            try:
                ...
                try:
                    ...
                    # Get the model class
                    model_class = getattr(
                        self.entity_type, "__uno_model__", self.entity_type
                    )
        ...
        if filters:
            ...
            for key, value in filters.items():
                if isinstance(value, dict) and "lookup" in value and "val" in value:
                    filter_params.append(
                        FilterParam(label=key, lookup=value["lookup"], val=value["val"])
                    )
                else:
                    filter_params.append(FilterParam(label=key, lookup="eq", val=value))
            ...
            if order_by:
                for field in order_by:
                    if field.startswith("-"):
                        filter_params.append(
                            FilterParam(label="order_by", lookup="desc", val=field[1:])
                        )
                    else:
                        filter_params.append(
                            FilterParam(label="order_by", lookup="asc", val=field)
                        )

            # Add pagination parameters if provided
            if limit is not None:
                filter_params.append(FilterParam(label="limit", lookup="eq", val=limit))

            if offset is not None:
                filter_params.append(
                    FilterParam(label="offset", lookup="eq", val=offset)
                )

        try:
            # Pass the select_related parameter to control which relationships to eager load
            if load_relations is not None:
                results = await self.db.filter(
                    filter_params, select_related=load_relations
                )
            else:
            ...
            if (
                load_relations
                and not getattr(self.db, "supports_joins", True)
                and entities
            ):
        ...
        try:
            ...
            if not await self.exists(entity.id):
                raise DomainException(
                    f"Entity with ID {entity.id} not found", "NOT_FOUND"
                )
            ...
            if isinstance(e, UniqueViolationError):
                raise DomainException(
                    "Unique constraint violation", "CONSTRAINT_VIOLATED"
                )
            if isinstance(e, NotFoundException):
                raise DomainException(
                    f"Entity with ID {entity.id} not found", "NOT_FOUND"
    ...
    async def batch_update(
            ...
            for entity in entities:
                try:
                    ...
                    logging.getLogger(__name__).error(
                        f"Error updating entity {entity.id}: {e}"
                    )
            return count
        ...
        for entity in entities:
            ...
            if fields:
                model_data = {
                    k: v for k, v in model_data.items() if k in fields or k == "id"
                }
    ...
    async def _load_relationship(self, entity: T, relation_name: str) -> T:
        ...
        try:
            ...
            if not rel_meta["is_collection"] and hasattr(
                entity, f"{rel_meta['field']}_id"
            ):
                fk_value = getattr(entity, f"{rel_meta['field']}_id")
                if fk_value:
                    # Create a repository for the target type
                    target_repo = UnoDBRepository(
                        rel_meta["target_type"], self.db_factory
                    )
            ...
            # Handle to-many relationships (foreign key in related entity)
                ...
                fk_field = (
                    ...
                    or f"{self.entity_type.__name__.lower()}_id"
                )
    ...
    async def _load_relationship_batch(
        self, entities: List[T], relation_name: str
    ) -> List[T]:
        ...
        try:
            ...
            if not rel_meta["is_collection"]:
                ...
                related_entities = await target_repo.list(
                    filters={"id": {"lookup": "in", "val": fk_values}}
                )
            ...
            # Handle to-many relationships (foreign key in related entity)
                ...
                related_entities = await target_repo.list(
                    filters={fk_field: {"lookup": "in", "val": entity_ids}}
                )
    ...
    def _convert_to_model_data(self, entity: T) -> Dict[str, Any]:
        ...
        exclude_fields = {"events", "child_entities"}
...
class LazyRelationshipProxy:
    ...
    async def load(self):
        ...
        setattr(
            self.entity,
            self.attr_name,
            getattr(loaded_entity, self.relation_name, None),
        )
        self._loaded = True
