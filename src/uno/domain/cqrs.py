...
class QueryHandler(Generic[QueryT, OutputT], ABC):
    ...
    def __init__(
        self, query_type: Type[QueryT], logger: Optional[logging.Logger] = None
    ):
        """
...
class Dispatcher:
    ...
    def register_command_handler(self, handler: CommandHandler) -> None:
        ...
        self.logger.debug(
            f"Registered command handler for {handler.command_type.__name__}"
        )
    ...
    async def dispatch_command(self, command: Command) -> CommandResult:
        ...
        if not handler:
            self.logger.error(
                f"No handler registered for command {command.__class__.__name__}"
            )
            return CommandResult.rejection(
                command_id=command.command_id,
                command_type=command.__class__.__name__,
    ...
    async def dispatch_query(self, query: Query) -> QueryResult:
        ...
        if not handler:
            self.logger.error(
                f"No handler registered for query {query.__class__.__name__}"
            )
            return QueryResult.failure(
                query_id=query.query_id,
                query_type=query.__class__.__name__,
