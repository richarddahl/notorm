...
async def basic_authorization_example():
    ...
    # Check authorization
    print("Admin Context:")
    print(f"  Can read products: {await auth_service.authorize(admin_context, 'products', 'read')}")
    print(f"  Can write products: {await auth_service.authorize(admin_context, 'products', 'write', product)}")
    print(f"  Can delete products: {await auth_service.authorize(admin_context, 'products', 'delete')}")
    
    print("\nUser Context:")
    print(f"  Can read products: {await auth_service.authorize(user_context, 'products', 'read')}")
    print(f"  Can write products: {await auth_service.authorize(user_context, 'products', 'write', product)}")
    ...
    print(f"  Can write own products: {await auth_service.authorize(user_context, 'products', 'write', user_product)}")
...
async def rbac_example():
    ...
    manager_role = rbac_service.create_role("manager", [
    ...
    print(f"  Can read products: {rbac_service.has_permission('admin', 'products:read')}")
    print(f"  Can write products: {rbac_service.has_permission('admin', 'products:write')}")
    print(f"  Can delete products: {rbac_service.has_permission('admin', 'products:delete')}")
    
    print("\nManager User:")
    print(f"  Has manager role: {manager_user.has_role('manager')}")
    print(f"  Can read products: {rbac_service.has_permission('manager', 'products:read')}")
    print(f"  Can write products: {rbac_service.has_permission('manager', 'products:write')}")
    print(f"  Can delete products: {rbac_service.has_permission('manager', 'products:delete')}")
    
    print("\nRegular User:")
    print(f"  Has user role: {user1.has_role('user')}")
    print(f"  Can read products: {rbac_service.has_permission('user1', 'products:read')}")
    print(f"  Can write products: {rbac_service.has_permission('user1', 'products:write')}")
    
    print("\nUser with Extra Permission:")
    print(f"  Has user role: {user2.has_role('user')}")
    print(f"  Can read products: {rbac_service.has_permission('user2', 'products:read')}")
    ...
    print(f"  Can read products: {await auth_service.authorize(manager_context, 'products', 'read')}")
    print(f"  Can write products: {await auth_service.authorize(manager_context, 'products', 'write', product)}")
    print(f"  Can delete products: {await auth_service.authorize(manager_context, 'products', 'delete')}")
...
async def multi_tenant_example():
    ...
    print(f"  Can access tenant1 product: {await mt_auth_service.authorize(global_admin_context, 'products', 'read', product_tenant1)}")
    print(f"  Can access tenant2 product: {await mt_auth_service.authorize(global_admin_context, 'products', 'read', product_tenant2)}")
    
    print("\nTenant 1 Admin:")
    tenant1_admin_context = tenant_rbac.create_service_context("tenant1_admin", "tenant1")
    print(f"  Can access tenant1 product: {await mt_auth_service.authorize(tenant1_admin_context, 'products', 'read', product_tenant1)}")
    print(f"  Can access tenant2 product: {await mt_auth_service.authorize(tenant1_admin_context, 'products', 'read', product_tenant2)}")
    
    print("\nTenant 2 Admin:")
    tenant2_admin_context = tenant_rbac.create_service_context("tenant2_admin", "tenant2")
    print(f"  Can access tenant1 product: {await mt_auth_service.authorize(tenant2_admin_context, 'products', 'read', product_tenant1)}")
    print(f"  Can access tenant2 product: {await mt_auth_service.authorize(tenant2_admin_context, 'products', 'read', product_tenant2)}")
    
    print("\nMulti-Tenant User:")
    # Check in tenant 1 context
    mt_user_context1 = tenant_rbac.create_service_context("multi_tenant_user", "tenant1")
    print(f"  In tenant1 context, can access tenant1 product: {await mt_auth_service.authorize(mt_user_context1, 'products', 'read', product_tenant1)}")
    print(f"  In tenant1 context, can access tenant2 product: {await mt_auth_service.authorize(mt_user_context1, 'products', 'read', product_tenant2)}")
    
    # Check in tenant 2 context
    mt_user_context2 = tenant_rbac.create_service_context("multi_tenant_user", "tenant2")
    print(f"  In tenant2 context, can access tenant1 product: {await mt_auth_service.authorize(mt_user_context2, 'products', 'read', product_tenant1)}")
    print(f"  In tenant2 context, can access tenant2 product: {await mt_auth_service.authorize(mt_user_context2, 'products', 'read', product_tenant2)}")
...
class ExampleServiceContext(ServiceContext):
    ...
    @classmethod
    def from_rbac(cls, user_id: str, rbac_service: RbacService) -> 'ExampleServiceContext':
    ...
    @classmethod
    def from_tenant_rbac(cls, user_id: str, tenant_id: str, rbac_service: TenantRbacService) -> 'ExampleServiceContext':
...
async def application_integration_example():
    ...
    print(f"  Can read products: {await auth_service.authorize(admin_context, 'products', 'read')}")
    print(f"  Can write own products: {await auth_service.authorize(admin_context, 'products', 'write', admin_product)}")
    print(f"  Can write user products: {await auth_service.authorize(admin_context, 'products', 'write', user_product)}")
    
    print("\nUser Authorization:")
    print(f"  Can read products: {await auth_service.authorize(user_context, 'products', 'read')}")
    print(f"  Can write own products: {await auth_service.authorize(user_context, 'products', 'write', user_product)}")
    print(f"  Can write admin products: {await auth_service.authorize(user_context, 'products', 'write', admin_product)}")
