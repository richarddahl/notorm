...
async def example_query_usage() -> None:
    ...
    query_executor.create_materialized_view(
        ...
        query=BaseModel(
            filters={"category_id": "electronics", "is_active": True}, order_by=["name"]
        ),
...
async def example_path_query_usage() -> None:
    ...
    path_query = GraphPathQuery(
        track_performance=True, use_cache=True, cache_ttl=300, logger=logger
    )
    ...
    products, metadata = await path_query_service.query_entities(
        query=string_path_query, repository=repository, entity_type=Product
    )
    ...
    path_query = PathQuerySpecification(
        path=query_path, params={"t.name": "Electronics"}, limit=10
    )

    # Execute the query
    products, metadata = await path_query_service.query_entities(
        query=path_query, repository=repository, entity_type=Product
    )
...
async def example_complex_path_query() -> None:
    ...
    # Execute the query
    products, metadata = await path_query_service.query_entities(
        query=query, repository=repository, entity_type=Product
    )
