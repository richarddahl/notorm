...
from uno.domain.specifications.base import (
    ...
    AttributeSpecification, PredicateSpecification, DictionarySpecification
)
from uno.domain.specifications.enhanced import (
    RangeSpecification, DateRangeSpecification, TextMatchSpecification,
    InListSpecification, NotInListSpecification, ComparableSpecification,
    NullSpecification, NotNullSpecification, RelativeDateSpecification,
    CollectionSizeSpecification, CollectionContainsSpecification,
    UUIDSpecification, JsonPathSpecification, HasAttributeSpecification
)
...
class PostgreSQLSpecificationTranslator(Generic[T]):
    ...
    def _translate_range_specification(
        ...
        min_condition = column > specification.min_value if not specification.include_min else column >= specification.min_value
        max_condition = column < specification.max_value if not specification.include_max else column <= specification.max_value
    ...
    def _translate_date_range_specification(
        ...
        start_condition = column > specification.start_date if not specification.include_start else column >= specification.start_date
        end_condition = column < specification.end_date if not specification.include_end else column <= specification.end_date
    ...
    def _translate_relative_date_specification(
        ...
        delta = timedelta(days=specification.days, hours=specification.hours, minutes=specification.minutes)
