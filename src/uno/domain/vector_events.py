...
class VectorContentEvent(UnoDomainEvent):
    ...
    @classmethod
    def from_entity(
        cls, entity: Entity, content_fields: List[str], operation: str = "update"
    ) -> "VectorContentEvent":
        """
...
class VectorEventHandler(EventSubscriber):
    ...
    async def _handle_entity_updated(self, event: EntityUpdatedEvent) -> None:
        ...
        changed_vectorizable_fields = [
            field for field in event.changed_fields if field in content_fields
        ]
...
class VectorUpdateHandler(EventSubscriber):
    ...
    def __init__(
        self, dispatcher: EventDispatcher, logger: Optional[logging.Logger] = None
    ):
        """
    ...
    async def handle_vector_content_event(self, event: VectorContentEvent) -> None:
        ...
        try:
            ...
            if event.operation == "delete":
                ...
                await self._update_embedding(event.entity_type, event.entity_id, None)
                return
    ...
    @domain_event_handler()
    async def handle_embedding_update_requested(
        self, event: VectorEmbeddingUpdateRequested
    ) -> None:
        """
        ...
        try:
            ...
            success = await self._update_embedding(
                event.entity_type, event.entity_id, event.content
            )
    ...
    async def _update_embedding(
        self, entity_type: str, entity_id: str, content: Optional[str]
    ) -> bool:
        """
    ...
    def setup_entity_triggers(
        ...
        def patched_post_init(self, __context):
            ...
            if self.created_at and (
                self.updated_at is None or self.created_at == self.updated_at
            ):
