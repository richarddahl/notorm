...
class EventDispatcher(Generic[E]):
    ...
    def __init__(
        self,
        event_store: Optional[EventStore[E]] = None,
    ...
    def subscribe(self, event_type: str, handler: EventHandler) -> None:
        ...
        self.logger.debug(
            f"Handler {handler.__name__} subscribed to event type {event_type}"
        )
    ...
    def unsubscribe(self, event_type: str, handler: EventHandler) -> None:
        ...
        self.logger.debug(
            f"Handler {handler.__name__} unsubscribed from event type {event_type}"
        )
    ...
    async def publish(self, event: E) -> None:
        ...
        """
        self.logger.debug(
            f"Publishing event {event.event_type} with ID {event.event_id}"
        )
        ...
        for handler in handlers:
            ...
            async def safe_execute(h: EventHandler, e: E) -> None:
                try:
                    ...
                    self.logger.error(
                        f"Error in event handler {h.__name__} for event {e.event_type}: {ex}"
                    )
        ...
        self.logger.debug(
            f"Event {event.event_type} with ID {event.event_id} published to {len(handlers)} handlers"
        )
