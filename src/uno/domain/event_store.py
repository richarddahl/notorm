            ...
            async with async_session() as session:
                ...
                try:
                    rows = (
                        ...
                        if hasattr(result.fetchall, "__await__")
...
class EventSourcedRepository(Generic[E]):
    ...
    def _apply_event(self, aggregate: AggregateRoot, event: DomainEvent) -> None:
        ...
        if hasattr(aggregate, handler_name) and callable(
            getattr(aggregate, handler_name)
        ):
            try:
                ...
                self.logger.error(
                    f"Error applying event {event.event_type} to aggregate {aggregate.id}: {e}"
                )
                raise
        else:
            self.logger.warning(
                f"No handler found for event type {event.event_type} on aggregate {type(aggregate).__name__}"
            )
