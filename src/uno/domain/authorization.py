...
class Permission:
    ...
    def from_string(cls, permission_string: str) -> "Permission":
        ...
        if ":" not in permission_string:
            raise ValueError(
                f"Invalid permission format: {permission_string}. Expected format: resource:action"
            )
    ...
    async def _authorize_internal(
        ...
        for policy in self.policies:
            ...
            if self.mode == self.CombinationMode.ANY and result:
                self.logger.info(
                    f"ANY mode: {policy.__class__.__name__} passed, authorizing"
                )
                return True
            if self.mode == self.CombinationMode.ALL and not result:
                self.logger.info(
                    f"ALL mode: {policy.__class__.__name__} failed, denying"
                )
                return False
...
class AuthorizationService:
    ...
    async def authorize(
        ...
        if policy is None:
            self.logger.debug(
                f"No policy registered for {resource}:{action}, checking permissions only"
            )
