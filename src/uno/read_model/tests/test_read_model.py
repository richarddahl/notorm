...
from uno.read_model.cache_service import ReadModelCache, InMemoryReadModelCache
...
class TestProjection(Projection[TestReadModel, TestEvent]):
    ...
    async def apply(self, event: TestEvent) -> Optional[TestReadModel]:
        ...
        return TestReadModel(
            id=event.entity_id, entity_id=event.entity_id, data=event.data
        )
...
async def test_read_model_repository(repository):
    ...
    # Create a read model
    model = TestReadModel(id="test-1", entity_id="test-1", data="test data")
