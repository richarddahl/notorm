...
from uno.authorization.objs import User, Role

from uno.workflows.models import (
    WorkflowConditionType,
    WorkflowConditionModel
)
from uno.workflows.objs import WorkflowCondition
    ...
    async def evaluate(
        ...
        try:
            ...
            if not config or not config.get("field"):
                return Failure(ConditionError("Field value condition missing field configuration"))
            ...
            if operator == ComparisonOperator.IS_EMPTY:
                return Success(actual_value == "" or actual_value == [] or actual_value == {})
            
            if operator == ComparisonOperator.IS_NOT_EMPTY:
                return Success(not (actual_value == "" or actual_value == [] or actual_value == {}))
            ...
            if operator == ComparisonOperator.EQUAL:
                ...
                if isinstance(expected_value, list) and len(expected_value) == 2:
                    ...
                    return Failure(ConditionError(f"Between operator requires a list of two values, got: {expected_value}"))
            else:
            ...
            return Failure(ConditionError(f"Error evaluating field value condition: {str(e)}"))
...
class TimeBasedEvaluator:
    ...
    async def evaluate(
        ...
        try:
            ...
            if not config or not config.get("operator"):
                return Failure(ConditionError("Time-based condition missing operator configuration"))
            ...
            if operator == TimeOperator.BEFORE:
                ...
                if not target_time:
                    return Failure(ConditionError("Before operator requires a datetime value"))
                ...
                target_time = self._parse_datetime(config.get("datetime"))
                if not target_time:
                    return Failure(ConditionError("After operator requires a datetime value"))
                ...
                if not start_time or not end_time:
                    return Failure(ConditionError("Between operator requires start_datetime and end_datetime values"))
                ...
                if not weekdays:
                    return Failure(ConditionError("On-weekday operator requires weekdays value"))
                ...
                if not reference_time:
                    return Failure(ConditionError("Recurring operator requires reference_datetime value"))
                ...
                intervals_passed = elapsed.total_seconds() / interval_delta.total_seconds()
                # Check if it's close to a whole number of intervals
                is_on_interval = abs(intervals_passed - round(intervals_passed)) * interval_delta.total_seconds() <= tolerance.total_seconds()
            ...
            return Failure(ConditionError(f"Error evaluating time-based condition: {str(e)}"))
    ...
    def _is_business_hours(self, dt: datetime) -> bool:
        ...
        if dt.weekday() not in self.business_hours.get("weekdays", [0, 1, 2, 3, 4]):  # Default M-F
            return False
...
class RoleBasedEvaluator:
    ...
    async def evaluate(
        ...
        try:
            config = condition.condition_config
            if not config:
                return Failure(ConditionError("Role-based condition missing configuration"))
            ...
            user_id_fields = ["user_id", "userId", "created_by", "modified_by", "owner_id", "ownerId"]
            ...
            if not user_id:
                self.logger.warning("No user ID found for role-based condition evaluation")
                return Success(False)
            ...
            return Failure(ConditionError("Role-based condition missing roles or permission configuration"))
                
        except Exception as e:
            self.logger.exception(f"Error evaluating role-based condition: {e}")
            return Failure(ConditionError(f"Error evaluating role-based condition: {str(e)}"))
...
class QueryMatchEvaluator:
    ...
    async def evaluate(
        ...
        try:
            ...
            return Failure(ConditionError(f"Error evaluating query match condition: {str(e)}"))
    ...
    async def evaluate(
        ...
        try:
            ...
            if not evaluator_type:
                return Failure(ConditionError("No evaluator type provided for custom condition"))
            
            # Find the corresponding evaluator function
            evaluator_func = self.custom_evaluators.get(evaluator_type)
            if not evaluator_func:
                return Failure(ConditionError(f"No evaluator found for custom condition type: {evaluator_type}"))
            ...
            return Failure(ConditionError(f"Error evaluating custom condition: {str(e)}"))
    ...
    async def evaluate(
        ...
        try:
            ...
            if not subconditions:
                return Failure(ConditionError("Composite condition has no subconditions"))
            
            # For NOT operator, there should be only one subcondition
            if operator == LogicalOperator.NOT and len(subconditions) != 1:
                return Failure(ConditionError("NOT operator should have exactly one subcondition"))
            ...
            for subcond in subconditions:
                ...
                subcond_obj = WorkflowCondition(
                    ...
                    condition_type=subcond.get("type", WorkflowConditionType.FIELD_VALUE),
                ...
                if not evaluator:
                    self.logger.warning(f"No evaluator found for condition type: {subcond_obj.condition_type}")
                ...
                if result.is_failure:
                    self.logger.warning(f"Failed to evaluate subcondition: {result.error}")
                    results.append(False)
                else:
            ...
            if operator == LogicalOperator.AND:
                ...
                return Failure(ConditionError(f"Unsupported logical operator: {operator}"))
                
        except Exception as e:
            self.logger.exception(f"Error evaluating composite condition: {e}")
            return Failure(ConditionError(f"Error evaluating composite condition: {str(e)}"))
...
class ConditionEvaluatorRegistry:
    ...
    def get(self, condition_type: WorkflowConditionType) -> Optional[ConditionEvaluator]:
...
def get_evaluator(condition_type: WorkflowConditionType) -> Optional[ConditionEvaluator]:
