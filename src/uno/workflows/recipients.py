...
from uno.workflows.objs import WorkflowRecipient
...
class AttributeResolver:
    ...
    async def resolve(
        ...
        try:
            ...
            if len(parts) != 2:
                return Failure(RecipientError(f"Invalid attribute specification: {attribute_spec}. Format should be attribute_name:attribute_value"))
            ...
            if not users:
                self.logger.warning(f"No users found with attribute {attribute_name}:{attribute_value}")
                return Success([])
            ...
            return Failure(RecipientError(f"Error resolving attribute recipient: {str(e)}"))
    
    async def _get_users_by_attribute(self, attribute_name: str, attribute_value: str) -> List[User]:
        ...
        try:
            ...
            self.logger.error(f"Error fetching users for attribute {attribute_name}:{attribute_value}: {e}")
            return []
        ...
        try:
            ...
            if target_type == "user":
                ...
                if result.is_failure:
                    return Failure(RecipientError(f"Error executing query: {result.error}"))
                ...
                return Failure(RecipientError(f"Unsupported target type: {target_type}"))
    ...
    async def resolve(
        ...
        try:
            ...
            if not resolver_func:
                return Failure(RecipientError(f"No resolver found for dynamic recipient type: {resolver_type}"))
                ...
                for item in result:
                    if isinstance(item, User):
                        ...
                        users.append(User(
                            ...
                            email=item.get("email", f"user_{item['id']}@example.com"),
            ...
            return Failure(RecipientError(f"Dynamic resolver returned unrecognized result type: {type(result)}"))
            
        except Exception as e:
            self.logger.exception(f"Error resolving dynamic recipient: {e}")
            return Failure(RecipientError(f"Error resolving dynamic recipient: {str(e)}"))
...
class RecipientResolverRegistry:
    ...
    def get(self, recipient_type: Union[WorkflowRecipientType, str]) -> Optional[RecipientResolver]:
...
def get_resolver(recipient_type: Union[WorkflowRecipientType, str]) -> Optional[RecipientResolver]:
