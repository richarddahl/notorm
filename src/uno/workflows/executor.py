...
from uno.workflows.objs import WorkflowAction
...
class EmailExecutor:
    ...
    async def execute(
        ...
        try:
            ...
            subject = config.get("subject", f"Notification from {context.workflow_name}")
            ...
            for recipient in recipients:
                ...
                if not recipient_email:
                    self.logger.warning(f"No email address for recipient {recipient.id}")
    ...
    async def _load_email_template(self, template_name: str) -> str:
        ...
        try:
            ...
            try:
                ...
                self.logger.warning(f"Email template {template_name} not found at {template_path}")
                return ""
...
class WebhookExecutor:
    ...
    async def execute(
        ...
        try:
            ...
            if payload_fields:
                payload = {field: context.event_data.get(field) for field in payload_fields if field in context.event_data}
            else:
    ...
    async def execute(
        ...
        try:
            ...
            if not field_mappings:
                return Failure(UnoError("No field mappings provided for database action"))
            ...
            async with self.db_manager.get_enhanced_session() as session:
                if operation == "INSERT":
                    ...
                    if not record_id and not filter_condition:
                        return Failure(UnoError("No record ID or filter condition provided for UPDATE operation"))
                    
                    # Generate SQL for update
                    set_clause = ", ".join([f"{field} = :{field}" for field in record_data.keys()])
                    
                    if record_id:
                        # If record_id is a template with variables, interpolate it
                        if isinstance(record_id, str) and "{{" in record_id and "}}" in record_id:
                        ...
                        if isinstance(filter_condition, str) and "{{" in filter_condition and "}}" in filter_condition:
                    ...
                    self.logger.info(
                        f"Updated {row_count} records in {target_table}"
                    )
                    ...
                    return Failure(UnoError(f"Unsupported database operation: {operation}"))
    ...
    async def execute(
        ...
        try:
            ...
            if not executor_func:
                return Failure(UnoError(f"No executor found for custom action type: {executor_type}"))
            ...
            self.logger.info(
                f"Executed custom action '{executor_type}'"
            )
