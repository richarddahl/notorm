...
async def setup_dynamic_recipient_resolver():
    ...
    if not dynamic_resolver or not isinstance(dynamic_resolver, DynamicRecipientResolver):
...
async def trigger_workflows_with_events():
    ...
    print(f"Dispatched significant price change event for product '{big_price_change_event.name}'")
...
async def run_advanced_targeting_example():
    ...
    try:
        ...
        logger.info("5. Triggered workflows with events to demonstrate advanced condition evaluation")
