...
from uno.core.base.error import ErrorCategory, ErrorSeverity
...
class SchemaNotFoundError(BaseError):
    ...
    def __init__(self, schema_name: str, message: Optional[str] = None, **context: Any):
...
class SchemaFieldTypeMismatchError(BaseError):
    ...
    def __init__(
        ...
        **context: Any,
        ...
        message = (
            message
            or f"Field '{field_name}' type mismatch: expected {expected_type_str}, got {actual_type_str}"
        )
        super().__init__(
            message=message,
