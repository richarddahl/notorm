...
class CacheMonitor:
    ...
    def record_error(self, cache_type: str, operation: str, error_message: str) -> None:
        ...
        with self._lock:
            ...
            if log_level == "DEBUG":
                logger.debug(f"Cache error ({cache_type}, {operation}): {error_message}")
            elif log_level == "INFO":
                logger.info(f"Cache error ({cache_type}, {operation}): {error_message}")
            elif log_level == "WARNING":
                logger.warning(f"Cache error ({cache_type}, {operation}): {error_message}")
            else:
                logger.error(f"Cache error ({cache_type}, {operation}): {error_message}")
    
    def record_latency(self, cache_type: str, operation: str, latency_ms: float) -> None:
        ...
        with self._lock:
            ...
            if latency_ms > self._latency_threshold:
                logger.warning(f"Cache latency threshold exceeded: {cache_type}.{operation} = {latency_ms} ms")
    ...
    def record_metric(self, name: str, value: Union[int, float, str], labels: Optional[Dict[str, str]] = None) -> None:
    ...
    def analyze_performance(self, time_window: Optional[float] = None) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            events = [event for event in self._events if event.timestamp >= start_time]
    ...
    async def analyze_performance_async(self, time_window: Optional[float] = None) -> Dict[str, Any]:
    ...
    def _add_event(self, cache_name: str, event_type: str, details: Optional[Dict[str, Any]] = None) -> None:
        ...
        if "avg_latency" in self._stats:
            for cache_type, operations in self._stats["avg_latency"].items():
                for operation, latency in operations.items():
                    ...
                    if alerts[alert_name]:
                        logger.warning(f"High latency alert: {cache_type}.{operation} = {latency:.2f} ms")
        ...
        if "size" in self._stats and "local" in self._stats["size"]:
            ...
            if self.local_cache and hasattr(self.local_cache, "max_size"):
                ...
                if max_size > 0:
                    ...
                    if alerts[alert_name]:
                        logger.warning(f"High memory usage alert: local = {memory_usage:.2f}")
    ...
    def _setup_prometheus_exporter(self) -> None:
        ...
        try:
            ...
            # Define metrics
            self._prom_hits = Counter("cache_hits_total", "Total number of cache hits", ["cache_type"])
            self._prom_misses = Counter("cache_misses_total", "Total number of cache misses", ["cache_type"])
            self._prom_errors = Counter("cache_errors_total", "Total number of cache errors", ["cache_type"])
            
            self._prom_hit_rate = Gauge("cache_hit_rate", "Cache hit rate", ["cache_type"])
