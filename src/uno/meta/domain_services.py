...
class MetaTypeService(UnoEntityService[MetaType]):
    ...
    async def register_meta_type(self, id: str, name: Optional[str] = None, description: Optional[str] = None) -> Result[MetaType]:
        """
    ...
    async def get_with_records(self, id: str, record_limit: int = 100) -> Result[MetaType]:
        """
        ...
        try:
            ...
            return Failure(MetaServiceError(f"Error getting meta type with records: {str(e)}"))
...
class MetaRecordService(UnoEntityService[MetaRecord]):
    ...
    async def create_record(self, meta_type_id: str, record_id: Optional[str] = None) -> Result[MetaRecord]:
        """
        ...
        try:
            ...
            if self.meta_type_service:
                ...
                if meta_type_result.is_failure:
                    return Failure(MetaServiceError(f"Meta type validation failed: {meta_type_result.error}"))
                
                if not meta_type_result.value:
                    return Failure(MetaServiceError(f"Meta type {meta_type_id} not found"))
    ...
    async def find_by_meta_type(self, meta_type_id: str, limit: int = 100) -> Result[List[MetaRecord]]:
        """
    ...
    async def get_with_meta_type(self, id: str) -> Result[MetaRecord]:
        ...
        try:
            ...
            meta_type_repo = None
            if self.meta_type_service:
                meta_type_repo = cast(MetaTypeRepository, self.meta_type_service.repository)
    ...
    async def add_attribute(self, meta_record_id: str, attribute_id: str) -> Result[MetaRecord]:
        """
        ...
        try:
            ...
            if not meta_record:
                return Failure(MetaServiceError(f"Meta record {meta_record_id} not found"))
