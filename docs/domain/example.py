...
async def create_user(
    username: str, email: str, address: Optional[Address] = None
) -> User:
    """
    Create a new user.
...
async def run_example():
    ...
    user = await create_user(
        username="johndoe", email="john@example.com", address=address
    )
    ...
    # Change the user's address
    new_address = Address(
        street="456 Oak Ave", city="Othertown", state="NY", zip_code="67890"
    )
    ...
    print(
        f"Updated user address to {updated_user.address.city}, {updated_user.address.state}"
    )

    # Get the user by ID
    retrieved_user = await user_service.get_by_id(user.id)
    print(
        f"Retrieved user: {retrieved_user.username} from {retrieved_user.address.city}"
    )
