...
def schema_manager_example():
    ...
    user_list = user_list_schema(
        items=[view_user, edit_user], total=2, page=1, page_size=10, pages=1
    )

    return {"view_user": view_user, "edit_user": edit_user, "user_list": user_list}
...
def validate_user_data(user_data: Dict[str, Any]) -> ValidationContext:
    ...
    if "age" in user_data:
        ...
        if not isinstance(age, int):
            ...
            context.add_error(
                "age", "Age must be between 18 and 120", "INVALID_AGE_RANGE"
            )
        ...
        if not isinstance(address, dict):
            ...
            if not address.get("country"):
                address_context.add_error(
                    "country", "Country is required", "REQUIRED_FIELD"
                )

    # Business rule validation
    if user_data.get("role") == "admin" and user_data.get("age", 0) < 21:
        context.add_error(
            "role", "Admin users must be at least 21 years old", "INVALID_ROLE_FOR_AGE"
        )
...
def demonstrate_schemas():
    ...
    try:
        ...
        UserWithValidation(
            username="johndoe", email="john@example.com", age=16, score=95.5
        )
    except Exception as e:
    ...
    if context.has_errors():
        ...
        for error in context.errors:
            print(
                f"- Field: {error['field']}, Message: {error['message']}, Code: {error['error_code']}"
            )
