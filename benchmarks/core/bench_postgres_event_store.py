...
class PostgresEventStoreAppendBenchmark(Benchmark):
    ...
    description = (
        "Measures the performance of appending events to the PostgreSQL event store"
    )
...
class PostgresEventStoreRetrievalBenchmark(Benchmark):
    ...
    description = (
        "Measures the performance of retrieving events from the PostgreSQL event store"
    )
    tags = ["events", "event_store", "postgres", "core"]
    ...
    async def run_iteration(self) -> dict[str, Any]:
        ...
        if method == "by_aggregate":
            ...
            event_type = random.choice(
                ["user_created", "user_email_changed", "user_deactivated"]
            )
