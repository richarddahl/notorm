...
class Benchmark:
    ...
    async def run(self) -> BenchmarkResult:
        ...
        try:
            ...
            # Warm-up iterations
            self.logger.debug(f"Running {self.config.warmup_iterations} warm-up iterations")
            ...
            for i in range(self.config.iterations):
                ...
                if self.config.verbose:
                    self.logger.debug(f"Iteration {i+1}/{self.config.iterations}: {iteration_time:.6f}s")
            ...
            stddev = statistics.stdev(iteration_times) if len(iteration_times) > 1 else 0
            ...
            if self.config.include_resource_metrics:
                try:
                    ...
                    self.logger.warning("psutil not available, resource metrics will not be included")
    ...
    async def run_all(self) -> Dict[str, BenchmarkResult]:
        ...
        if self.config.benchmarks_to_run:
            benchmarks = {
                name: benchmark
                for name, benchmark in benchmarks.items()
                if name in self.config.benchmarks_to_run or benchmark.name in self.config.benchmarks_to_run
            }
...
async def main() -> None:
    ...
    parser.add_argument("--parallel", type=int, default=1,
                        help="Number of parallel benchmark runs")
    ...
    parser.add_argument("--no-resource-metrics", action="store_true",
    ...
    if args.list:
        ...
        for category in BenchmarkRegistry.get_categories():
            ...
            for name, benchmark_class in BenchmarkRegistry.get_benchmarks_by_category(category).items():
