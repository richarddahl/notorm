        ...
        try:
            ...
            if self.profile_queries:
                ...
                if sql_tracker.queries:
                    ...
                    profile_data["total_query_time_ms"] = sum(
                        q.duration_ms for q in sql_tracker.queries
                    )
            ...
            if hasattr(result, "headers"):
                ...
                if self.profile_queries:
                    result.headers["X-Profile-Queries"] = str(
                        profile_data.get("query_count", 0)
                    )
                    if "total_query_time_ms" in profile_data:
                        result.headers["X-Profile-Query-Time"] = (
                            f"{profile_data['total_query_time_ms']:.2f}ms"
                        )
    ...
    def _save_profile(self, profile_id: str, profile_data: dict[str, Any]) -> None:
        ...
        if len(self.profiles) > self.max_profiles:
            ...
            oldest_profiles = sorted(
                self.profiles.items(), key=lambda x: x[1]["start_time"]
        ...
        if self.save_profiles and self.save_directory:
            try:
                ...
                filepath = os.path.join(
                    self.save_directory, f"profile_{profile_id}.json"
                )
    ...
    def get_profiles(
        ...
        for profile in self.profiles.values():
            ...
            if (
                ...
                and profile.get("duration_ms", 0) < min_duration_ms
            ):
                continue

            if path_prefix is not None and not profile.get("path", "").startswith(
                path_prefix
            ):
                continue
