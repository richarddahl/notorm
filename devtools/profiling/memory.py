...
class MemoryProfiler:
    ...
    def _calculate_delta(
        ...
        if (
            ...
            and start.tracemalloc_snapshot
            and end.tracemalloc_snapshot
        ):
            try:
                stats = end.tracemalloc_snapshot.compare_to(
                    start.tracemalloc_snapshot, "lineno"
                )
                tracemalloc_delta = []
    ...
    def print_stats(self, name: str | None = None) -> str:
        ...
        if name and name in self.results:
            ...
            output.write(
                f"RSS: {_format_bytes(result.start_snapshot.rss)} -> {_format_bytes(result.end_snapshot.rss)} "
            )
            ...
            output.write(
                f"VMS: {_format_bytes(result.start_snapshot.vms)} -> {_format_bytes(result.end_snapshot.vms)} "
            )
            ...
            if result.start_snapshot.python_total is not None:
                output.write(
                    f"Python memory: {_format_bytes(result.start_snapshot.python_total)} -> {_format_bytes(result.end_snapshot.python_total)} "
                )
            ...
            if "tracemalloc" in result.delta:
                ...
                for item in result.delta["tracemalloc"]:
                    output.write(
                        f"  {item['file']}:{item['line']} - {_format_bytes(item['size'])} ({item['count']} objects)\n"
                    )
        elif name:
            ...
            for result_name, result in sorted(self.results.items()):
                ...
                output.write(
                    f"  RSS change: {_format_bytes(result.delta.get('rss', 0))}\n"
                )
                if result.peak:
                    output.write(f"  Peak RSS: {_format_bytes(result.peak.rss)}\n")
