...
class DocGenerator:
    ...
    def _discover_modules(self) -> None:
        ...
        # Find all submodules
        for finder, name, is_pkg in pkgutil.iter_modules(self.root_module.__path__, f"{self.module_name}."):
            # Check if module should be included
            if self.include_modules and not any(name.startswith(m) for m in self.include_modules):
                continue
            
            # Check if module should be excluded
            if self.exclude_modules and any(name.startswith(m) for m in self.exclude_modules):
                continue
    ...
    def _generate_module_docs(self, module_name: str, module: Any) -> None:
        ...
        with open(index_path, "w") as f:
            ...
            # Classes
            classes = [
                cls for name, cls in self.classes.items()
            ...
            # Functions
            functions = [
                func for name, func in self.functions.items()
                if name.startswith(f"{module_name}.")
            ]
            ...
            if functions:
                ...
                for func in functions:
                    ...
                    f.write(f"- [{func_name}]({func_name}.md): {func_doc.split('.')[0]}\n")
                f.write("\n")
    ...
    def _generate_class_docs(self, cls: Any, module_dir: Path) -> None:
        ...
        with open(cls_path, "w") as f:
            ...
            if cls.__bases__ and cls.__bases__[0] != object:
                base_classes = ", ".join(b.__name__ for b in cls.__bases__ if b != object)
                    ...
                    try:
                        ...
                        if doc.raises:
                            ...
                            for raised in doc.raises:
                                f.write(f"- `{raised.type_name}`: {raised.description}\n")
                            f.write("\n")
    ...
    def _generate_index(self) -> None:
        ...
        with open(index_path, "w") as f:
            ...
            for module_name in sorted(self.modules.keys()):
                ...
                relative_name = module_name[len(self.module_name) + 1:] if module_name.startswith(f"{self.module_name}.") else module_name
                module_doc = self.modules[module_name].__doc__
                doc_summary = module_doc.strip().split('.')[0] if module_doc else ""
                
                f.write(f"- [{relative_name}]({module_name.replace('.', '/')}/index.md): {doc_summary}\n")
