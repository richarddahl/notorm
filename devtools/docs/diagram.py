...
def generate_diagram(
    ...
    if diagram_type == "db_schema":
        return _generate_db_schema_diagram(
            target, output_path, include_modules, exclude_modules, options
        )
    elif diagram_type == "api":
        return _generate_api_diagram(
            target, output_path, include_modules, exclude_modules, options
        )
    elif diagram_type == "deps":
        return _generate_dependency_diagram(
            target, output_path, include_modules, exclude_modules, options
        )
    elif diagram_type == "class":
        return _generate_class_diagram(
            target, output_path, include_modules, exclude_modules, options
        )
    else:
