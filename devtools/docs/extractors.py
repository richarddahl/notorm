    ...
    def extract_from_object(self, obj: Any) -> list[ExtractionResult]:
        ...
        try:
            ...
            class_info["type_hints"] = {
                name: str(hint) for name, hint in type_hints.items()
            }
            ...
            if inspect.isfunction(member):
                ...
                try:
                    ...
                    method_info["type_hints"] = {
                        name: str(hint) for name, hint in method_type_hints.items()
                    }
                except (NameError, TypeError):
                    method_info["type_hints"] = {}
        ...
        return [
            ExtractionResult(
                content=class_info, source_file=source_file, source_line=source_line
            )
        ]
...
class FunctionExtractor(BaseExtractor):
    ...
    def extract_from_object(self, obj: Any) -> list[ExtractionResult]:
        ...
        return [
            ExtractionResult(
                content=function_info, source_file=source_file, source_line=source_line
            )
        ]
...
class ModuleExtractor(BaseExtractor):
    ...
    def extract_from_object(self, obj: Any) -> list[ExtractionResult]:
        ...
        return [
            ExtractionResult(
                content=module_info, source_file=source_file, source_line=1
            )
        ]
    ...
    def _find_example_files(self, module_name: str) -> list[str]:
        ...
        if examples_path.exists():
            for file_path in examples_path.glob("*.py"):
                if (
                    file_path.name.endswith("_example.py")
...
class APIEndpointExtractor(BaseExtractor):
    ...
    def extract_from_object(self, obj: Any) -> list[ExtractionResult]:
        ...
        elif inspect.isclass(obj) and any(
            base.__name__ == "UnoEndpoint" for base in obj.__mro__
        ):
    ...
    def _extract_endpoint_from_class(self, cls: Any) -> Optional[Dict[str, Any]]:
        ...
        endpoint_info = {
            ...
            or getattr(cls, "__deprecated__", False),
        }
        ...
        for method_name in ("get", "post", "put", "delete", "patch", "options", "head"):
            if hasattr(cls, method_name):
                ...
                for param in method_info.get("parameters", []):
                    if not any(
                        p["name"] == param["name"] for p in endpoint_info["parameters"]
                    ):
...
class SQLExtractor(BaseExtractor):
    ...
    def extract_from_object(self, obj: Any) -> list[ExtractionResult]:
        ...
        if inspect.isclass(obj) and any(
            hasattr(obj, attr) for attr in ("emit_sql", "generate_sql", "to_sql")
        ):
        ...
        elif inspect.isfunction(obj) and any(
            name in obj.__name__.lower() for name in ("sql", "query", "statement")
        ):
...
class ModelExtractor(BaseExtractor):
    ...
    def extract_from_object(self, obj: Any) -> list[ExtractionResult]:
        ...
        is_model = (
            any(
                base.__name__ in ("BaseModel", "BaseModel", "Model")
                for base in obj.__mro__
            )
            or "model" in obj.__name__.lower()
            or hasattr(obj, "__tablename__")
            or hasattr(obj, "__table__")
        )
        ...
        # Get Pydantic fields
            ...
            for name, field in fields.items():
                field_info = {
                    ...
                    "default": (
                        str(field.default) if field.default is not None else None
                    ),
                }
        ...
        return [
            ExtractionResult(
                content=model_info, source_file=source_file, source_line=source_line
            )
        ]
