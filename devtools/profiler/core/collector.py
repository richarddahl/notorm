        ...
        with self.lock:
            ...
            for pattern, stats in patterns.items():
                ...
                for p, s in patterns.items():
                    if (
                        p != pattern
                        and self._are_patterns_similar(p, pattern)
                        and s["count"] > 5
                    ):
                ...
                if similar_patterns:
                    n_plus_1_candidates[pattern] = {
                        ...
                        "count": stats["count"],
                        "similar_patterns": [
                            {"pattern": p, "count": s["count"]}
                            for p, s in similar_patterns
                        ],
                    }
    ...
    def analyze_endpoints(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            for key, stats in endpoint_stats.items():
                total_requests = sum(
                    int(count) for count in stats["status_codes"].values()
                )
                error_count = sum(
                    int(count)
            ...
            return {
                ...
                "total_requests": sum(
                    stats["count"] for stats in endpoint_stats.values()
                ),
...
class ResourceCollector(MetricCollector):
    ...
    def __init__(
        ...
        """
        super().__init__(
            "resource-collector", capacity, report_interval, report_callback
        )
    ...
    def analyze_resources(self) -> Dict[str, Any]:
        ...
        memory_process_percent = [
            m.process_percent for m in memory_metrics if m.process_percent is not None
        ]
        memory_percent = [m.percent for m in memory_metrics]

        # Calculate CPU statistics
        cpu_process_percent = [
            m.process_percent for m in cpu_metrics if m.process_percent is not None
        ]
        ...
        return {
            "memory": {
                ...
                "avg_percent": (
                    sum(memory_percent) / len(memory_percent)
                    if memory_percent
                    else None
                ),
                "max_percent": max(memory_percent) if memory_percent else None,
                "avg_process_percent": (
                    sum(memory_process_percent) / len(memory_process_percent)
                    if memory_process_percent
                    else None
                ),
                "max_process_percent": (
                    max(memory_process_percent) if memory_process_percent else None
                ),
            },
            "cpu": {
                ...
                "avg_percent": (
                    sum(cpu_percent) / len(cpu_percent) if cpu_percent else None
                ),
                "max_percent": max(cpu_percent) if cpu_percent else None,
                "avg_process_percent": (
                    sum(cpu_process_percent) / len(cpu_process_percent)
                    if cpu_process_percent
                    else None
                ),
                "max_process_percent": (
                    max(cpu_process_percent) if cpu_process_percent else None
                ),
            },
        }
...
class FunctionCollector(MetricCollector):
    ...
    def _update_stats(
        self, name: str, duration: float, module: str | None = None
    ) -> None:
        """
        Update function statistics.
    ...
    def analyze_functions(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            for key, stats in function_stats.items():
                ...
                if (
                    stats["count"] > 10
                    and stats["avg"] > self.slow_function_threshold / 10
                ):
                    hotspots.append(
                        {
                            "name": stats["name"],
