...
class SQLQueryCapture:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        try:
            if (
                hasattr(self, "original_asyncpg_execute")
                and self.original_asyncpg_execute
            ):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            if duration >= self.slow_request_threshold:
                logger.warning(
                    f"Slow request detected ({duration:.3f}s): {method} {path}"
                )
...
class DebugResponseMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if self.include_request_header in request.headers:
            include_debug = request.headers[self.include_request_header].lower() in (
                "true",
                "1",
                "yes",
            )
                    ...
                    if isinstance(json_data, dict):
                        json_data["__debug"] = {
                            ...
                            "queries": {
                                ...
                                "slow_count": len(
                                    analysis["queries"].get("slow_queries", [])
                                ),
                            },
                            "endpoints": {
                                "path": request.url.path,
