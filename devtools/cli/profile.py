...
if TYPER_AVAILABLE:
    ...
    def start_dashboard_command(
        ...
        app_path: Annotated[
            str | None,
            typer.Option("--app", "-a", help="Path to ASGI application to profile"),
        ] = None,
        ...
        try:
            ...
            try:
                from uno.devtools.profiler.middleware.profiling_middleware import (
                    ProfilerMiddleware,
                )
                ...
                logger.error(
                    "Make sure the dependencies are installed: fastapi uvicorn psutil"
                )
                sys.exit(1)
            ...
            if app_path:
                ...
                try:
                    ...
                    module_path, app_name = (
                        app_path.split(":") if ":" in app_path else (app_path, "app")
                    )
    ...
    def run_profile_command(
        target: Annotated[
            str,
            typer.Argument(
                help="Module.function to profile (e.g., 'myapp.main:run_app')"
            ),
        ],
        output: Annotated[
            str, typer.Option(help="Output format: text, json, or html")
        ] = "text",
        output_file: Annotated[
            Optional[Path], typer.Option(help="Output file path")
        ] = None,
        detailed: Annotated[
            bool, typer.Option(help="Enable detailed profiling with cProfile")
        ] = True,
        use_yappi: Annotated[
            bool, typer.Option(help="Use yappi for multi-threaded profiling")
        ] = False,
        include_builtins: Annotated[
            bool, typer.Option(help="Include Python builtins in profile")
        ] = False,
        max_depth: Annotated[
            int, typer.Option(help="Maximum call depth to display")
        ] = 10,
        verbose: Annotated[
            int, typer.Option("--verbose", "-v", count=True, help="Verbosity level")
        ] = 0,
            ...
            if output == "text":
                ...
                if output_file and hasattr(visualize_profile, "__call__"):
                    ...
                    if profile_result:
                        ...
                        logger.info(
                            f"HTML profile visualization written to {output_file}"
                        )
                else:
                    logger.error(
                        "HTML output requires visualization module and output file"
                    )
                    sys.exit(1)
    ...
    def memory_profile_command(
        ...
        output: Annotated[
            str, typer.Option(help="Output format: text or json")
        ] = "text",
        output_file: Annotated[
            Optional[Path], typer.Option(help="Output file path")
        ] = None,
        detailed: Annotated[
            bool, typer.Option(help="Enable detailed memory profiling")
        ] = False,
        track_gc: Annotated[
            bool, typer.Option(help="Track garbage collector objects")
        ] = False,
        use_tracemalloc: Annotated[
            bool, typer.Option(help="Use tracemalloc for detailed tracking")
        ] = False,
        interval: Annotated[
            float,
            typer.Option(
                help="Interval in seconds between snapshots (0 for start/end only)"
            ),
        ] = 0.0,
        verbose: Annotated[
            int, typer.Option("--verbose", "-v", count=True, help="Verbosity level")
    ...
    def find_hotspots_command(
        target: Annotated[
            str,
            typer.Argument(
                help="Module to analyze for hotspots (e.g., 'myapp.models')"
            ),
        ],
        depth: Annotated[
            int, typer.Option(help="Maximum callstack depth to analyze")
        ] = 10,
        min_time: Annotated[
            float, typer.Option(help="Minimum percentage of total time to include")
        ] = 1.0,
        output_file: Annotated[
            Optional[Path], typer.Option(help="Output file path")
        ] = None,
    ...
    def setup_parser(subparsers):
        ...
        profile_parser = subparsers.add_parser(
            "profile", help="Profiling utilities for Uno"
        )
        profile_subparsers = profile_parser.add_subparsers(dest="subcommand")

        # Dashboard command
        dashboard_parser = profile_subparsers.add_parser(
            "dashboard", help="Start the performance profiling dashboard"
        )
        dashboard_parser.add_argument(
            "--host", "-h", default="localhost", help="Host to bind to"
        )
        dashboard_parser.add_argument(
            "--port", "-p", type=int, default=8765, help="Port to bind to"
        )
        dashboard_parser.add_argument(
            "--app", "-a", help="Path to ASGI application to profile"
        )
        dashboard_parser.add_argument(
        ...
        # Run command
        run_parser = profile_subparsers.add_parser(
            "run", help="Profile a Python function or method"
        )
        run_parser.add_argument(
            "target", help="Module.function to profile (e.g., 'myapp.main:run_app')"
        )
        run_parser.add_argument(
            "--output",
            default="text",
        ...
        run_parser.add_argument(
            "--use-yappi",
            action="store_true",
            help="Use yappi for multi-threaded profiling",
        )
        run_parser.add_argument(
            "--include-builtins",
            action="store_true",
            help="Include Python builtins in profile",
        )
        run_parser.add_argument(
            "--max-depth", type=int, default=10, help="Maximum call depth to display"
        )
        run_parser.add_argument(
            "-v", "--verbose", action="count", default=0, help="Verbosity level"
        )
        ...
        # Memory command
        memory_parser = profile_subparsers.add_parser(
            "memory", help="Profile memory usage of a Python function or method"
        )
        ...
        memory_parser.add_argument(
            "--detailed", action="store_true", help="Enable detailed memory profiling"
        )
        memory_parser.add_argument(
            "--track-gc", action="store_true", help="Track garbage collector objects"
        )
        memory_parser.add_argument(
            "--use-tracemalloc",
            action="store_true",
            help="Use tracemalloc for detailed tracking",
        )
        memory_parser.add_argument(
            "--interval",
            type=float,
        ...
        # Hotspots command
        hotspots_parser = profile_subparsers.add_parser(
            "hotspots", help="Find performance hotspots in a module"
        )
        hotspots_parser.add_argument(
            "target", help="Module to analyze for hotspots (e.g., 'myapp.models')"
        )
        hotspots_parser.add_argument(
            "--depth", type=int, default=10, help="Maximum callstack depth to analyze"
        )
        hotspots_parser.add_argument(
            "--min-time",
            type=float,
            default=1.0,
            help="Minimum percentage of total time to include",
        )
        ...
        try:
            ...
            if hasattr(args, "app") and args.app:
                ...
                try:
                    ...
                    module_path, app_name = (
                        args.app.split(":") if ":" in args.app else (args.app, "app")
                    )
                    module = __import__(module_path, fromlist=[app_name])
                    app = getattr(module, app_name)
    ...
    def _handle_run_command(args):
        ...
        try:
            ...
            # Format and output results
            if args.output == "text":
                profile_output = profiler.print_stats(
                    func.__qualname__, top=args.max_depth
                )
                if args.output_file:
                ...
                if args.output_file and hasattr(visualize_profile, "__call__"):
                    ...
                    if profile_result:
                        ...
                        logger.info(
                            f"HTML profile visualization written to {args.output_file}"
                        )
                else:
                    logger.error(
                        "HTML output requires visualization module and output file"
                ...
                if profile_result:
                    ...
                    if args.output_file:
                        with open(args.output_file, "w") as f:
                            f.write(json_output)
                        logger.info(
                            f"Memory profile results written to {args.output_file}"
                        )
                    else:
                        print(json_output)
