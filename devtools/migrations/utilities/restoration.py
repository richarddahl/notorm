...
def restore_file(
    ...
    if target_path.exists() and not force:
        raise FileExistsError(
            f"Target file exists, use force=True to overwrite: {target_path}"
        )
...
def restore_directory(
    backup_path: Union[str, Path], target_dir: Union[str, Path], force: bool = False
) -> Path:
    ...
    if not force and any(target_dir.iterdir()):
        raise FileExistsError(
            f"Target directory is not empty, use force=True to overwrite: {target_dir}"
        )
    ...
    if backup_path.name.endswith(".gz"):
        ...
        with gzip.open(backup_path, "rb") as f:
            process = subprocess.run(
                cmd, input=f.read(), stderr=subprocess.PIPE, check=False
            )
    else:
