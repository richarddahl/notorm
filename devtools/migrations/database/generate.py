...
def generate_migration_script(
    ...
    for table_name, columns in diff.added_columns.items():
        for col_name in columns:
            if format == "python":
                upgrade_ops.append(
                    f"    # TODO: Add column {col_name} to table {table_name}"
                )
                downgrade_ops.append(
                    f"    # TODO: Drop column {col_name} from table {table_name}"
                )
            else:  # SQL
                upgrade_ops.append(
                    f"-- TODO: Add column {col_name} to table {table_name}"
    ...
    for table_name, columns in diff.modified_columns.items():
        for col_name, changes in columns.items():
            if format == "python":
                upgrade_ops.append(
                    f"    # TODO: Modify column {col_name} in table {table_name}"
                )
                downgrade_ops.append(
                    f"    # TODO: Revert changes to column {col_name} in table {table_name}"
                )
            else:  # SQL
                upgrade_ops.append(
                    f"-- TODO: Modify column {col_name} in table {table_name}"
...
def generate_migration_plan(diff: SchemaDiff, format: str = "markdown") -> str:
                ...
                for column, changes in columns.items():
                    ...
                    for change_type, details in changes.items():
                        if change_type == "type_changed":
                            parts.append(
                                f"  - Change type from `{changes.get('old_type')}` to `{changes.get('new_type')}`"
                            )
                        elif change_type == "nullable_changed":
                            old_nullable = changes.get("old_nullable")
                            new_nullable = changes.get("new_nullable")
                            parts.append(
                                f"  - Change nullable from `{old_nullable}` to `{new_nullable}`"
                            )
