    ...
    try:
        if script_path.suffix == ".py":
            ...
            if not hasattr(module, "downgrade"):
                logger.error(
                    f"Migration script has no downgrade function: {script_path}"
                )
                return False
                    ...
                    class MockOp:
                        def __getattr__(self, name):
                            def method(*args, **kwargs):
                                logger.info(
                                    f"Would call op.{name}({args}, {kwargs}) (dry run)"
                                )
            ...
            rollback_script_path = script_path.with_name(
                f"{script_path.stem}_rollback.sql"
            )
...
def rollback_migrations(
    ...
    for script_path in scripts:
        ...
        success = rollback_migration(
            script_path, engine, schema, dry_run, context, verify
        )
...
def rollback_to_revision(
    ...
    scripts = [script for script in scripts if script.stem.split("_")[0] > revision]
    ...
    return rollback_migrations(
        scripts, engine, schema, dry_run, context, verify, stop_on_error
    )


def generate_rollback_script(
    script_path: Union[str, Path], output_dir: Optional[Union[str, Path]] = None
) -> Path:
