    ...
    def to_markdown(self) -> str:
        ...
        if self.issues:
            ...
            for file_path, issues in issues_by_file.items():
                ...
                for issue in issues:
                    lines.append(
                        f"- **Line {issue.line_number}** - {issue.issue_type} ({issue.severity})"
                    )
...
class CodeAnalyzer:
    ...
    def analyze_file(
        self, file_path: Union[str, Path], patterns: list[str] | None = None
    ) -> AnalysisResult:
        ...
        for node in ast.walk(tree):
            ...
            if isinstance(node, ast.BinOp) and isinstance(node.op, ast.BitOr):
                if (
                    ...
                    and node.left.id.startswith(
                        ("int", "str", "list", "dict", "set", "tuple", "Union")
                    )
                ):
                    result.add_issue(
                        CodeIssue(
