...
class SQLQueryTracker:
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        # Count query types
        select_count = sum(
            1 for q in self.queries if q.query.strip().lower().startswith("select")
        )
        insert_count = sum(
            1 for q in self.queries if q.query.strip().lower().startswith("insert")
        )
        update_count = sum(
            1 for q in self.queries if q.query.strip().lower().startswith("update")
        )
        delete_count = sum(
            1 for q in self.queries if q.query.strip().lower().startswith("delete")
        )
        other_count = (
            len(self.queries)
            - select_count
            ...
            def patched_execute(query, *args, **kwargs):
                ...
                if self.tracker.track_context:
                    ...
                    for frame in stack[1:]:
                        if (
                            frame.function != "execute"
                            and not frame.function.startswith("_")
                        ):
                            context = (
