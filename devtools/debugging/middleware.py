...
class DebugMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        ...
        if self.enable_sql_tracking:
            try:
                ...
                logger.warning(
                    "SQL tracking not available - required modules not found"
                )
        ...
        if self.enable_dependency_tracking:
            try:
                ...
                logger.warning(
                    "Dependency tracking not available - container not accessible"
                )
                self.enable_dependency_tracking = False
    ...
    async def dispatch(
                ...
                try:
                    ...
                    if len(body) > 0:
                        try:
                            ...
                            debug_info["request"][
                                "body"
                            ] = f"<non-JSON body of length {len(body)}>"
        ...
        if self.debug_headers:
            response.headers["X-Debug-Time-Ms"] = str(
                int(debug_info["timing"]["duration_ms"])
            )
            response.headers["X-Debug-SQL-Queries"] = str(
                len(debug_info["sql_queries"])
            )
    ...
    def _log_debug_info(self, debug_info: Dict[str, Any], response: Response) -> None:
        ...
        if self.enable_sql_tracking:
            ...
            if sql_count > 0:
                total_sql_time = sum(
                    q.get("duration_ms", 0) for q in debug_info["sql_queries"]
                )
