...
class FunctionTracer:
    ...
    def trace_call(self, func: Callable[..., Any], args: tuple, kwargs: dict) -> Any:
        ...
        try:
            ...
            if self.log_return:
                ...
                self.logger.log(
                    self.level,
                    f"{indent}← {func_name} completed{time_str}"
                )
    ...
    def _format_args(self, func: Callable[..., Any], args: tuple, kwargs: dict) -> str:
        ...
        try:
            ...
            arg_parts.extend(f"{name}={self._format_value(value)}" for name, value in kwargs.items())
    ...
    def _format_value(self, value: Any) -> str:
        ...
        if hasattr(value, "__class__"):
            if value.__class__.__name__ in ("DataFrame", "Series") and hasattr(value, "shape"):
                return f"<{value.__class__.__name__} shape={getattr(value, 'shape', '?')}>"
            
            if value.__class__.__name__ in ("ndarray", "Tensor") and hasattr(value, "shape"):
