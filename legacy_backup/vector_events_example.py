...
class DocumentVectorHandler(EventSubscriber):
    ...
    async def handle_embedding_updated(self, event: VectorEmbeddingUpdated) -> None:
        ...
        if event.entity_type == "Document" and event.success:
            ...
            print(
                f"❌ Error updating embedding for {event.entity_type} {event.entity_id}: {event.error_message}"
            )
...
async def setup_event_driven_system():
    ...
    vector_event_handler = VectorEventHandler(
        dispatcher=dispatcher, vectorizable_types=vectorizable_types, logger=logger
    )

    vector_update_handler = VectorUpdateHandler(dispatcher=dispatcher, logger=logger)
    ...
    update_service = VectorUpdateService(
        dispatcher=dispatcher, batch_size=10, update_interval=1.0, logger=logger
    )
    ...
    batch_service = BatchVectorUpdateService(
        dispatcher=dispatcher, batch_size=100, logger=logger
    )
