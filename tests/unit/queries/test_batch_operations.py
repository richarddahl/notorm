...
def batch_operations(mock_session):
    ...
    with (
        patch("uno.queries.batch_operations.CommonQueryPatterns") as mock_patterns_cls,
...
class TestBatchProcessor:
    ...
    async def test_optimal_batch_size(self, batch_processor):
        ...
        large_records = [
            {"id": i, "name": f"L{i}", "data": "x" * 10000} for i in range(10)
        ]
...
class TestBatchOperations:
    ...
    async def test_batch_update(self, batch_operations):
        ...
        result = await batch_operations.batch_update(
            records, id_field="id", fields_to_update=["name"], return_models=False
        )
    ...
    async def test_batch_upsert(self, batch_operations):
        ...
        result = await batch_operations.batch_upsert(
            records, constraint_columns=["id"], return_models=True
        )
    ...
    async def test_batch_compute(self, batch_operations):
        ...
        mock_processor.process_batch.side_effect = (
            lambda records, operation_fn, **kwargs: asyncio.create_task(
                asyncio.gather(
        ...
        result = await batch_operations.batch_compute(
            id_values, compute_fn=compute_name_upper
        )
    ...
    async def test_batch_import(self, batch_operations):
        ...
        async def simulate_process_batch(records, operation_fn, **kwargs):
            ...
            return result, BatchMetrics(
                processed_records=len(records), total_records=len(records)
            )
        ...
        result = await batch_operations.batch_import(
            records, unique_fields=["id"], update_on_conflict=True, return_stats=True
        )
        ...
        result = await batch_operations.batch_import(
            records, unique_fields=["id"], update_on_conflict=False, return_stats=True
        )
