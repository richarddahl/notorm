...
class MockModel:
    ...
    def __init__(
        self, tablename, schema, columns, primary_key=None, unique_constraints=None
    ):
...
class TestUnoDBFactory:
    ...
    def test_table_keys_finds_unique_constraints(self, mock_obj):
        ...
        model = MockModel(
            "users", "public", columns, unique_constraints=unique_constraints
        )
    ...
    async def test_filter_with_paging(self, db_factory):
        ...
        try:
            ...
            # Call the mock filter function
            result = await db_factory.filter(
                [limit_filter, offset_filter, order_by_filter]
            )
