...
class TestRepositoryGenerator:
    ...
    def test_generate_repository_with_query_methods(self):
        ...
        assert "def find_by_category(self, category: str) -> List[Product]:" in repo_code
        assert "return self.query.filter(self.model_class.category == category).all()" in repo_code
        assert "def find_active_by_price_range(self, min_price: float, max_price: float) -> List[Product]:" in repo_code
    ...
    def test_generate_repository_with_bulk_operations(self):
        ...
        assert "def bulk_update(self, order_updates: List[Tuple[int, Dict[str, Any]]])" in repo_code
    ...
    def test_formatting_options(self):
        ...
        assert "from uno.database.repository import UnoRepository" in repo_code
