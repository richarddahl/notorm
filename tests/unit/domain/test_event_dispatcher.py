...
class TestPostgresEventListener:
    ...
    @pytest.mark.asyncio
    async def test_listen_for_events(self, listener, mock_asyncpg_pool, mock_asyncpg_connection):
        """Test the _listen_for_events method."""
        # Set up a mock for _on_notification
        with patch.object(listener, '_on_notification', AsyncMock()) as mock_on_notification:
