...
from uno.core.unified_events import (
    DomainEvent, EventBus, initialize_events, reset_events, collect_event, get_event_bus
)
from uno.domain.core import Entity, AggregateRoot, DomainException
from uno.domain.repository import Repository, InMemoryRepository, InMemoryAggregateRepository
...
class CreateEntityService(DomainService[CreateEntityInput, EntityOutput, UnitOfWork]):
    ...
    async def _execute_internal(self, input_data: CreateEntityInput) -> Result[EntityOutput]:
        """Create a new entity."""
        # Create entity
        entity = TestEntity(
            id=uuid.uuid4(),
...
class TestDomainService:
    ...
    async def test_domain_service_validation_failure(self, entity_repository):
        ...
        service.validate = lambda data: Failure("Invalid name") if data.name == "Invalid" else None
...
class TestEntityService:
    ...
    async def test_entity_service_update(self, entity_repository):
        ...
        # Act
        result = await service.update(entity_id, {
            "name": "Updated Name",
...
class TestAggregateService:
    ...
    async def test_aggregate_service_update_with_concurrency(self, aggregate_repository):
        ...
        result = await service.update(aggregate_id, 1, {
            "name": "Updated Name"
        })
        ...
        result2 = await service.update(aggregate_id, 1, {
    ...
    async def test_aggregate_service_events(self, aggregate_repository):
        ...
        # Act
        result = await service.update(aggregate.id, 1, {
            "name": "Updated Name"
        })
...
class TestDomainServiceFactory:
    ...
    async def test_create_domain_service(self, service_factory, unit_of_work_factory, entity_repository):
    ...
    async def test_create_read_only_service(self, service_factory, unit_of_work_factory, entity_repository):
