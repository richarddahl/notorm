...
from uno.core.unified_events import (
    DomainEvent, EventBus, EventPublisher, EventStore, InMemoryEventStore,
    EventHandler, EventPriority, EventSubscriber, event_handler,
    ...
    clear_collected_events, scan_for_handlers, scan_instance_for_handlers
)
...
class TestUnifiedEventSystem:
    ...
    async def test_event_handler(self, test_event):
        ...
        assert not handler.can_handle(UserCreatedEvent(user_id="123", email="a@b.com", username="test"))
    ...
    @pytest.mark.asyncio
    async def test_event_subscriber(self, event_bus, user_created_event, order_placed_event):
    ...
    @pytest.mark.asyncio
    async def test_event_publisher(self, event_publisher, test_event, user_created_event):
    ...
    async def test_in_memory_event_store(self):
        ...
        # Save events
        event1 = TestEvent(data="store test 1", aggregate_id="agg-1", aggregate_type="Test")
        event2 = TestEvent(data="store test 2", aggregate_id="agg-1", aggregate_type="Test")
    ...
    async def test_scan_for_handlers(self):
        ...
        await get_event_bus().publish(OrderPlacedEvent(
            order_id="order-123",
            user_id="user-456",
            items=[],
            total=0.0,
