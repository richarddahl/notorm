...
class TestListSchema(PaginatedList[TestModel]):
    ...
    class Config:
        schema_extra = {
            "example": {
                "items": [{"id": "1", "name": "Test", "email": "test@example.com", "age": 30}],
                "total": 1,
                "page": 1,
                "page_size": 25,
                "pages": 1
...
class TestUnoSchemaManager:
    ...
    def test_generic_list_schema(self):
        ...
        test_model_dict = TestModel(id="1", name="Test", email="test@example.com", age=30).model_dump()
        
        instance = list_schema(
            items=[test_model_dict],
            total=1,
            page=1,
            page_size=25,
            pages=1
