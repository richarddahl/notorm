...
from .test_repositories import (
    template_data, field_data, trigger_data, output_data, execution_data,
    template, field, trigger, output, execution
)
...
def execution_service(
    ...
    return ReportExecutionService(
        db_session, template_repo, field_repo, execution_repo, output_execution_repo, output_repo
    )
...
def trigger_service(
    ...
    return ReportTriggerService(db_session, template_repo, trigger_repo, execution_service)
...
def output_service(
    ...
    return ReportOutputService(
        db_session, template_repo, output_repo, execution_repo, output_execution_repo, field_repo
    )
...
class TestReportFieldService:
    ...
    async def test_validate_field_config(
        self,
        field_service: ReportFieldService
    ):
        ...
        result = await field_service.validate_field_config(ReportFieldType.DB_COLUMN, valid_config)
        assert result.is_success
        assert result.value is True
        ...
        result = await field_service.validate_field_config(ReportFieldType.DB_COLUMN, invalid_config)
...
class TestReportTriggerService:
    ...
    async def test_handle_event(
        ...
        with patch.object(execution_service, 'execute_report') as mock_execute:
            mock_execute.return_value = Success(ReportExecution(
    ...
    async def test_process_scheduled_triggers(
        ...
        create_result = await trigger_service.trigger_repository.create(scheduled_trigger)
        assert create_result.is_success
        ...
        with patch.object(execution_service, 'execute_report') as mock_execute:
            ...
            get_result = await trigger_service.trigger_repository.get_by_id(scheduled_trigger.id)
            assert get_result.is_success
...
class TestReportExecutionService:
    ...
    async def test_execute_report_with_validation_error(
        ...
        template_with_required = await execution_service.template_repository.get_by_id(template.id)
    ...
    async def test_execute_report_with_outputs(
        ...
        output_executions_result = await execution_service.output_execution_repository.list_by_execution(
            result.value.id
        )
        assert output_executions_result.is_success
        assert len(output_executions_result.value) > 0
        assert output_executions_result.value[0].report_output_id == output_result.value.id
    ...
    async def test_get_execution_result_with_format(
        ...
        # Get in JSON format
        json_result = await execution_service.get_execution_result(execution.id, format="json")
        
        assert json_result.is_success
        assert isinstance(json_result.value, str)  # JSON string
        
        # Get in CSV format
        csv_result = await execution_service.get_execution_result(execution.id, format="csv")
        
        assert csv_result.is_success
        assert isinstance(csv_result.value, str)  # CSV string
        
        # Test with invalid format
        invalid_format_result = await execution_service.get_execution_result(execution.id, format="invalid")
...
class TestReportOutputService:
    ...
    async def test_format_report(
        ...
        complete_result = await execution_service.execution_repository.complete_execution(
            execution.id,
            row_count=100,
        ...
        formats_to_test = [ReportFormat.CSV, ReportFormat.JSON, ReportFormat.HTML, ReportFormat.TEXT]
    ...
    async def test_deliver_report(
        ...
        output_execution_result = await output_service.output_execution_repository.create(output_execution)
