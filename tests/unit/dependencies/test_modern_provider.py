...
class TestUnoServiceProvider:
    ...
    async def test_extension_registration(self, service_provider, service_collection):
        ...
        # Arrange
        service_collection.add_singleton(MockServiceProtocol, TestService, value="base")
        service_provider.configure_services(service_collection)

        # Create extension
        extension = ServiceCollection()
        extension.add_singleton(
            TestDependencyProtocol, TestDependency, name="extension"
        )
    ...
    @pytest.mark.asyncio
    async def test_reconfigure_after_init_error(
        self, service_provider, service_collection
    ):
        """Test that reconfiguring after initialization raises an error."""
        # Arrange
        service_collection.add_singleton(MockServiceProtocol, TestService)
        service_provider.configure_services(service_collection)
    ...
    @pytest.mark.asyncio
    async def test_extension_after_init_error(
        self, service_provider, service_collection
    ):
    ...
    @pytest.mark.asyncio
    async def test_service_resolution_before_init_error(
        self, service_provider, service_collection
    ):
        ...
        with pytest.raises(Exception, match="Service provider must be initialized"):
            service_provider.get_service(MockServiceProtocol)
    ...
    async def test_service_in_scope(self, service_provider, service_collection):
        ...
        # Act - Get in scope
        service1 = service_provider.get_service_in_scope(MockServiceProtocol, "scope1")
        service2 = service_provider.get_service_in_scope(MockServiceProtocol, "scope2")
    ...
    async def test_async_scope(self, service_provider, service_collection):
        ...
        # Arrange
        service_collection.add_scoped(
            MockServiceProtocol, TestService, value="async_scope"
        )
        service_provider.configure_services(service_collection)
        await service_provider.initialize()
        ...
        async with service_provider.create_scope("test_scope") as scope:
            service = scope.resolve(MockServiceProtocol)
    ...
    @pytest.mark.asyncio
    async def test_register_singleton_function(
        self, service_provider, service_collection, clean_container
    ):
        ...
        register_singleton(MockServiceProtocol, instance)

        # Assert
        resolved = service_provider.get_service(MockServiceProtocol)
    ...
    async def test_global_service_provider(self, clean_container):
        ...
        collection = ServiceCollection()
        collection.add_singleton(MockServiceProtocol, TestService, value="global_test")
        ...
        service = provider.get_service(MockServiceProtocol)
...
class TestServiceCollectionIntegration:
    ...
    async def test_factory_registration(self, service_collection, clean_container):
        ...
        # Assert
        service = get_service(MockServiceProtocol)
    ...
    async def test_complex_dependencies(self, service_collection, clean_container):
        ...
        service_collection.add_singleton(
            TestDependencyProtocol, TestDependency, name="complex_dep"
        )
