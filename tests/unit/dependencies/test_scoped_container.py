...
class TestService(MockServiceProtocol):
    """Concrete implementation of MockServiceProtocol."""
...
class DependentService:
    ...
    def __init__(
        self, dependency: TestDependencyProtocol, service: MockServiceProtocol
    ):
...
class TestServiceCollection:
    ...
    def test_singleton_registration(self, service_collection):
        ...
        service_collection.add_singleton(
            MockServiceProtocol, TestService, value="singleton_test"
        )

        # Assert
        registrations = service_collection._registrations
        assert MockServiceProtocol in registrations
        assert registrations[MockServiceProtocol].scope == ServiceScope.SINGLETON
        assert registrations[MockServiceProtocol].implementation == TestService
        assert registrations[MockServiceProtocol].params == {"value": "singleton_test"}

    def test_scoped_registration(self, service_collection):
        """Test registering a scoped service."""
        # Arrange & Act
        service_collection.add_scoped(
            MockServiceProtocol, TestService, value="scoped_test"
        )

        # Assert
        registrations = service_collection._registrations
        assert MockServiceProtocol in registrations
        assert registrations[MockServiceProtocol].scope == ServiceScope.SCOPED
        assert registrations[MockServiceProtocol].params == {"value": "scoped_test"}
    ...
    def test_transient_registration(self, service_collection):
        ...
        service_collection.add_transient(
            MockServiceProtocol, TestService, value="transient_test"
        )

        # Assert
        registrations = service_collection._registrations
        assert MockServiceProtocol in registrations
        assert registrations[MockServiceProtocol].scope == ServiceScope.TRANSIENT
    ...
    def test_instance_registration(self, service_collection):
        ...
        # Act
        service_collection.add_instance(MockServiceProtocol, instance)

        # Assert
        instances = service_collection._instances
        assert MockServiceProtocol in instances
        assert instances[MockServiceProtocol] is instance
    ...
    def test_factory_registration(self, service_collection):
        ...
        # Act
        service_collection.add_factory(MockServiceProtocol, create_service)

        # Assert
        factories = service_collection._factories
        assert MockServiceProtocol in factories
        assert factories[MockServiceProtocol]() is not None
        assert factories[MockServiceProtocol]().get_value() == "factory_test"
...
class TestServiceResolver:
    ...
    async def test_singleton_resolution(self, service_collection, clean_container):
        ...
        # Arrange
        service_collection.add_singleton(
            MockServiceProtocol, TestService, value="singleton_value"
        )
        initialize_container(service_collection)

        # Act
        service1 = get_service(MockServiceProtocol)
        service2 = get_service(MockServiceProtocol)
    ...
    async def test_scoped_resolution(self, service_collection, clean_container):
        ...
        # Arrange
        service_collection.add_scoped(
            MockServiceProtocol, TestService, value="scoped_value"
        )
        initialize_container(service_collection)

        # Act
        with create_scope("scope1") as scope1:
            service1 = scope1.resolve(MockServiceProtocol)
            service1_again = scope1.resolve(MockServiceProtocol)

            with create_scope("scope2") as scope2:
                service2 = scope2.resolve(MockServiceProtocol)
    ...
    async def test_transient_resolution(self, service_collection, clean_container):
        ...
        # Arrange
        service_collection.add_transient(
            MockServiceProtocol, TestService, value="transient_value"
        )
        initialize_container(service_collection)
    ...
    async def test_instance_resolution(self, service_collection, clean_container):
        ...
        # Act
        resolved = get_service(MockServiceProtocol)
    ...
    async def test_dependency_resolution(self, service_collection, clean_container):
        ...
        # Arrange
        service_collection.add_singleton(
            TestDependencyProtocol, TestDependency, name="test_dep"
        )
        service_collection.add_singleton(
            MockServiceProtocol, TestService, value="test_service"
        )
    ...
    async def test_dependency_not_found(self, service_collection, clean_container):
        ...
        service_collection.add_singleton(
            MissingDependencyService, MissingDependencyService
        )
        initialize_container(service_collection)
...
class TestAsyncScope:
    ...
    async def test_async_scope_basic(self, service_collection, clean_container):
        ...
        # Arrange
        service_collection.add_scoped(
            MockServiceProtocol, TestService, value="async_scoped"
        )
        initialize_container(service_collection)

        # Act
        async with create_async_scope("async_scope") as scope:
            service1 = scope.resolve(MockServiceProtocol)
            service2 = scope.resolve(MockServiceProtocol)
    ...
    async def test_nested_async_scopes(self, service_collection, clean_container):
        ...
        # Arrange
        service_collection.add_scoped(
            MockServiceProtocol, TestService, value="parent_scope"
        )
        service_collection.add_scoped(
            DisposableScope, DisposableScope, name="test_scope"
        )
        initialize_container(service_collection)
        ...
        async with create_async_scope("parent") as parent_scope:
            parent_scope_service = parent_scope.resolve(MockServiceProtocol)
            parent_disposable = parent_scope.resolve(DisposableScope)

            async with create_async_scope("child") as child_scope:
                child_scope_service = child_scope.resolve(MockServiceProtocol)
                ...
                assert (
                    parent_scope_service is not child_scope_service
                )  # Different instances
