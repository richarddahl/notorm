...
class TestOrder(Base):
    ...
    id = Column(Integer, primary_key=True)
    customer_id = Column(
        Integer, ForeignKey("test_optimizer_customers.id"), nullable=False
    )
...
class TestOrderItem(Base):
    ...
    product_id = Column(
        Integer, ForeignKey("test_optimizer_products.id"), nullable=False
    )
...
async def test_index_recommendations(query_optimizer):
    ...
    query = select(TestProduct).where(
        (TestProduct.category == "Category 1") & (TestProduct.price > 15.0)
    )
    ...
    if recommendations:
        ...
        assert (
            recommendation.table_name == "test_optimizer_products"
            or recommendation.table_name == "test_optimizer_products"
        )
...
async def test_execute_optimized_query(query_optimizer, setup_test_db):
    ...
    # Define a query
    query = select(TestProduct).where(
        (TestProduct.price > 15.0) & (TestProduct.is_active == True)
    )
...
async def test_query_cache_dependencies(query_cache):
    ...
    await query_cache.set("query1", "result1", dependencies=["products", "customers"])
    await query_cache.set("query2", "result2", dependencies=["products"])
    await query_cache.set("query3", "result3", dependencies=["orders"])
...
async def test_cached_query_decorator(query_cache, setup_test_db):
    ...
    @cached_query(
        ttl=10.0, dependencies=["test_optimizer_products"], cache_instance=query_cache
    )
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_integration_optimizer_with_cache(
    query_optimizer, query_cache, setup_test_db
):
    ...
    await query_cache.set(
        cache_key, query_result, dependencies=["test_optimizer_products"]
    )
