...
from uno.domain.authorization import (
    Permission, Role as AuthRole, AuthorizationService, 
    ServiceContext, SimplePolicy, OwnershipPolicy, TenantPolicy
)
from uno.database.session import async_session
from uno.database.schema_manager import SchemaManager
from uno.authorization.rlssql import (
    RowLevelSecurity, UserRowLevelSecurity, TenantRowLevelSecurity,
    AdminRowLevelSecurity, DefaultRowLevelSecurity, SuperuserRowLevelSecurity
)
...
async def test_schema(schema_manager):
    ...
    async with async_session() as session:
        ...
        for model in [TestTenant, TestUser, TestResource, TestUserRole, TestRolePermission]:
...
async def rls_setup(test_schema):
    ...
    async with async_session() as session:
        ...
        default_rls = DefaultRowLevelSecurity(config=config, table=TestResource.__table__)
    ...
    async with async_session() as session:
        for tenant in tenants:
            await session.execute(
                text("INSERT INTO test_rbac.test_tenants (id, name, active) VALUES (:id, :name, :active)"),
                tenant
            )
        await session.commit()
...
async def test_role_permissions(test_roles):
    ...
    async with async_session() as session:
        for rp in role_permissions:
            await session.execute(
                text("INSERT INTO test_rbac.test_role_permissions (role_id, permission) VALUES (:role_id, :permission)"),
                rp
            )
        await session.commit()
...
async def test_user_roles(test_users, test_roles):
    ...
    async with async_session() as session:
        for ur in user_roles:
            await session.execute(
                text("INSERT INTO test_rbac.test_user_roles (user_id, role_id) VALUES (:user_id, :role_id)"),
                ur
            )
        await session.commit()
...
def auth_service():
    ...
    auth_service.register_policy(OwnershipPolicy("resource", "update", owner_field="owner_id"))
    auth_service.register_policy(TenantPolicy("resource", "delete", tenant_field="tenant_id"))
...
def test_api(rbac_service, auth_service):
    ...
    async def rls_middleware(request: Request, call_next):
        ...
        tenant_id = "tenant-1" if user_id in ["user-1", "user-2"] else ("tenant-2" if user_id in ["user-3", "user-4"] else None)
        ...
        async with async_session() as session:
            ...
            await session.execute(text(f"SET rls_var.is_tenant_admin = '{is_tenant_admin}'"))
            if tenant_id:
    ...
    async def list_resources():
        ...
        async with async_session() as session:
            result = await session.execute(text("SELECT * FROM test_rbac.test_resources"))
    ...
    async def get_resource(resource_id: int, request: Request):
        ...
        if not await auth_service.authorize(context, "resource", "read"):
            raise HTTPException(status_code=403, detail="Not authorized to read resources")
    ...
    async def create_resource(resource: dict, request: Request):
        ...
        tenant_id = None
        async with async_session() as session:
            result = await session.execute(text("SELECT current_setting('rls_var.tenant_id', true)"))
    ...
    async def update_resource(resource_id: int, updates: dict, request: Request):
        ...
        if not await auth_service.authorize(context, "resource", "update", resource_dict):
        ...
        async with async_session() as session:
            try:
                ...
                updates_filtered = {k: v for k, v in updates.items() if k in ["name", "description", "is_public"]}
                ...
                if result.rowcount == 0:
                    raise HTTPException(status_code=403, detail="Failed to update resource (RLS denied)")
    ...
    async def delete_resource(resource_id: int, request: Request):
        ...
        if not await auth_service.authorize(context, "resource", "delete", resource_dict):
        ...
        async with async_session() as session:
            try:
                ...
                if result.rowcount == 0:
                    raise HTTPException(status_code=403, detail="Failed to delete resource (RLS denied)")
                
                return {"message": "Resource deleted"}
            except Exception as e:
                await session.rollback()
                raise HTTPException(status_code=400, detail=f"Failed to delete resource: {str(e)}")
...
class TestDatabaseRowLevelSecurity:
    ...
    async def test_update_permissions(self, rls_setup, test_resources):
        ...
        async with async_session() as session:
            ...
            await session.execute(text("SET rls_var.is_superuser = 'true'"))
            result = await session.execute(
                text("SELECT id FROM test_rbac.test_resources WHERE owner_id = 'user-2' LIMIT 1")
            )
            resource_id = result.scalar()
            ...
            await session.execute(text("SET rls_var.is_superuser = 'true'"))
            result = await session.execute(
                text("SELECT id FROM test_rbac.test_resources WHERE owner_id = 'user-1' LIMIT 1")
            )
...
class TestRbacApiIntegration:
    ...
    def test_resource_listing_with_rls(self, test_client, test_resources):
        ...
        response = test_client.get(
            "/resources",
            headers={"X-User-ID": "superuser"}
        )
        assert response.status_code == 200
        data = response.json()
        assert len(data["resources"]) >= 5  # Should see at least the 5 original resources
