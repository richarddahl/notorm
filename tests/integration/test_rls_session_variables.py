...
from uno.authorization.rlssql import (
    ...
    AdminRowLevelSecurity, SuperuserRowLevelSecurity
)
    ...
    async with async_session() as session:
        ...
        for tenant in tenants:
            await session.execute(
                text(f"INSERT INTO {test_schema}.test_rls_tenants (id, name, active) VALUES (:id, :name, :active)"),
                tenant
            )
...
async def test_setting_session_variables(test_schema, test_data):
    ...
    async with async_session() as session:
        ...
        await session.execute(
            text(f"SELECT test_rls.set_user_context(:user_id, :is_superuser, :is_tenant_admin, :tenant_id, :email)"),
            {
                "user_id": "user-1",
                "is_superuser": False,
                "is_tenant_admin": True,
        ...
        result = await session.execute(text(f"SELECT * FROM {test_schema}.get_session_variables()"))
        ...
        assert len(variables) <= 2  # Some implementations might keep is_superuser and is_tenant_admin as 'false'
...
async def test_rls_superuser_access(test_schema, rls_setup, test_data):
    ...
    async with async_session() as session:
        ...
        result = await session.execute(text(f"SELECT COUNT(*) FROM {test_schema}.test_rls_resources"))
        count = result.scalar()
...
async def test_rls_tenant_admin_access(test_schema, rls_setup, test_data):
    ...
    async with async_session() as session:
        ...
        result = await session.execute(text(f"SELECT * FROM {test_schema}.test_rls_resources"))
...
async def test_rls_update_permissions(test_schema, rls_setup, test_data):
    ...
    async with async_session() as session:
        ...
        result = await session.execute(
            text(f"SELECT id FROM {test_schema}.test_rls_resources WHERE owner_id = 'user-2' LIMIT 1")
        )
        user2_resource_id = result.scalar()
        
        # Get ID of a resource owned by user-1
        result = await session.execute(
            text(f"SELECT id FROM {test_schema}.test_rls_resources WHERE owner_id = 'user-1' LIMIT 1")
        )
...
async def test_rls_session_persistence(test_schema, rls_setup, test_data):
    ...
    async with async_session() as session:
        ...
        await session.execute(text(f"SELECT COUNT(*) FROM {test_schema}.test_rls_resources"))
...
async def test_multiple_sessions_isolation(test_schema, rls_setup, test_data):
    ...
    async with async_session() as session1, async_session() as session2:
        ...
        result1 = await session1.execute(text(f"SELECT COUNT(*) FROM {test_schema}.test_rls_resources"))
        count1 = result1.scalar()
        
        result2 = await session2.execute(text(f"SELECT COUNT(*) FROM {test_schema}.test_rls_resources"))
        ...
        result1 = await session1.execute(text(f"SELECT * FROM {test_schema}.get_session_variables()"))
        variables1 = dict(result1.fetchall())
        
        result2 = await session2.execute(text(f"SELECT * FROM {test_schema}.get_session_variables()"))
