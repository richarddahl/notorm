...
from uno.database.query_cache import (
    QueryCache, QueryCacheConfig, CacheBackend, CacheStrategy,
    cached_query, set_named_cache, get_named_cache, clear_all_caches
)
...
async def test_distributed_cache_invalidation(distributed_query_cache, setup_test_db):
    ...
    async with async_session() as session:
        ...
        queries = [
            (select(TestCacheItem).where(TestCacheItem.is_active == True), "active_items", ["test_cache_items"]),
            (select(TestCacheItem).where(TestCacheItem.value > 100), "high_value_items", ["test_cache_items"]),
            (select(TestCacheItem).where(TestCacheItem.name.like("Item%")), "named_items", ["test_cache_items"]),
        ]
        ...
        for query, key_suffix, dependencies in queries:
            ...
            await distributed_query_cache.set(cache_key, query_result, dependencies=dependencies)
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_cached_query_with_distributed_cache(distributed_query_cache, setup_test_db):
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_cross_process_cache_synchronization(distributed_query_cache, setup_test_db):
