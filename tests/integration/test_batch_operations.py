...
from sqlalchemy import Column, Integer, String, DateTime, ForeignKey, Boolean, Text, create_engine, Table
...
class TestModel(Base, Model):
    ...
    details = relationship("TestDetailModel", back_populates="parent", cascade="all, delete-orphan")
...
@pytest.mark.asyncio
async def test_batch_transaction_management(batch_operations, detail_batch_operations, sequential_batch_config):
    ...
    try:
        ...
        assert "foreign key constraint" in str(e).lower() or "fk constraint" in str(e).lower()
...
@pytest.mark.asyncio
async def test_multi_table_batch_operations(batch_operations, detail_batch_operations, tag_batch_operations):
...
@pytest.mark.asyncio
async def test_batch_operations_performance(optimized_batch_config, batch_config, sequential_batch_config, database):
    ...
    for strategy_name, ops in strategies.items():
        ...
        await database.execute("DELETE FROM test_batch_models WHERE name LIKE 'Performance Test%'")
    ...
    for strategy_name, ops in strategies.items():
        ...
        print(f"    Average operation time: {metrics.average_operation_time:.6f} seconds")
    ...
    assert performance_results["Optimized (Parallel)"]["insertion_time"] < performance_results["Sequential"]["insertion_time"], \
