...
class HealthCheckRouter(UnoRouter):
    ...
    def endpoint_factory(self):
        ...
        async def endpoint(self) -> HealthStatus:
            ...
            return HealthStatus(
                status=overall_status, details=details, timestamp=datetime.now()
            )
...
class AuditLogRouter(UnoRouter):
    ...
    def endpoint_factory(self):
        ...
        async def endpoint(
            self,
            resource_id: Optional[str] = QueryParam(
                None, description="Filter logs for a specific resource ID"
            ),
            action: Optional[str] = QueryParam(
                None, description="Filter by action type (create, update, delete)"
            ),
            start_date: Optional[str] = QueryParam(
                None, description="Filter logs after this date (ISO format)"
            ),
            end_date: Optional[str] = QueryParam(
                None, description="Filter logs before this date (ISO format)"
            ),
            user_id: Optional[str] = QueryParam(
                None, description="Filter logs by the user who performed the action"
            ),
            ...
            for i in range(1, 50):
                ...
                if log_action == "create":
                    ...
                    changes = {
                        ...
                        "price": {"old": f"{i * 8}.99", "new": f"{i * 10}.99"},
                        "updated_at": log_timestamp.isoformat(),
                    }
                elif log_action == "delete":
                    changes = {"deleted": True, "deleted_at": log_timestamp.isoformat()}
...
class ExportRouter(UnoRouter):
    ...
    def endpoint_factory(self):
        ...
        async def endpoint(
            ...
            filter: Optional[str] = QueryParam(
                None, description="JSON filter criteria"
            ),
            fields: Optional[str] = QueryParam(
            ...
            if filter:
                try:
                    ...
                    raise HTTPException(
                        status_code=400, detail="Invalid filter JSON format"
                    )
            ...
            if format.lower() == "json":
                ...
                if len(data) > 1000:  # Threshold for streaming
                    ...
                    return StreamingResponse(
                        ...
                        headers={
                            "Content-Disposition": f"attachment; filename={self.model.__name__.lower()}_export.json"
                        },
                    )
                else:
                ...
                if len(data) > 1000:
                    ...
                    return StreamingResponse(
                        ...
                        media_type="text/csv",
                        headers={
                            "Content-Disposition": f"attachment; filename={self.model.__name__.lower()}_export.csv"
                        },
                    )
                else:
                    # Small dataset, return directly
    ...
    def endpoint_factory(self):
        ...
        class ImportOptions(BaseModel):
            update_existing: bool = Field(
                False, description="Update items if they already exist"
            )
            skip_validation: bool = Field(
                False, description="Skip validation checks (not recommended)"
            )
            batch_size: int = Field(
                100, description="Number of items to process in each batch"
            )

        class BulkImportRequest(BaseModel):
            items: List[Dict[str, Any]] = Field(
                ..., description="Array of resources to import"
            )
                    ...
                    try:
                        ...
                        if not options.skip_validation:
                            ...
                            if validation_context.has_errors():
                                raise ValueError(
                                    f"Validation failed: {validation_context.errors}"
                                )
                        ...
                        results.append(
                            ImportItemResult(
                                success=False, index=absolute_index, error=str(e)
                            )
                        )
                        error_count += 1
    ...
    def endpoint_factory(self):
        ...
        class WebhookRegistrationRequest(BaseModel):
            ...
            description: Optional[str] = Field(
                None, description="Description of this webhook"
            )
        ...
        async def endpoint(
            self, request: WebhookRegistrationRequest
...
class CustomActionRouter(UnoRouter):
    ...
    def endpoint_factory(self):
        ...
        async def endpoint(
            ...
            if not resource:
                raise HTTPException(status_code=404, detail=f"Resource not found: {id}")
            ...
            if action == "publish":
                ...
                if not workflow_name:
                    ...
                    response["message"] = (
                        f"Resource processed through {workflow_name} workflow"
                    )
                    response["workflow"] = workflow_name
                    response["process_id"] = (
                        f"proc-{id}-{datetime.now().strftime('%Y%m%d%H%M%S')}"
                    )
...
def create_app():
    ...
    # Create the app
    app = FastAPI(
        title="Custom Endpoints Example",
        description="Example API with custom endpoint behaviors",
    )
