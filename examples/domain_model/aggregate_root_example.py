...
from pydantic import BaseModel, Field, model_validator, ConfigDict, PositiveInt, PositiveFloat
...
class Order(EntityBase[str]):
    ...
    def add_item(self, product_id: str, quantity: int, unit_price: float) -> None:
        ...
        if self.status != OrderStatus.NEW:
            raise ValueError(f"Cannot add items to an order with status {self.status.name}")
