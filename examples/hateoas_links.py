...
class LinkBuilder:
    ...
    def build_link(
        ...
        return Link(href=href, rel=rel, method=method, title=title, type=link_type)
    ...
    def build_self_link(self, resource_path: str, title: Optional[str] = None) -> Link:
        ...
        return self.build_link(
            href=resource_path, rel=LinkRelation.SELF, title=title or "Self"
        )

    def build_collection_link(
        self, collection_path: str, title: Optional[str] = None
    ) -> Link:
    ...
    def build_create_link(
        self, resource_path: str, title: Optional[str] = None
    ) -> Link:
    ...
    def build_pagination_links(
        ...
        query_params = {
            k: v for k, v in query_params.items() if k not in ["page", "page_size"]
        }
        ...
        links["self"] = self.build_link(
            self_path, LinkRelation.SELF, title="Current Page"
        )

        # First page
        first_path = f"{base_path}?{query_string_base}&page=1"
        links["first"] = self.build_link(
            first_path, LinkRelation.FIRST, title="First Page"
        )
        ...
        if page > 1:
            ...
            links["prev"] = self.build_link(
                prev_path, LinkRelation.PREV, title="Previous Page"
            )
        ...
        if page < total_pages:
            ...
            links["next"] = self.build_link(
                next_path, LinkRelation.NEXT, title="Next Page"
            )
...
class HATEOASListRouter(UnoRouter):
    ...
    def endpoint_factory(self):
        ...
        async def endpoint(
            ...
            # Add create link
            collection_links["create"] = link_builder.build_create_link(
                resource_path
            ).dict()
...
class HATEOASDetailRouter(UnoRouter):
    ...
    def endpoint_factory(self):
        ...
        async def endpoint(
            ...
            # Build the response
            response = {"data": resource_dict, "_links": resource_links}
    ...
    def endpoint_factory(self):
        ...
        async def endpoint(
            ...
            body: Dict[str, Any] = Body(
                ..., description=f"{self.model.display_name} data"
            ),
        ) -> Dict[str, Any]:
            # Get the base URL for links
            base_url = str(request.base_url).rstrip("/")
            ...
            # Build the response
            response_data = {"data": resource_dict, "_links": resource_links}
    ...
    async def api_root(request: Request):
        ...
        links = {
            ...
            "docs": link_builder.build_link(
                href="/docs", rel="documentation", title="API Documentation"
            ).dict(),
            "openapi": link_builder.build_link(
                href="/openapi.json", rel="describedby", title="OpenAPI Schema"
            ).dict(),
        }
