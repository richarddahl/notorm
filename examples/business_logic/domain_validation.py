...
class Product(UnoObj[ProductModel]):
    ...
    def validate(self, schema_name: str) -> ValidationContext:
        ...
        if (
            hasattr(self, "min_order_quantity")
            and hasattr(self, "max_order_quantity")
            and self.min_order_quantity is not None
...
class Customer(UnoObj[CustomerModel]):
    ...
    schema_configs = {
        ...
        "edit_schema": UnoSchemaConfig(
            exclude_fields={"created_at", "updated_at", "verification_date"}
        ),
        "list_schema": UnoSchemaConfig(
            include_fields={"id", "first_name", "last_name", "email", "is_verified"}
        ),
        "address_schema": UnoSchemaConfig(
            include_fields={
                "id",
                "first_name",
                "last_name",
    ...
    def validate(self, schema_name: str) -> ValidationContext:
        ...
        if (
            hasattr(self, "postal_code")
            and hasattr(self, "country")
            and self.postal_code
            and self.country
        ):
            if self.country == "US" and not re.match(
            ...
            elif self.country == "CA" and not re.match(
                r"^[A-Za-z]\d[A-Za-z] \d[A-Za-z]\d$", self.postal_code
            ):
                context.add_error(
                    field="postal_code",
            ...
            elif self.country == "GB" and not re.match(
                r"^[A-Z]{1,2}\d[A-Z\d]? \d[A-Z]{2}$", self.postal_code, re.IGNORECASE
            ):
                context.add_error(
                    field="postal_code",
...
class OrderModel(UnoModel):
    ...
    status: Mapped[PostgresTypes.String50] = mapped_column(
        nullable=False, default="pending"
    )
    shipping_address_line1: Mapped[PostgresTypes.String255] = mapped_column(
        nullable=False
    )
    shipping_address_line2: Mapped[PostgresTypes.String255] = mapped_column(
        nullable=True
    )
    ...
    billing_address_line1: Mapped[PostgresTypes.String255] = mapped_column(
        nullable=True
    )
    billing_address_line2: Mapped[PostgresTypes.String255] = mapped_column(
        nullable=True
    )
    ...
    subtotal: Mapped[PostgresTypes.Decimal] = mapped_column(nullable=False)
    tax: Mapped[PostgresTypes.Decimal] = mapped_column(nullable=False, default=0)
    shipping_cost: Mapped[PostgresTypes.Decimal] = mapped_column(
        nullable=False, default=0
    )
    total: Mapped[PostgresTypes.Decimal] = mapped_column(nullable=False)
...
class OrderItemModel(UnoModel):
    ...
    unit_price: Mapped[PostgresTypes.Decimal] = mapped_column(nullable=False)
    subtotal: Mapped[PostgresTypes.Decimal] = mapped_column(nullable=False)
    created_at: Mapped[datetime] = mapped_column(nullable=False, default=datetime.now)
    updated_at: Mapped[datetime] = mapped_column(
        nullable=False, default=datetime.now, onupdate=datetime.now
...
class OrderItem(UnoObj[OrderItemModel]):
    ...
    def validate(self, schema_name: str) -> ValidationContext:
        ...
        # Subtotal validation
        if (
            hasattr(self, "quantity")
            and hasattr(self, "unit_price")
            and hasattr(self, "subtotal")
...
class Order(UnoObj[OrderModel]):
    ...
    schema_configs = {
        "view_schema": UnoSchemaConfig(),
        "edit_schema": UnoSchemaConfig(
            exclude_fields={"created_at", "updated_at", "order_date"}
        ),
        "list_schema": UnoSchemaConfig(
            include_fields={
                "id",
        ...
        "shipping_schema": UnoSchemaConfig(
            include_fields={
                "id",
    ...
    def validate(self, schema_name: str) -> ValidationContext:
        ...
        if (
            hasattr(self, "subtotal")
            and hasattr(self, "tax")
            and hasattr(self, "shipping_cost")
            and hasattr(self, "total")
            and self.subtotal is not None
        ...
        if hasattr(self, "shipping_country") and self.shipping_country:
            valid_shipping_countries = [
                "US",
                "CA",
                "GB",
            ]  # Example limited shipping countries
        ...
        if (
            hasattr(self, "status")
            and hasattr(self, "payment_method")
        ...
        if (
            hasattr(self, "status")
            and hasattr(self, "tracking_number")
            and self.status == self.STATUS_SHIPPED
    ...
    async def ship(self, tracking_number: str) -> bool:
        ...
        if (
            self.status != self.STATUS_CONFIRMED
            and self.status != self.STATUS_PROCESSING
        ):
            raise UnoError(
...
async def test_product_validation():
    ...
    invalid_product = Product(
        ...
        tags=[
            "",
            "very-long-tag-that-exceeds-the-fifty-character-limit-for-tags",
        ],  # Invalid tags
        inventory_count=100,
...
async def test_api_integration():
    ...
    with patch("uno.obj.UnoObj.get", new_callable=AsyncMock) as mock_get:
        ...
        with patch.object(Product, "save", new_callable=AsyncMock) as mock_save:
            ...
            response = await update_product_endpoint(
                "product123", {"price": Decimal("39.99")}
            )
            assert response["status"] == "success"
            ...
            response = await update_product_endpoint(
                "product123", {"price": Decimal("-10.00")}
            )
